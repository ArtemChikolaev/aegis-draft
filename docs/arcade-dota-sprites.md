...
(не `_hat`, `_weapon` и прочие части косметики — базовая модель героя уже содержит стандартный сет).
...
- `--pitch 58` — наклон камеры как в Dota (90 = строго сверху). `--yaw-offset 90`, если модель
