...
- **Платье/плащ выходит светлым «голым» телом** (аркана PA): в текстуре ткань белая, светящийся узор — в альфе. `--white-to R,G,B[,порог]`
  красит почти белые непрозрачные пиксели в цвет ткани до `--glow-from-alpha`. Проверить альфу: Blender numpy по PNG (доля a<0.5, яркость).
