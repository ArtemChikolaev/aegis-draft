...
- **Земля:** `web/scripts/pixel_terrain.mts` (Playwright-canvas, без sharp/PIL) делает из 512-px `dota/terrain/<name>.png` пиксельные
  128-px текстуры в `dota_px/terrain/<name>.png`. В репозитории и в загрузчике земля — только WebP: исходник PNG взять из экспорта §4
  (или распаковать `dwebp`), результат перегнать `cwebp -lossless` в `<name>.webp`. Сами текстуры: усреднение до 64, 5 ступеней яркости лестницей контраста вокруг среднего цвета, затемнение под тон карты
