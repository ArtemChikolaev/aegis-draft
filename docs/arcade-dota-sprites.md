...
Гардероб собирает облик из частей разных сетов (меч одного, шлем другого), как в Dota. Пре-рендеренных
листов на все сочетания не напасёшься, поэтому лист собирается в рантайме: **тело без сменных частей**
(`<hero>+body`) плюс **слой на каждую часть** (`<hero>+<источник>.<слот>`, источник — `base` или сет
`<hero>@<set>`), слои рисуются друг на друга в порядке слотов (`features/arcade/sprites.ts`,
`compositeSheet`). Слой = разница рендеров «тело» и «тело + части слота» (`sprite_part_layer.mts`):
Workbench не умеет holdout, а разница даёт ровно видимые пиксели части с правильным перекрытием телом.

Два условия, без которых слои не лягут: **одно кадрирование** у всей семьи (центр, высота и охват
меряются пробным рендером силуэта, у голого тела и тела с плащом они разные) — `--fit-from <образец.json>`
в `render_dota_sprites.py` берёт поле `fit` из меты листа-образца (базовая строка героя, `<hero>+ref`);
и **сырые листы** без палитры — `RAW=1` в `dota_pipeline.sh` (разные палитры двух листов дают шум по
всему телу). Слот части — по имени файла (`scripts/lib/dota_slots.mjs`, общий с `qa_set_slots.mjs`).

```bash
cd web && D=/tmp/parts
npx tsx scripts/dota_part_layers.mts plan --heroes juggernaut,lina --dir $D      # семья: +ref, +body, +<src>.<slot>
...
npx tsx scripts/dota_part_layers.mts build --heroes juggernaut,lina --dir $D     # слои → public, манифесты слоёв, parts.ts
```

`build` печатает долю расхождения композита «тело + базовые слои» с образцом — это и есть проверка, что
слои легли (ожидаемо доли процента: отсев одиночных пикселей и тени между частями). Слои живут в
`dota_manifest_parts_{px2,px}.tsv` (не в основном манифесте: их не рендерят напрямую), таблица для игры —
`src/game/arcade/content/parts.ts` (генерируется). Смешивание только на базовом теле: арканы и персоны с
другой моделью — цельные облики. Сет со стилями (`~style`) в слоях — в базовом стиле.
