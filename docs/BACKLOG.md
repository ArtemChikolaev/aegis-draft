...
| Специализация актов и поздние цели | Dire-очищение/свет, River-переправы, поздняя подготовка; затем кузня/разлом/караван | Акты отличаются решениями; события не требуют недоступного билду действия; финал и путь к нему всегда проходимы; у разлома определено поведение часов | Плейтест целей карты и чемпионов; не начинать с четвёртого биома |
