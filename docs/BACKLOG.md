...
- **T6.4 Meta progression — только после T6.3 🟨 (Stakes ✅, Штаб срез 1 ✅ 2026-09-02, Playbook ⬜):** постоянный **Штаб** хранит коллекцию открытых определений карточек, статистику и трофеи, но не усиленные экземпляры. **Playbook** (гипотеза ≈8 карточек) выбирается перед обычным забегом и ограничивает reward/market pool, не давая бесплатной силы со старта; daily/challenge получает фиксированный Playbook, seeded run учитывает `playbookId + balanceConfigVersion`. После первой победы открываются **Stakes**: новые ограничения/boss/economy trade-off вместо простого роста OVR и без постоянного `+OVR`. ⬜
  ...
  - ⬜ **Playbook (срез 2).** Вопросы к пользователю: (а) гейтить ли новичку пул наград до открытия карт в Штабе (PRD говорит «за milestones открываются определения») или Playbook — только добровольный фильтр ≈8 карт; (б) `playbookId` в run-link/сейве и в `sim_run` (иначе A/B баланса по Playbook невозможен); (в) фиксированный Playbook дейлика.
