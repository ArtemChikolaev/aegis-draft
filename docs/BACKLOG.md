...
- 🟨 **R12.6 — потолок билда наступает к этапу ~22 (вход в Династию). Спека согласована
  ...
  **trade-in карт с переносом тира −1** как главный сайдгрейд. Порядок работ из спеки:
  ...
  - 🟨 **LG1 — trade-in карточки: реализовано 2026-08-10 (b1.28.0), калибровка цены — за A/B.**
    «Обменять» (⇄ рядом со сбросом ✕) на экипированной карте → тройка офферов из невзятого пула
    тактик+предметов (поток `:trade`, реролл по лестнице рынка со своим счётчиком), новый тир =
    текущий − 1 (пол common; тактика отдаёт common), Edition и заряды сгорают, отданная карта не
    выпадает больше никогда; цена `tradeInCost = 4` — синк золота. Превью дельты на офферах — тем
    же `evaluateCampPower`, что текущая сила. Агенты сима меняются через общий runStrength-слой
    (`trd` в отчёте); «осмысленность» лагеря учитывает trade лениво. Тесты: 6 unit + e2e (обмен и
    реролл, оба датасета). ⚠️ Смок 40 сидов показал заметный рост винрейта — решение по цене
    после честного A/B 400×400 на одних сидах (крутится).
