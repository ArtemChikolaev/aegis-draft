...
> **Ключевое ограничение источника:** `/proMatches` — это rolling-лента по времени (пагинация `less_than_match_id`); достать матчи конкретной старой лиги back-пагинацией нереально (десятки тысяч страниц через не-tier-1). Нужен **league-targeted discovery**. ПЕРВЫЙ ШАГ (verify до кода): подтвердить на free-tier путь получения match_id по `leagueId` — приоритет `/explorer?sql=SELECT match_id, leagueid, start_time FROM matches WHERE leagueid IN (...)` (SQL, free), фолбэк — существующая proMatches-пагинация. Проверить один живой запрос, зафиксировать в `external-data-etl`.
- **A. Rolling-окно = last_5y (одно широкое, вложенное).** `1y ⊂ 2y ⊂ 5y` уже даёт `formats.Assign`, так что собираем самое широкое окно один раз. Discovery по tier-1 league_id через explorer (не тратим бюджет на пагинацию через не-tier-1). Details `/matches/{id}` копятся в raw-кэше между прогонами (resumable, как сейчас).
- **B. valve_legacy = отдельная ось по league_id.** Для каждой курируемой лиги (все TI + `valveMajors`) explorer `SELECT match_id WHERE leagueid=X` → match_id → details. Вне времени, отдельный discovery-проход; те же details-кэш/normalize/aggregate. Список TI id — добрать из `/leagues` по имени, Major id уже курируются в `tier1.valveMajors`.
- ✅ **C. Эмит career-глубины (чинит Yatoro/Hero Synergy).** [c96825d] Проведено через контракт: `schema/careerPlayerHeroStats.schema.json`, `model.Dataset.CareerPlayerHeroStats`, `domain.Build` прокидывает `aggregate.CareerPlayerHeroStats`, `emit` пишет `careerPlayerHeroStats.json`, TS-тип + `DataSource` (грузит **опционально** — деплой фронта не зависит от тайминга data-refresh), мок-генератор. `heroStatsForAssignment`: career — широкая база player×hero, окно/событие уточняют свежесть. `PlayerInspector` читает реальный career. Бамп `ratingModelVersion` v1.2.0→v1.3.0 (вход Hero Synergy изменился; сейвы инвалидируются). Go build/vet/test + gofmt, tsc, verify, validate:data (career-схема), build, антипаттерны — зелёные. **Эффект появится после первого прогона пайплайна с этим кодом** (career-файл наполнится). ⚠️ **Заменено на point-in-time (TDATA3):** lifetime-подход (`{account→hero}` из `/players/{id}/heroes`) переделан на as-of-event `{eventId→account→hero}` из match-level (см. TDATA3). career-**эндпоинт** пока фетчится, но его результат больше не эмитится — **cleanup: убрать fetch** (кэширован, ~0 сети, но чистит код/бюджет).
- **D. Кап матчей на лигу.** Текущий 25 — слишком тонко для стат. Поднять `--max-matches-per-league` (ориентир 100–150; топ-события отдают глубину player×hero, но бюджет ограничен). last-run потратил 843/2000 — запас есть, но с 5y объём вырастет → сойдётся за несколько дневных прогонов.
...
### TDATA3 — Темпоральная модель: point-in-time (as-of-event) статистика + окна 🟨
> **Решение 2026-07-13 (после анализа реальных данных 322-0):** 322-0 использует career-пул **пожизненный «на сейчас»** (один блок на игрока, `playerHeroStats` — 2700+ игр, не по датам), а OVR — per-event. То есть point-in-time для пула героев — **осознанное ОТКЛОНЕНИЕ** от референса, а не парити (по продукт-решению: эпохо-точность честнее). Реализуемо **чисто на нашей стороне** (билд-агрегация из уже собранных match-level матчей), **CI/сбор не меняется**.
> ✅ **player×hero — сделано** ([коммит ниже]): `careerPlayerHeroStats` теперь `{eventId→account→hero→stat}` = кумулятив игрока из матчей с датой ≤ конца события (`domain.buildCareerToEvent`); `heroStatsForAssignment` берёт срез по `candidate.eventId`; schema/model/type/mock/`PlayerInspector` обновлены; Go+tsc+verify+validate+build зелёные. Сбор не тронут — работает на текущих данных, глубже станет с TDATA2 A/B.
> ⬜ Осталось: (1) Chemistry point-in-time (сейчас из lifetime `/peers`); (2) cleanup — убрать неиспользуемый career-`/players/{id}/heroes` fetch.
> Решение продукта 2026-07-13: пак — это **снимок на дату турнира**. Для пака события E у игрока
> НЕ должны учитываться турниры **после** E (напр. пак «Yatoro @ TI2025» не тянет EWC2026). Это
> относится к player×hero (пул героев) и Chemistry; Base/OVR уже per-event и корректен.
> **Следствие:** пожизненные эндпоинты `/players/{id}/heroes` и `/players/{id}/peers` дают статистику
> «на сейчас» (без временной отсечки) → они **непригодны** для point-in-time. Правильный источник —
> **match-level данные с датами** (их мы и так тянем через `/matches/{id}`), агрегируемые на билд-этапе
> с отсечкой по дате. Это совпадает с уже записанным принципом PRD «окна строим из match-level, а не из
> career-эндпоинтов». Значит TDATA2-C (career из lifetime-эндпоинта) — **временный стоп-гэп**, здесь заменяется.
- **Два независимых временных понятия — не путать:**
  1. **Окно (1y/2y/5y)** — какие СОБЫТИЯ в драфт-пуле (дата события в пределах N лет от `as-of`). Ограничивает пул (существующее поведение).
  2. **As-of-event отсечка** — внутри выбранного пака (событие E) стата игрока считается только по матчам с датой **≤ E** (верхняя граница). Новое.
- **Принцип:** для пака (E, P) все вспомогательные статы игрока (player×hero, Chemistry) = кумулятивно из его собранных матчей с датой ≤ E (tier-1 срез = tier-1-карьера на момент E). Base/OVR — уже событийный.
- **Модель данных / «БД» (static-first, БД для игровых данных НЕТ — только сервер под юзер-стейт, ADR 0002):**
  - игровые данные остаются предрассчитанным JSON на CDN; «выбор промежутка» — это НЕ запрос в БД, а фильтр/срез по предагрегированным полям;
  - билд-этап из нормализованных match-level матчей считает **per-(event,player) кумулятивную** статистику как-of-event: новая эмит-структура (напр. `eventPlayerHeroStats` = пул героев игрока на момент события) вместо lifetime-career для скоринга; Chemistry-пары так же point-in-time (из match-level тиммейтов до E, не из `/peers`);
  - фронт: выбор окна фильтрует события пула; сам пак уже несёт as-of-event статы → пост-фильтрация не нужна, детерминизм по `seed + dataset version` сохранён.
- **Что меняется против текущего:**
  - `/players/{id}/heroes` + `/peers` убираются из скоринг-входов (оставляем только если введём явный режим «all-time»); TDATA2-C lifetime-as-base — интерим, заменяется этой моделью;
  - Chemistry (TDATA1) пересобирается point-in-time из match-level тиммейтов, а не из lifetime `/peers`;
  - per-league cap (TDATA2-D) становится важнее: больше матчей = точнее кумулятивная стата.
- **CI-сбор (итоговая картина):** две оси TDATA2 (rolling 5y по explorer + valve_legacy по league_id) наполняют raw-кэш match-level деталями (с датами) по дневному крону, resumable, сходится за N прогонов. Агрегация — чистый детерминированный билд-шаг над накопленными матчами (версионируется `ratingModelVersion`).
- **Файлы:** `pipeline/internal/{aggregate,domain}` (as-of-event агрегаты), `internal/model` + `schema/` + `web/src/{types,game/score.ts}` (новая структура вместо/рядом с career), при отказе от lifetime — упростить `pipeline.go` (убрать career/peers-обогащение из скоринга).
- **DoD:** для пака события E пул героев/Chemistry игрока не содержат вклада турниров после E (тест на 2 события одного игрока: ранний пак ≠ поздний); окно выбирает пул событий; детерминизм сохранён; JSON Schema зелёная.
- **Deps:** TDATA2 A/B (нужна глубина match-level, чтобы кумулятив до E был осмысленным), `scoring-model` (это меняет вход Hero Synergy/Chemistry → бамп `ratingModelVersion`), `data-contract`.

- **TDATA1/2 хвост — Цель:** собрать tier-1 срез за несколько событий/окон так, чтобы `squadSynergy`/`teammates` содержали **пожизненные кросс-командные co-games**, а `playerHeroStats`/`eventHeroStats` — достаточную глубину player×hero.
