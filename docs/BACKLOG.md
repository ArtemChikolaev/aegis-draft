...
- ⬜ **Дальше волнами по 8:** Omniknight, Abaddon, Beastmaster, Brewmaster, Centaur, Dark Seer, Death Prophet, Disruptor → Lycan, Lone Druid, Alchemist, Bane, Batrider, Bounty Hunter, Broodmother, Clockwerk → … Порядок — по популярности в датасете (`heroes.json` / picks). После каждой волны — `npm run sim:arcade` по новым героям, полоса 30–50% в разминке.
