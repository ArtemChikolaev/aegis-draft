...
- **Alchemist: мечи всегда в руках, а бьёт кулаками (владелец 2026-09-12).** В Dota мечи в ножнах за спиной и достаются только в Chemical Rage (клипы `ability_4_*`: `pullout`, `idle`, `run`, `attack1-3`, `putback`). Сделано: из базовой строки и обоих сетов убран `alchemist_sword`/`*_weapon` (остаётся `scabbard` — мечи за спиной, атака кулаками `attack1`), добавлены строки формы `alchemist@meta`, `alchemist@frankenstein@meta`, `alchemist@jungle_chief@meta` с мечами и клипами ярости; у R (`frenzy`) появился `form` — `formNow()` теперь берёт любую способность с `form`, а не только `metamorphosis`, и `frenzy` с формой ставит `formUntil` на время бафа; тип атаки и дальность не меняются (форма с `range` = база 88). Гардероб показывает кнопку «Форма» по наличию `form`. Тест в `arcadeForm.test.ts` + проверка манифестов (у не-@meta строк Alchemist нет оружия в руках).
