...
- ⬜ **Дальше волнами по 8:** Earth Spirit, Elder Titan, Ember Spirit, Grimstroke, Gyrocopter, Keeper of the Light, Magnus, Mars → Meepo?, Morphling, Naga Siren, Nature's Prophet, Nyx, Oracle, Outworld Destroyer, Pangolier → … Порядок — по популярности в датасете (`heroes.json` / picks). После каждой волны — `npm run sim:arcade` по новым героям, полоса 30–50% в разминке.
