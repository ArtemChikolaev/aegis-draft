...
*(R12.1–R12.3 закрыты 2026-07-30, `BALANCE_CONFIG_VERSION b1.18.0 → b1.19.0`; R12.4–R12.7 открыты)*
...
- ⚠️ **R12.4 — кривая редкости насыщается к этапу 5 и дальше не растёт.** Жалоба: «пассивки
  ...
  **Направление:** растянуть шаг под 25 этапов (насыщение ближе к 18–20), и поднять потолок
  `immortal/arcana` — сейчас 15 на сотню при том, что это единственный тир, который в Династии
  ещё что-то меняет.
- ⚠️ **R12.5 — два боссовых условия из пяти мертвы к середине сезона.** Жалоба: «условие босса —
  ...
  **Причина** — пороги в `BOSSES` заданы абсолютными константами под диапазоны, из которых игра уже
  ...
  `heroBan` тоже.
  **Направление:** порог обязан быть ОТНОСИТЕЛЬНЫМ — от кривой поля этапа или от собственного
  состава игрока (как это уже сделано у `chemistryBlackout`), а не константой. Это же снимает
  необходимость перекалибровывать пороги при каждом изменении экономики.
