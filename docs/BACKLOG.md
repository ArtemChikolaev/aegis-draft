...
- ⬜ **Тот же класс, полный список (аудит слотов, не начато):** скрипт сверки «части базовой строки ↔ части сета» нашёл ещё ~90 строк, где сет не перекрывает базовый слот (например `alchemist@frankenstein` без гоблина-наездника, `gyrocopter@dwarven_gyrocopter` без винта, `tinker@mecha_hornet` без рук, `warlock@greevil_master` без мантии). Часть — ложные срабатывания эвристики по именам, поэтому каждую строку надо смотреть глазами; заводить отдельной задачей.
