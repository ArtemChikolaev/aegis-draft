...
*(R14.1–R14.7 закрыты; открыт вопрос компенсации сужения)*
...
- ✅ **R14.7 — рынок сужен до трёх карт в паке (2026-08-04).** `MARKET_PACK.size = 3` вместо «карта
  ...
  - ⚠️ **Оговорка к замеру:** агенты почти не реролят (`rrl` 0.72), а вся задуманная контригра —
    покупка вариантов рероллом. Sim показывает цену сужения БЕЗ компенсации. **Открытый вопрос до
    плейтеста:** оставить 3+3 как есть (забег честно тяжелее), вернуть 4+4 как компромисс или
    удешевить реролл, сделав контригру реальной.
