...
- ⬜ **R12.6 — потолок билда наступает к этапу ~22, дальше покупать нечего (вход в Династию).**
  Жалоба: «к этой волне я уже полностью уперся в потолок наших апгрейдов — мне даже героев нет
  смысла свапать» (Stage 22: 3/3 тактик, 2/2 действий, 44 золота, 14 доступных рероллов; Stage 25
  выжато 176 силы).
  Это НЕ новый диагноз, а подтверждение `T5.8`/`PF-1` живым забегом: симулятор на 120 сидах даёт
  «качество на максимуме 39% лагерей, слоты полны 22%». Отличие от `T5.9`: там синк лечил
  *убегающее золото*, здесь исчерпан сам **набор осей роста**. Три оси (тир героя, тир карточки,
  форма игрока) конечны и упираются в потолок примерно одновременно.
  **Важно:** бесконечный `+OVR` PRD §5.9.2 запрещает и запрет остаётся в силе — «штатный финал
  Династии = Loss». Нужна не инфляция силы, а **бесконечная вариативность при конечном потолке**:
  сайдгрейды (обмен одной оси на другую), пере-специализация билда, расширение слотов как
  разменная монета, растущая угроза быстрее, чем растёт доступная сила. Конкретный набор — за
  отдельной спекой; R12.1–R12.5 являются её предусловием, потому что три из четырёх осей роста
  сейчас работают не так, как задумано, и мерить потолок на сломанных осях бессмысленно.
...
- **R13.5 — Editions как ВТОРАЯ ось вместо новых тиров редкости.** Совпадает с уже принятым
  ...
  - `Charged` (заряды за выполненное условие, потолок 3) — лучший из трёх: контролируемый рост с
    конечным потолком, растёт вместе с билдом, теряется при поломке условия. Брать первым.
...
- ✅ **R14.6 — build rail: билд виден на всех экранах забега (2026-08-04).** `BuildRail` —
  ...
  - ⬜ **Осталось от исходного замысла: вспышка карточки В МОМЕНТ срабатывания во время reveal.**
    Честная оговорка: у нас тактика не «стреляет» — это статический модификатор этапа, а не
    событие по ходу турнира. Чтобы вспышка не врала, нужен либо по-матчевый вклад в модели, либо
    осознанное решение, что вспышка играет один раз на старте симуляции как «вот что участвует».
...
### R15 — Аудит 2026-08-09: game feel / juice против Balatro ⬜
...
- ⬜ **R15.1 — отклик покупки/апгрейда (P1).** Сейчас карточка рынка исчезает без exit-анимации
  (grid схлопывает ячейку), а Run power в радаре подменяется мгновенно. Сделать: (1) exit-состояние
  карточки — короткий подъём+fade (~200ms) до удаления из DOM (паттерн: держать «уходящий» оффер в
  локальном UI-state `MarketPanel` до `onAnimationEnd`, движок не трогать — `consumed`-офферы уже
  отфильтрованы в `currentMarketOffers`); (2) вспышка направления на центре радара — переиспользовать
  правило `--motion-flash` из вспышки золота (`CampScreen` `data-gold-direction`), НЕ count-up:
  урок R14.4 — набегающее число на rAF врёт в неактивной вкладке, а Run power — цифра решения.
  (3) Пульс слота BuildRail при новой карте — тот же `.enter`. Файлы: `MarketPanel.tsx`,
  `CampScreen.tsx`, `camp.css`/`base.css`, `BuildRail.tsx`. DoD: `getAnimations()` показывает exit
  при покупке; значение силы всегда точное; reduced-motion гасит всё; e2e лагеря зелёные
  (порядок Rng не меняется — правки чисто презентационные).
- ⬜ **R15.2 — секвенция «этап пройден» + переход этап↔Буткемп (P1).** Поглощает остаток R14
  («переход — уровень `app/App.tsx`»). Сейчас payout — статичная строка в шапке лагеря, переход —
  `fade-soft` 360ms. Сделать промежуточный шаг между `finishTournament()` и лагерем: перекрытие
  на тёмном фоне с последовательным появлением строк «Target: top 8 → 3rd ✓» / «Prize +3» /
  «Placement +3» / «Interest +N» (стаггер ~120ms, итог золота — точный, анимируется появление строк,
  не цифры), затем вход в лагерь существующим `enter-fade` + раздача. Это **presentation shortcut**:
  все числа берутся из уже посчитанного `prizeBreakdown` (`anteEconomy.ts`), движок и Rng не трогаются;
  скип по клику. Той же секвенцией оформить смерть забега (промах порога) и победу сезона
  (проверить `SeasonVictory` живьём — в аудите не дошёл). DoD: секвенция скипается кликом; resume
  посреди секвенции не дублирует награду (`finishTournament` идемпотентен); golden Classic не тронут.
- ⬜ **R15.3 — reveal турнира как драматургия (P1).** Сейчас строки Match results появляются без
  анимации (running только `live-pulse`), свой матч никак не выделен. Сделать: (1) вход строки
  результата — slide+fade ~240ms (глобальный кейфрейм); (2) матч своей команды — рамка/фон на
  `--win`/`--loss` вспышкой в момент появления; (3) пересортировка таблицы групп — FLIP-анимация
  позиций (мера: только transform, без layout-таймеров); (4) финальная таблица — «печать» строки
  своей команды (scale-in бейджа места). Опционально: синг на босс-этапе при входе в field-стадию
  (пульс `StageKindBadge`). Файлы: `TournamentScreen.tsx`, `tournament.css`. DoD: `Skip phase`/
  `Show result` мгновенно доигрывают без осиротевших анимаций; reveal-тайминги не меняют
  `tournamentStep`-семантику; mobile без горизонтального оверфлоу.
- ⬜ **R15.4 — shake/impact-бюджет (P2, после R15.1–R15.3).** Тряска только на largest-tier событиях
  (Aegis взят, смерть забега, босс завален) — ±2–4px, ≤300ms, `transform` на контейнере экрана.
  Обязательно: уважает `prefers-reduced-motion` (глобальное правило уже гасит) и **отдельный тумблер
  в Settings** («Screen shake», default on) — Balatro его тоже даёт. Не вешать shake на частые события —
  инфляция откликов убивает лестницу эскалации.
...
- ⬜ **R15.6 — hover tilt карточек (P2, дёшево).** Лёгкий 3D-tilt по позиции курсора на
  `camp-inspectable-card` (CSS `perspective` + `transform`, JS только `--mx/--my` переменные),
  синергирует с уже живой фольгой `card-foil`. Touch — без tilt (нет hover). Бюджет: transform-only,
  без re-layout.
- ⚠️ **R15.7 — «пропавшие кнопки» после reveal групп (unknown, чистый repro нужен).** В аудите один
  раз после полного reveal групповой стадии исчезли все action-кнопки (`Skip phase`/`Show result`/
  advance) — тупик без CTA; reload+resume вернул кнопки. Прогон шёл под HMR параллельной сессии
  (правки `engine.ts` на лету), поэтому багом не записываю. Проверить в чистой сессии: полный reveal
  групп до конца БЕЗ Show result → должна появиться кнопка перехода к плей-офф. Воспроизведётся —
  это P0 (тупик забега).
