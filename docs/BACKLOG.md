...
- ⬜ **Инкремент 2 — коллектор + wiring.** Resumable сбор `/peers` для всей pro-вселенной (по паттерну career-heroes в `collect`), tier-1 фильтр лиг в сборе матчей, вызов `MergePeers` в `pipeline`/`domain.Build`, CLI-флаг; бюджетный прогон.
