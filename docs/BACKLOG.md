...
- 🟨 **R13.3 — два уровня информации: карточка = решение, инспектор = математика.** *(рынок сделан
  ...
  - ⛔ **`SOLD`-заглушка НЕ делается — замер показал, что чинить нечего.** Пак использует
    ...
    косметики не стали.
  - ⬜ **Осталось:** reward-карточки трогать не потребовалось — они уже показывают «что делает +
    цена», а не `до → после` (проверено на живом экране). Открытым остаётся панель Build
    (описания экипированных карточек) — но там текст и есть содержание слота.
- **R13.4 — разделение `CampScreen`** на `RewardPanel` / `MarketPanel` / `BuildPanel` /
...
#### Порядок, который предлагаю (отличается от плана)

1. **R13.1 + R13.2** — два подтверждённых бага, это часы.
2. **R13.3 + R13.4** — компактная карточка, инспектор, разделение экрана. Это и есть ответ на
   исходную жалобу; всё остальное ждёт.
