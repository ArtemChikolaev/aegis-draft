...
### T13.34 — Геймпад DualSense (владелец 2026-09-07) ⬜
Gamepad API: левый стик — движение, правый — направление атаки/каста (или авто), кнопки ×/○/□/△ — Q/W/E/R, R1 — подобрать, L1 — сборка, Options — пауза, тач-пад/⊕ — вспышка. Игра должна сама понять, что ввод идёт с геймпада (событие `gamepadconnected` + первое движение стика), и переключить подсказки HUD на глифы кнопок (слой поверх `arcade.hud.*`, i18n не трогать — глифы SVG из `ui/`). Меню/карточки — навигация стиком + × выбрать / ○ назад. Детерминизм: геймпад даёт тот же `ArcadeInput` через `input.ts`, в лог попадают те же `act`.
