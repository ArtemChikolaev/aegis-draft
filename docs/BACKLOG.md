...
- ✅ **Срез 1 (2026-08-12): играбельная сезонная петля по образцу 322-0.** Онбординг (имя/6 регионов-flavor/сложность = доход $80–120k) → трайауты 8×5 из реальных pack-игроков со **скрытыми зарплатами** ($-бенды) + 1 реролл → пул 12 героев (4×3) → переговоры: раскрытие зарплат + 5 дешёвых филлеров по ролям → отбор пятёрки 1C/1M/1O/2S под доход (подписать дороже дохода нельзя) → сезон: 5 циклов «2×tier2 → квалификация(top-2) → онлайн → LAN» + отбор(top-1) + финал «Aegis Championship»; мгновенный розыгрыш (KO-сетка, ELO-формула 322-0 `10^(−Δ/22)`), призовые по измеренным таблицам, ELO-рейтинг 17 бот-оргов, лента, календарь → оффсезон: дрифт ±3, пересмотр зарплат, release с заменой филлером → Season N+1.
  ...
  - ⬜ **Осталось (срез 4+):** live-reveal финала через полноценный `TournamentEngine` (сейчас мини-сетка без анимации); шард-мета/Hall of Legends (решить связку с T6.4 Stakes); запись сезона в карьеру (CareerEntry не подходит по форме); калибровка экономики симом (`scripts/sim_manager` по образцу sim_run: приток кэша от rival/событий, окупаемость дорогой пятёрки, доля прохода квалификаций).
- **Прежний контекст:**
...
- **MP1 — турнир комнаты на общем сиде, драфт из личных паков ⬜.** Комната → ready → общий сид →
  ...
  детерминирована, сервер раздаёт только seed и составы). Career-запись видом `arena`. Mode
  shell: четвёртая карточка `"arena"`, красный акцент. Играбельный мультиплеер почти без новых
  правил — общего пула ещё нет. DoD: полный турнир двумя браузерами + ботами на живом сервере;
  обрыв любой стороны не ломает комнату; e2e двумя контекстами Playwright.
- **MP2 — целевая форма: общий пул + одновременные раунды ⬜.** Приоритет посадки по сиду комнаты,
  змейка инвертируется каждый раунд; раунд ~25с — все выбирают одновременно (пик + необязательный
  запасной), сервер резолвит атомарно (конфликт → лучший приоритет, проигравшему запасной или
  авто-пик). 5 раундов игроков (пик занимает свободный ролевой слот) + 5 раундов героев
  (уникальны глобально; автоназначение лучшему слоту, свап внутри команды до лока). Серверная
  валидация пика — Go-подмножество правил (роль-слот/занятость/пул), полный счёт — клиентский.
  Экран драфта в стиле Dota ban/pick. DoD: комната 18 (люди+боты) проходит драфт за ≤10 минут;
  таймаут/дисконнект ведёт бот-политика; спектатор-режим (read-only ws) — дешёвый бонус шага.
...
- **T11.1 — Service worker: оболочка + автопрекэш датасета ⬜.**
  ...
  - **DoD:** прод-сборка открывается после `setOffline(true)` и полный классический забег проходится без сети; `data-refresh` с новым `dataHash` не рвёт активный забег (кэш переключается после него); тост апдейта не появляется в забеге; `npm run build` + `tsc --noEmit` чисты.
- **T11.2 — Зеркало арта в репозитории ⬜.**
  ...
  - **DoD:** офлайн-забег идёт с портретами и логотипами; удаление файла из `art/` не ломает экран (виден CDN-фолбэк, затем монограмма); прирост репозитория ≈1.7 МБ; скрипт идемпотентен и не ходит в сеть при повторном запуске без новых сущностей.
- **T11.3 — Проверка связности и офлайн-состояние онлайн-режимов ⬜.**
  ...
  - **DoD:** при офлайне карточка Arena показывает причину и не ведёт в мёртвый экран; при живом интернете, но недоступном API — тот же честный экран, а не бесконечный спиннер; одиночные режимы не задеты; юнит на «онлайн-флаг врёт, health не отвечает».
- **T11.4 — Панель «Офлайн» в настройках + установка на устройство ⬜.**
  ...
  - **DoD:** статус отражает реальное содержимое кэша (не константу); «очистить» действительно освобождает место и возвращает статус в «нет»; установка проверена живьём на Android и iOS.
  - **Deps:** T11.1.
- **T11.5 — Офлайн-регресс в тестах ⬜.**
  ...
  - **Грабля:** SW и Playwright требуют прод-сборки (`vite preview`), на dev-сервере проверка бессмысленна; прогон — `--workers=1` (флейк параллельного mobile-контекста уже ловили).
  - **DoD:** спека зелёная локально и в CI; падает, если убрать регистрацию SW.
  - **Deps:** T11.1, T11.3.
