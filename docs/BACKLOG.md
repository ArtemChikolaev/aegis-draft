...
- ✅ **Срез 1 (2026-08-19): режим играбелен конец-в-конец.** Решения зафиксированы по данным:
  ...
  **минус ВСЕ аккаунты события** (лочится событие целиком, не только топ-17 поля).
  ...
  - ⬜ **Дальше (срез 2+):** underdog-проекция как явный челлендж (§2.5.1) — projection уже есть,
    нужен акцент подачи; тематические фильтры пула («только TI-чемпионы», §2.5.2); alt-вариант
    «прими реальную команду» (§2.5.3); point-in-time career для героики эпохи (отложено из
