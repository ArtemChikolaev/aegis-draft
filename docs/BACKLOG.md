...
- ⬜ Ряд `death` у многих героев по-прежнему «рвётся» (тело падает — метрика это и ловит); отдельные клипы каста всё ещё сомнительны (Meepo `earthbind` задан вручную). Смотреть по ранжированию, а не по одному листу.
