...
- ✅ **Самоцветы красят не то (владелец: «зелёные пятна на всех самоцветах должны стать цвета самоцвета»).** Рендер пишет тон свечения в мету листа (`glow`, градусы, из `--glow-color`/`--glow-from-alpha`); `gemSheet` берёт его вместо скана доминирующего тона и с мягкими порогами (s ≥ 0.16, v ≥ 0.38, ±55°) — блёклые пиксели ядра тоже перекрашиваются; при перекраске насыщенность и яркость подтягиваются (s ≥ 0.75, v ≥ 0.85), самоцвет читается. Метаморфоза TB не тронута (`demon_detailmask` пуста, ⬜).
