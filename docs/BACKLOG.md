...
- **TREF12 — Иконки команд в турнире (P2) ⬜.** У 322-0 каждой команде присвоены эмодзи (список `Td`, 50 шт.) и цвет (`Od`, 12 шт.); в группах, RESULTS и финальной таблице команда узнаётся по значку. У нас 18 однотипных строк, и при live-симуляции глазу не за что зацепиться, чтобы следить за конкретной командой. Не косметика: имена ботов генерируются из общих префиксов/существительных (`Ranked Techies` / `Ranked Goblins` / `Ranked Spirits`) и намеренно похожи.
  - **Файлы:** `web/src/game/tournament.ts` (иконка присваивается детерминированно от seed рядом с `botNames`), `web/src/features/tournament/TournamentScreen.tsx`, `web/src/ui/` (примитив значка, если понадобится ≥2 экранам).
  - **Скиллы:** `frontend-architecture` (цвет — токен, не литерал), `game-state-architecture` (иконка — часть детерминированного поля, не UI-случайность).
  - **DoD:** иконка стабильна для команды в пределах забега (тот же seed ⇒ та же иконка); видна в группах, MATCH RESULTS и финальной таблице; цвет — токен; `tsc` + `npm run test` чисты; golden не поехал или обновлён осознанно.
  - **Прим.:** эмодзи у нас уже используются в `botNames`-стиле? Нет — проверить перед началом (`discovery-before-code`). Копировать их список не обязательно: нужен свой, консистентный с design-language (Dota-тема, не случайный зоопарк).
