...
*(R14.1–R14.5 закрыты; R14.6–R14.7 открыты)*
...
- ⬜ **R14.6 — build rail: билд как герой интерфейса.** Главное расхождение с Balatro. Там ряд
  джокеров виден всегда и СТРЕЛЯЕТ во время скоринга; у нас Build спрятан во вкладку, а во время
  турнира весь билд сжат в одну строку `Roster 80.3 · Mult +5% · Total 84.4`. Условие срабатывания
  тактики уже вычисляется (`evaluateTactics`) — данных для подсветки «карточка сработала» хватает,
  не используется только показ. Предложение: постоянный ряд мини-карточек билда на всех экранах
  забега + вспышка карточки в момент срабатывания во время reveal.
- ⬜ **R14.7 — сузить одновременное предложение рынка.** Market — 15 карточек в трёх рядах
  (`scrollHeight` 2210px при 1440×720 = три экрана). Магазин Balatro — 2 слота + 2 бустера + 1
  ваучер в один ряд. Предложение: 3+3+3 вместо 5+5+5, а «больше вариантов» продавать за реролл —
  это усиливает и экономику. **Меняет игру, а не показ:** нужен `npm run sim` и бамп
