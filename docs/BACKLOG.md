...
- 🐛 **R13.1 — Item-награда не знает про занятые слоты.** [`CampScreen.tsx:780`](../web/src/features/run/CampScreen.tsx:780):
...
- 🐛 **R13.2 — пентагон подписывает Tournament Power как Team OVR.** В Буткемп передаётся
  ...
  Team OVR, — поэтому подпись обязана стать пропом, а не переименованием константы. **Deps:** нет.
...
- **R13.3 — два уровня информации: карточка = решение, инспектор = математика.** Это прямой ответ на
