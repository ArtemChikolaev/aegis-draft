...
- 🟨 **R12.6 — потолок билда наступает к этапу ~22 (вход в Династию). Спека согласована
  ...
  **trade-in карт с переносом тира −1** как главный сайдгрейд. Порядок работ из спеки:
  ...
  - 🟨 **LG3 — мутаторы круга Династии (= Stakes): реализовано 2026-08-10 (b1.31.0), числа —
    ...
    - ⚠️ **Калибровка v1-чисел — за A/B и плейтестом.** Смоук 60 сидов: глубина Династии
      synergy-build всё ещё упирается в потолок сима (`DYNASTY_DEPTH_CAP` 25, ~6 кругов) —
      внутри него v1-мутаторы informed-агента не убивают («смерть неизбежна» держится на росте
      угрозы). Честный A/B 400×400 против b1.30 обязан показать сезонные строки БАЙТ-В-БАЙТ
      (мутаторов в сезоне нет по построению) и давление в Династии.
