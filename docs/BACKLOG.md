...
- **Файлы:** `server/internal/{transport,service,store,model}` (история + валидация), `schema/` (контракт записи), `web/src/features/tournament/*` (панель) + сетевой слой к API, `i18n/core.ts`.
- **DoD:** запись добавляется один раз на завершённый забег и видна с другого устройства (сервер); счётчики/last-N выводятся из истории по правилам C-A/C-B; дедуп при resume; сервер отвергает подделанную статистику (ре-симуляция); детерминизм забегов не затронут.
- **Deps:** TREF-TOUR1 (терминальный экран ✅); **сервер T8.0/T8.2/T8.3/T8.4** (транспорт, Postgres, аккаунты для cross-device, cloud-saves) — career-история встаёт поверх них.
