...
### T13.80 — Косметика как в Dota: сборка облика по слотам и скины призывов/форм (владелец 2026-09-12) 🟨 срез 1 (2026-09-13) P2
...
- **Модель данных.** Косметика героя становится набором слотов **по данным Dota** (`items_game.txt`: у каждого предмета `item_slot` — `weapon`, `head`, `shoulder`, `arms`, `back`, `belt`, `legs`, `misc`, `taunt`…): `cosmetics.loadout[hero] = { skinBase: "arcana"|"default", parts: Record<DotaSlot, cosmeticId | null> }`. Аркана — это базовый лист + части тех слотов, которые аркана занимает (у Jugg — `weapon+head+back`), остальные слоты свободны под сеты. Совместимость: текущий `skins[hero] = "<set>"` мигрируется в «все слоты этого сета».
