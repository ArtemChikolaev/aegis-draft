...
- ⬜ **P0 — live-симуляция как процесс**: group stage «LIVE» + Skip (пошаговое наполнение таблицы по сыгранным картам), пораундовое раскрытие сетки и итоговой таблицы (снизу вверх). Требуется, чтобы `TournamentEngine` отдавал последовательность матч-событий (карты/серии), а UI проигрывал их по таймеру со Skip; детерминизм по seed сохранить. **Файлы:** `game/tournament.ts`, `features/tournament/*`.
