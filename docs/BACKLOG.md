...
> **Подход (см. ниже) — согласовать открытые решения C-A…C-E до кода.**
...
- **Career-счётчики = ПРОИЗВОДНЫЕ от записей** (не храним агрегат отдельно → нет рассинхрона): 1st/2nd/3rd/4th/Top 8/Last из бакетов placement; Undefeated/Flawless Group/Games Won/Games Lost — из `results`; Runs = длина истории.
- **UI (реюз `StatTile`/`RoleTag`/`HeroThumb`/`Surface`):** career-панель на терминальном экране (как в 322-0: счётчики + last N runs с ростером/героями); опционально отдельный Career-экран со старта (MVP — панель на завершении).
- **Открытые решения:** **C-A** маппинг placement→счётчик (Top 8 = 5–8? Last = только 18?); **C-B** определения Undefeated/Flawless (по картам vs по сериям); **C-C** место показа (только панель завершения vs отдельный экран); **C-D** клиент-only сейчас vs ждать сервер (T8.4); **C-E** хранить историю через bump datasetVersion/ratingModelVersion или сегментировать.
- **Файлы:** `web/src/state/careerStore.ts` (новый), `features/tournament/*` (панель), `i18n/core.ts`; при серверной части — `server/internal/*` + `schema/` (T8.4).
- **DoD:** запись добавляется один раз на завершённый забег и переживает reload; счётчики/last-N выводятся из истории; дедуп при resume; private-mode localStorage не роняет; детерминизм забегов не затронут.
- **Deps:** TREF-TOUR1 (терминальный экран ✅). Серверная синхронизация — T8.4 (опционально, позже).
