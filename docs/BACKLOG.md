...
- **C. Эмит career-глубины (чинит Yatoro/Hero Synergy).** career (`/players/{id}/heroes`, пожизненные игры) уже стягивается в `aggregate.CareerPlayerHeroStats`, но **не доходит до фронта**: нет поля в `model.Dataset`, нет `careerPlayerHeroStats.json`, `domain.Build` его не прокидывает. Провести через контракт: `schema/` → Go `model.Dataset` → `emit` → TS `types` → потребление в назначении героев (`web/src/game/score.ts`: career как база player×hero, event/окно как override). Через скилл `data-contract`.
