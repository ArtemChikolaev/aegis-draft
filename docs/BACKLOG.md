...
### R15 — Аудит 2026-08-09: game feel / juice против Balatro 🟨 (R15.1–R15.3, R15.7–R15.8 ✅; осталось R15.4–R15.6, звук)
...
- ✅ **R15.2 — секвенция «этап пройден» (2026-08-09).** `CampCelebration` поверх лагеря: заголовок
  ...
  - ⬜ Переход этап→Буткемп как смена экранов (`app/App.tsx`) остаётся из R14: секвенция закрывает
    момент «пройден», но сам свап экранов по-прежнему `fade-soft`.
...
- ✅ **R15.8 — три красных e2e: пере-подбор сидов (долг R14.8/R14.9, закрыто 2026-08-09).**
  ...
  акта. По записанной конвенции («перебор, а не ослаблять проверку») подобраны новые сиды:
  `CAMP_SEED = camp-e2e-1`, `CHEAT_SEED = cheat-e2e-1`. Перебор больше не ручной: два модельных
  свипа повторяют ровно пути тестов через движковые функции (`scripts/find_camp_seed_deep.ts` —
  драфт «первым доступным» + 3 этапа с карточной наградой; `scripts/find_cheat_seed_deep.ts` —
  модель `boostInCamp` на весь акт; слои силы — те же функции, что игра, урок R10). Финальная
  правда — живой прогон: полный `anteRun` chromium 16 passed / 1 skipped на новых сидах.
