...
- **T9.0 — Публичный деплой `server/` ⬜.** Один контейнер по [ADR 0002](adr/0002-backend-now.md) (Fly.io / Render, без k8s) + домен с HTTPS. **DoD:** `/healthz` отвечает 200 снаружи; секреты из env; деплой воспроизводим командой/CI. **Deps:** T8.1 (готов). Блокирует всё остальное в M9.
...
- **T9.5 — TMA: тема, safe-area, поведение 🟨 (2026-07-20: сделано всё, кроме проверки на живом клиенте).**
...
- **T9.6 — Хранилище в TMA ⬜ (подтверждено на живом клиенте 2026-07-20, охват шире «сейвов»).** Наблюдение: открыть мини-приложение → переключить тему на тёмную → выйти → зайти снова ⇒ тема снова светлая, т.е. `localStorage` не пережил перезапуск webview. **Под ударом весь наш стейт, а не только забег:** `aegis-draft.theme`, `aegis-draft.locale`, `aegis:run:v1`, `aegis:team:v1`, `aegis:career:v1`. Оговорка к наблюдению: между запусками правился конфиг бота в BotFather, что само по себе могло пересоздать webview — чистый повтор (без правок конфига между шагами) нужен, чтобы считать это законом клиента.
  - **Цена перехода на CloudStorage, которую надо заложить:** он **асинхронный**, а тема и язык читаются СИНХРОННО на первом кадре (бутстрап-скрипт в `index.html` и `initialMode`/`detectLocale`). Значит либо мигание темы на старте, либо первый кадр рисуем по `WebApp.colorScheme` (для этого уже есть `watchTelegramColorScheme`), а сохранённый выбор применяем, когда CloudStorage ответит. Решать это до кода, а не по ходу.
  - В webview `localStorage` живёт по своим правилам и может очищаться, а у нас сейв local-first. Два пути: облачные сейвы T8.4 либо `WebApp.CloudStorage` (лимиты порядка 1024 ключей × 4KB — наш `config+seed+лог действий` укладывается) без всякого сервера. **Решить, что берём.** Что уже выяснено (2026-07-20): CloudStorage **не требует бэкенда вообще** — хранит Telegram, ключ на пару «пользователь × бот»; но его сейв живёт **только внутри Telegram**, на сайте забег из TMA не подхватится — общий для обоих каналов сейв это уже T8.4 и сервер. Второй разрез — аккаунты (ADR 0002, обновление 2026-07-20): сохранность статистики завязана на вход, а в TMA личность даёт `initData` (T9.8). **Deps:** T9.4; альтернативно T8.4.
- **T9.7 — Оформление бота в BotFather 🟨 (2026-07-20: ассеты и тексты готовы, заполнение — вручную в Telegram).** Аватар `web/public/bot-avatar.png` и **обложка `bot-cover.png` 640×360** рендерятся из `favicon.svg` командой `npm run gen:bot-assets` — DoD «один источник правды» закрыт. Осталось: залить их в BotFather, вписать описания и включить Menu Button / Main App / Direct Link на URL GitHub Pages. **Список команд НЕ заводим, пока нет T9.1:** меню команд у бота, который физически не может ответить, — обещание, которое не выполняется. **Deps (только для команд):** T9.1.
- **T9.8 — Валидация `initData` ⬜.** HMAC-SHA256, ключ = `HMAC(bot_token, "WebAppData")`, проверка **только на сервере**. Нужна, как только TMA начнёт что-то персонализировать (сейвы, лидерборд, дейлик): без неё клиент может назваться кем угодно. Хранить из профиля минимум — это персональные данные. **Deps:** T9.0, T9.1.

- **T9.10 — Fullscreen-режим TMA ⬜ (сейчас сознательно НЕ включён, Launch Mode = Fullsize).** Что понадобится, если решим включать:
