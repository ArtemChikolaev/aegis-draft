...
- **Рошан:** удар по телеграфу — голая константа (ранговый `dmgMult` до ×2.95 и второй Рошан на него не действуют); таймер ярости в River идёт, пока Рошан ждёт в яме (пришедший после 9:00 сразу получает ×3); у Рошана нет капа контроля — Time Lock под Frenzy и Call дают почти перманентный стан главного босса.
- **Мёртвые ранги у 35 умений** (20× `armor_buff` всегда +25 брони, 6× `dash` и 9× `mass_freeze` с нулевым `value`, включая Chronosphere и Global Silence): карточки прокачки предлагаются, ранги 2–4 ничего не меняют. Io стартует с Tether, которому не к кому привязаться.
