...
- ⬜ **Phantom Assassin, аркана «голая».** Сборка верная (`pa_arcana.vmdl_c` + `pa_arcana_weapons.vmdl_c`, других частей арканы в vpk нет), значит теряется материал: капюшон/плащ и дым арканы — альфа-текстуры и шейдер (в Workbench альфа-меши выпадают или чернеют, как хвост Jugg). Проверить меши тела в glb и текстуры `pa_arcana_*` в экспорте, включить альфу/`--glow-from-alpha`.
- ⬜ **Terrorblade: нет свечения груди и клинков.** Это selfillum-маски (`*_selfillummask_*`/альфа `detailmask`) — Workbench их не рисует; `--glow-mask-dir` включён только у арканы TB. Включить для базового TB и вообще для всех героев, у кого есть selfillum-маска (сканировать экспорт на `_selfillummask_`), цвет брать из текстуры, не константой.
