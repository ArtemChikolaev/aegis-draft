...
- ✅ **Phantom Assassin, аркана «голая» (2026-09-07).** Сборка верная (тело + оружие), а «голая» она потому, что в модели **нет** капюшона и плаща: кости `cape_*` есть, вершин на них ноль — в Dota плащ-дым и шипастый капюшон это частицы (`pa_arcana_elder_ambient_skined.vpcf` и др.), спрайт их взять не может. Сделано: (1) узор платья — прозрачные области color-текстуры (16%) красятся бирюзовым свечением `--glow-from-alpha 0.2,0.85,0.85` (был почти белый — читался как кожа), `--light 1.5`; (2) **эффект скина** `fx.aura: "veil"` в `content/cosmetics.ts`: новый вид свечения в effects.ts — тёмно-бирюзовый дым от ног за спину и шипастая корона над макушкой силуэта; рисуется всегда у этого скина (в бою и в превью гардероба), поверх — надетое свечение игрока. Тот же механизм пригодится Io (орбиты) и SF (дым).
