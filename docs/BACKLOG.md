...
- ⬜ **Дальше волнами по 8:** Morphling, Naga Siren, Nature's Prophet, Nyx Assassin, Oracle, Outworld Destroyer, Pangolier, Phoenix → Puck, Pudge, Ringmaster?, Rubick, Sand King, Shadow Demon, Slark, Snapfire → … Порядок — по популярности в датасете (`heroes.json` / picks). После каждой волны — `npm run sim:arcade` по новым героям, полоса 30–50% в разминке.
