...
- ⬜ **Juggernaut, аркана.** Строка манифеста пришивала к телу арканы **базовые** части (`jugg_mask`, `jugg_sword`, `jugg_cape`, `juggernaut_pants`) — отсюда бурый мех и базовый меч поверх арканы. Само тело `juggernaut_arcana.vmdl_c` — один меш «torso» с бирюзовой текстурой (второй стиль), маска арканы — `models/items/juggernaut/arcana/juggernaut_arcana_mask.vmdl_c` (в ней же хвост-огонь `tail_core` и цепи). Пробный рендер «тело + маска» даёт бирюзовое тело с чёрным хвостом/мечом: материалы хвоста и клинка — шейдерные (scroll/selfillum), текстур-цветов у них нет, нужен `--mat-map`/заливка через `--glow-from-alpha`, как у PA/TB. У маски есть пара текстур `mask_color` / `v2_mask_color` — два стиля арканы; какой из них оранжевый и где лежит второй цвет тела — проверить в экспорте (`--style v2` сейчас на `~style1`). Дракон арканы (`jugg_dragon.vmdl_c`) — отдельный проп-эффект, в спрайт не входит.
