...
- **Вывод:** контракт без клятвы — чистый плюс (+4/+8 п.п.), клятва и долг силы — настоящие компромиссы, которые бот проигрывает (он не строит игру под них). Числа не трогаем: кандидаты на смягчение после плейтеста — `oath.trashMult` 0.85 → 0.9 и `debt.share` 0.5 → 0.35, если и у человека окажутся «ловушками».
