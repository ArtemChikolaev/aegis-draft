...
### T13.15 — Фирменные пассивки героев (владелец 2026-09-06: «больше особенностей под конкретного героя, пример — души Shadow Fiend») ✅ срез 1 (2026-09-06, a0.11.0) · ✅ волна 14: пассивка у всех 119 (2026-09-06, a0.19.0) · ⬜ HUD-иконка
...
- **Предметы из сетов Dota** (вопрос владельца «можно ли конкретные предметы из сетов»). Да: в `pak01_dir.vpk` 14 033 модели `models/items/<hero>/…`, из них ~1300 полных сетов (части с общим префиксом и суффиксами `_head/_arms/_legs/_back/_weapon`) — ровно та же схема, что уже работает у аркан (части пришиваются к скелету базового героя через Copy Transforms). Заведены четыре сета как облики редкости exotic: Bladesrunner (Juggernaut), Assassination of Dark Feather (PA), Blackthorn (Axe), Nightmare Scarecrow (Pudge).
...
- ⬜ **Листы в WebP вместо PNG.** После волны сетов и стилей `public/art/sprites` весит около 160 МБ, и каждый полный перерендер добавляет в историю столько же — репозиторий уже 500 МБ. Индексированный пиксель-арт в WebP (lossless) обычно вдвое меньше PNG при той же картинке, браузеры его поддерживают все. Менять придётся конвертацию в конце `dota_pipeline.sh` и расширение в загрузчике `sprites.ts`; старые PNG можно оставить как фолбэк на одну версию.
...
- ⬜ Дальше: дым/искры при смерти горящего, вспышка Aegis (Dire), снаряды врагов (осада) — трассер.
...
### T13.18 — Легендарные апгрейды: тиры «как в Death Must Die» (владелец 2026-09-06: «разбить улучшения на тиры, чтобы выпадали мега-сильные пассивки») ✅ срез 1 (2026-09-06, a0.13.0) · ✅ партия 2: 12→18 легендарок (2026-09-06, a0.19.0) · ⬜ редкость влияет на потолок ранга
...
### T13.14 — Инвентарь предметов как в Death Must Die (фидбэк владельца 2026-09-06) ✅ срез 1 (2026-09-06, a0.10.0) · ✅ партия 2: 22→34 базы, 3→6 уникальных (2026-09-06, a0.19.0) · ⬜ баланс аффиксов
