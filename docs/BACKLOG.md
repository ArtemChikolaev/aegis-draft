...
- 🟨 **R12.6 — потолок билда наступает к этапу ~22 (вход в Династию). Спека согласована
  ...
  **trade-in карт с переносом тира −1** как главный сайдгрейд. Порядок работ из спеки:
  - ⬜ **LG5 — инструментарий раньше чисел (первым!):** Dynasty-прогоны сима, метрики по актам
    («лагерей с осмысленным действием»), edition/trade-метрики, trade-in у агентов. Без этого
    калибровка полуслепая (доказано A/B-заходом Charged).
  - ⬜ **LG1 — trade-in карточки:** «Обменять» на экипированной карте → 3 оффера из невзятого
    пула (поток `:trade`), новый тир = текущий − 1, Edition не переносится, цена — синк золота.
