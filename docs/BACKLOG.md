...
Живой прогон + [дизайн-бриф](archive/roguelite-balatro-brief.md). Два бага от пользователя **подтверждены по коду**, остальное — продуктовое направление, разложенное в веху **M5R** выше.
...
> Источник: [docs/archive/roguelite-balatro-brief.md](archive/roguelite-balatro-brief.md) (бриф + **ревизия при приёмке**: каждое утверждение проверено по коду, семь пунктов исправлено). Продуктовые решения — [PRD §5.9.3](PRD.md). Веха идёт **параллельно** M6 и переопределяет часть T5.8/T6.3.
>
...
- **T6.1 Tactics system:** 5 общих ограниченных слотов пассивных Tactics/Items; reward предлагает 3 варианта, новый можно экипировать, заменить/отклонить по правилам экономики. Эффекты data-driven, условные и с trade-off; безусловные глобальные `+N%` не добавляем. Первый набор-кандидат: `Signature Specialists`, `Old Teammates`, `Fresh Project`, `Wide Pool`, `No Superstars`, `Last Dance` (описания — PRD §5.10.3). UI показывает `до → после`, слагаемое Team OVR и причину срабатывания. Тесты фиксируют порядок применения, caps, stacking и детерминизм offers. ✅ (базовый набор из 5 — срез 4; **Wide Pool доехал 2026-08-30, b1.42.0** — стартовый набор PRD §5.10.3 полон)
  ...
  - 🔁 **2026-07-27:** дизайн экономических пассивок оформлен — [бриф §13/§16](archive/roguelite-balatro-brief.md), реализация в `R8.3` поверх контракта Tournament Power (`R8.2`). До фикса `R9.1` калибровать Last Dance бессмысленно: её trade-off сегодня убирает не «−2 случайные карты», а обоих саппортов.
...
### T13.83 — CI: плавающий красный Vitest при зелёных проверках ✅ (2026-09-13)
- **Причина:** тест «каждый герой детерминирован» (126 героев × 3 сима) на раннере крутится 40+ с одним синхронным циклом; воркер vitest не отвечает главному процессу, RPC истекает — `[vitest-worker]: Timeout calling "onTaskUpdate"` считается unhandled error, прогон красный при 963 зелёных проверках. Поэтому падало через раз (зависело от скорости раннера), а лимиты тестов и `testTimeout` не помогали.
- **Сделано:** тесты всех героев и полного акта стали async и отдают event loop (`setImmediate`) между героями и каждые 30 игровых секунд; vitest в CI в forks-пуле, пишет лог в файл, при падении хвост лога уходит аннотацией (`::error`) — читается через публичный API check-runs, когда лог джоба без токена недоступен; репортер `github-actions` показывает проваленные проверки.
- **Урок:** длинные синхронные циклы в тестах сима — только с yield; диагностика CI без токена — через аннотации.

- ⬜ **Замечание 2026-09-13 (вечер):** на 61add618 шаг «Browser smoke (Playwright)» Web-джобы шёл 20 минут (10:07 → 10:27 UTC), сама джоба — 22 минуты; юнит-тесты 69 с. Сравнить с прогонами до пакета спрайтов T13.80 (284 файла в public) — если smoke ждёт загрузки листов или таймаутов, вынести ожидание арта из smoke или ограничить preload в CI. Публичный API без токена даёт 60 запросов в час — опрос CI раз в 30 с его выедает, опрашивать реже.

### T13.82 — Спороносец: ядовитые лужи на ходу и после смерти ✅ прототип (2026-09-12) P2 (аудит 2026-09-12 §4, `a0.69.0`)
- **Что:** с 6-й минуты в любом акте раз в 75 с приходит пара спороносцев (новый вид `sporebearer`, 220 HP, медленный 46 px/с, модель Venomancer). Идёт как обычная толпа, но каждые 5 с оставляет лужу спор (r 60, 8 с), а после смерти — большую (r 110, 10 с) там, где убит. Герой в луже: 14 урона в секунду и −35% скорости. Решение игрока: убить издалека (лужа далеко) или выманить с нужного прохода перед добиванием. Луж не больше шести, старые гаснут первыми — обязательный путь не перекрыть навсегда (`ArcadeSim.spores`, `inSpore()`).
- **Проверки:** `arcadeSpores.test.ts` — пара по расписанию, лужи на ходу и большая после смерти, лимит с вытеснением старых, урон и замедление в луже против чистого пути, угасание по времени. Playwright: лужи на экране.
- **Замер ботом (n=48, seed modcal, short, Herald, все места; шаманы и лучники уже в игре):** Juggernaut 18.8% побед (база 18.8%), Sniper 37.5% (43.8% с лучниками, 41.7% база) — в пределах шума; спороносец ни разу не добил героя, хотя бот лужи не видит (поле опасности их не учитывает). Числа не трогаем до плейтеста. На этом §4 отчёта закрыт: шаман, строй стрелков, спороносец.
- ⬜ Если у человека лужи окажутся главной причиной смерти — сначала подсказка на экране (лужа читается?), потом `dps`/`slow`; боту добавить лужи в поле опасности, чтобы замер стал честнее.

### T13.81 — Строй стрелков с объявленным залпом ✅ прототип (2026-09-12) P2 (аудит 2026-09-12 §4, `a0.68.0`)
- **Что:** с 5-й минуты в любом акте раз в 70 с из-за края приходит линия из четырёх лучников (новый вид `archer`, 90 HP, модель Clinkz) поперёк направления на героя. Строй держит ~380 px: подходит издалека, отступает, если герой ближе 300; вплотную лучники хрупкие. Каждые 6 с линия **объявляет полосу** (90 × 480 px от центра строя к позиции героя) на 1,4 с — полоса наливается на земле — и даёт залп: урон 26 × доля живых лучников получает только тот, кто остался в полосе. Решение игрока: выйти вбок, зайти сбоку, разбить строй. Живых строев не больше двух (`ArcadeSim.archerLines`, направление залпа фиксируется в момент объявления).
- **Проверки:** `arcadeArchers.test.ts` — линия по расписанию поперёк направления, телеграф в дальности, залп бьёт в полосе и не бьёт после шага вбок, дистанция, исчезновение линии после гибели всех, лимит строев. Playwright: полоса залпа на экране.
- **Замер ботом (n=48, seed modcal, short, Herald, все места; шаманы уже в игре):** Juggernaut 16.7% побед (с шаманами без лучников 25.0%, база 18.8%), Sniper 43.8% (43.8%) — в пределах шума, лучник ни разу не добил героя: бот постоянно двигается и из полосы выходит сам. Для человека полоса — читаемое решение, бот её не «оценивает»; числа не трогаем до плейтеста.
- ⬜ «Использовать камень»: полоса пока не учитывает препятствия (камень не укрывает) — следующий срез, если плейтест покажет, что укрытие нужно; ночью дистанция объявления та же (полоса видна всегда). Замер ботом — вместе с §4. Дальше по §4: спороносец.

### T13.80 — Косметика как в Dota: сборка облика по слотам и скины призывов/форм (владелец 2026-09-12) 🟨 срез 3 (2026-09-19) P2
**Проверка 2026-09-15:** [аудит и план следующего среза](audits/2026-09-15-customization.md). Первый срез подтверждён кодом, 30 профильными тестами и гардеробом Jugg/MK, но полная цель «аркана + предметы других сетов» остаётся открытой. Проверено на `5d441c06`, Dota build `25132749`; игровые исправления в ходе аудита не выполнялись.

**Срез 2 (2026-09-15, Claude, по аудиту):** семейства основ + слоты из данных Dota.
- ✅ **A2 — слоты по item ID.** `scripts/dota_item_index.mts` парсит `items_game.txt` из vpk (KeyValues с наследованием prefab) → `dota_item_index.json` (путь модели → item ID, `item_slot`, редкость, подмены моделей `asset_modifier model`; основы `hero_base` по героям); `dota_slot_overrides.json` — две модели без предмета (волосы Axe → head, плащ MK → back) с причиной; `dota_part_layers.mts` берёт слот только из индекса, неизвестное — стоп. Новый слот `legs` (штаны Jugg, Belt of the Bladesrunner), голова арканы MK → `head`, наручи Lina → `arms`, воротник → `neck`, кинжалы PA → `belt` (отдельный слой), хвост Axe → часть слоя `head`. Слои переименованы без перерендера, `parts.ts` пересобирается режимом `table` из манифестов слоёв; сейв мигрирует (`migrateCosmeticsV2`, `v: 2`). `qa_set_slots.mjs` тоже читает индекс — и показал **30 строк сетов без базовой части в непокрытом слоте** (раньше эвристика видела 1). ✅ 2026-09-19: это был настоящий дефект — почти все случаи `body_head` (лицо героя — отдельный слот Dota, шлем сета его НЕ заменяет, а догадка по имени считала `*_head` покрытым): `lycan@ambry` стоял без головы. `--apply` дописал базовые части в 30 строк обоих манифестов (Clinkz, Necrophos, Luna, Shadow Shaman, Tinker, Omniknight, Lycan, Earth Spirit, KotL-конь, OD, Spirit Breaker, Timbersaw-пила, Gyrocopter, Underlord, PL, Invoker, Snapfire, Ringmaster-кисти), 60 листов перерендерены, глазами проверены по контактным листам; `qa_set_slots.mjs` теперь даёт 0.
- ✅ **A1 — настоящая аркана Lina.** `lina@arcana` = `origins_flamehair` (4794) + текстура тела `--style flamehair` (model_skin 1) + базовые рукава/платье/украшение; прежние листы и слои стали `lina@battle_caster` / `skin_lina_battle_caster` (exotic), кто носил «аркану» — носит Battle Caster дальше, аркана остаётся в собственности. Пламя: модель — только скальп на 277 вершин, огонь у Valve в частицах `lina_headflame.vpcf` — рисуется встроенным эффектом скина `fx.aura = "flamehair"` (effects.ts, грива с макушки силуэта), как дым-плащ у арканы PA.
- ✅ **Семейства основ (цель владельца «аркана + голова/меч из других сетов»).** `HERO_PARTS[hero].families[<основа>]` — базовая модель, аркана и её стили со своими `+body` и слоями; у семейства `defaults` (слоты самой основы: у арканы Jugg спина пуста, как в Dota) и `sources` (`base` — все части модели по умолчанию, `arcana` — собственные части основы, сеты — с подменами моделей из items_game: Cult of the Demon Trickster на аркане MK становится `*_arcana_*`). `familyOf/resolveLoadout/loadoutSheet` — по семейству; композит `<основа>+body+…`. Надевание **сета** сбрасывает части (как «надеть сет» в Dota), надевание **основы** (аркана/базовая) сохраняет их — как предметы при смене арканы. Гардероб: состав облика («Основа: … · Голова: …»), бейдж «Смешанный облик», «Сбросить части», пустой слот показан как «Пусто».
- ✅ **A3 — стиль основы.** Стиль-лист арканы (`~style1`) — своё семейство; если оно не отрендерено, цельный облик остаётся со стилем, а смешанный падает на основу без стиля и гардероб пишет об этом (`heroLook.styleDropped`). Самоцветы (hue) стиль не теряют — красят и композит.
- ✅ **A4 — память композитов.** `ByteLru` в sprites.ts: бюджет 256 МиБ RGBA полных композитов, вытеснение давно не читанных (текущий облик боя/витрины трогается каждый кадр); миниатюры гардероба собирают один кадр (`dotaSheetStill`), не лист; слой другого размера растягивается nearest до тела вместо рассинхрона. Замер RAM браузером не делался — проверка расчётная + тест LRU.
- ✅ **Рендеры семейств (Blender, 2026-09-15).** 12 семейств: `juggernaut`, `juggernaut@arcana`, `~style1`; `phantom_assassin` (перерендер: кинжалы → `belt`); `lina`, `lina@arcana`; `monkey_king`, `monkey_king@arcana`, `~style1..3` (сет Cult на аркане — арканные `*_arcana_*` модели); `axe` (перерендер: хвост в слое `head`). Расхождение композита «тело + слои по умолчанию» с образцом: Jugg-аркана 1.9/3.2%, PA 4.9/7.1%, Axe 1.7/2.2%, Lina-аркана 1.4/2.7%, MK-аркана 8.7–9.1/12.0–12.5% (px2/px) — у MK плащ и посох пересекаются с телом, как и в первом срезе; порог по-прежнему принимается глазами. `REUSE=1` в `dota_pipeline.sh` теперь кэширует и текстуры стиля/fix-tex/маски (у MK они стоили почти минуту на строку). Живьём проверены: Jugg аркана + голова Bladesrunner (оба стиля), Lina аркана + косы Dragonfire (пламя гаснет вместе с заменённой головой: `fx.slot`), MK аркана + оружие Fiery Vajrapani со стилем.
**Срез 3 (2026-09-19, Claude): формы и волки Lycan** — то, что владелец назвал прямо («волки Lycan, Метаморфоза Terrorblade»).
- ✅ **Скины форм — свой слот `form`.** По `items_game` форма — отдельный предмет в своём слоте (TB `ability3`, Lone Druid `ability_ultimate`, DK `shapeshift`) с подменой модели `asset_modifier hero_model_change`, поэтому выбирается независимо от облика: `CosmeticSlot "form"`, `cosmetics.formSkins[hero]`, `setFormSkin`, `heroLook.form`; рендерер: скин формы → форма надетого облика (`<hero>@<skin>@meta`) → базовая. Лист `<hero>@meta~<имя>` — скелет и аргументы базовой формы + `--hide-base` + модель скина (у TB маска свечения из материалов скина); нет листа — откат на `<hero>@meta` (`~` отрезается). Гардероб: раздел «Форма» у героев с формой, выбор сразу включает превью формы. Контент: 6 демонов TB, 5 форм Lone Druid, 10 драконов DK (без персоны, DPC-перекрасок и коллаборации Monster Hunter).
- ✅ **Волки Lycan — 9 скинов** (`summon_wolf_*`): у моделей скинов свой скелет, на чужом меш растягивался в полосу (срез 1) — теперь лист `wolf@<skin>` рендерит саму модель скина основной, клипы у неё свои. Кадр 120/60 при `--world 120 --margin 1.4` (размер в бою тот же, что у базового волка 96/1.12): в кадре 96 морды и хвосты в беге резались рамкой (`qa_sheet_health`: срез 55–88% у Watchdog/Hunter Kings); Watchdog (19721) и в кадре 120 режет хвост в 35% кадров бега — не включён.
- ✅ **Идентичность по данным.** `dota_item_index.json` получил `entityModels` — подмены `entity_model`/`hero_model_change` (путь модели скина → item ID, слот, герой, что подменяет); тест `arcadeForms` требует, чтобы модель каждой строки формы подменяла именно базовую форму своего героя, а волки — юнит `npc_dota_lycan_wolf`. Попутно видно: аркана TB (5957) — слот `head` с подменой модели героя, как у Lina, а не `hero_base`.
- ✅ **Плейтест владельца 2026-09-19 (в тот же день):**
  - «С красной арканой форма обычная»: в слоте формы был сохранён явный выбор Marauder's, а гардероб его никак не показывал. Теперь: у плиток формы/призыва/части метка «надето», под витриной строка «Форма: … (как у облика)», отдельные плитки **«Как у облика»** и **«Обычная»** (`LOOK_DEFAULT`), надевание любого облика сбрасывает выбор формы и призывов на «как у облика». «Как у облика» следует **бандлу сета** (`CosmeticDef.withSkin`): у сета Marauder — его демон, у сета Ambry — волки Ambry, у Dark Wood — медведь и форма Dark Wood. Аркана красит и скин формы: самоцвет, а без него родной тон свечения арканы (`gemSheet`, в бою и в превью одинаково).
  - «Слишком много листать, при пролистывании не видно, как будет выглядеть»: гардероб — **закреплённое превью + вкладки** (Облики / Слоты / Форма / Призывы / Эффекты), `Modal size="wide"` (вариант примитива `ui/`), на узком экране превью закреплено сверху, настройки вида превью свёрнуты под кнопку. Режим «Form» в превью включает вкладка формы и выключает уход с неё — не залипает.
  - «Сеты и для обычной формы TB»: 5 сетов Terrorblade (Marauder, Eternal Purgatory, Foulfell Corruptor, Chasm of the Broken Code, Forgotten Station) + семейства слоёв `terrorblade` и `terrorblade@arcana` (аркана TB — слот `head` с подменой модели героя).
  - «У Anti-Mage нельзя собирать по слотам»: слои есть только у героев отрендеренных волн; гардероб теперь пишет это прямо (вкладка «Слоты»: «ещё не подготовлена»). Для волн — возобновляемый `scripts/dota_part_wave.sh` (plan → рендер → build по героям, падение плана у героя волну не останавливает). Сухой план по 100 героям: 35 собирались сразу; ещё ~44 — после добавления слотов Dota `offhand_weapon`/`body_head`/`tail`/`gloves`/`costume` в `DRAW_ORDER`; 21 герой ждёт оверрайдов слота для моделей без предмета (волосы/шляпы/кони базовых моделей, fx-модели сетов) — каждую проверять по данным, не по имени.
  - «На аркане MK не нажимается посох Cult»: на текущем коде не воспроизвелось (аркана + оба сета, узкий экран: часть записывается, витрина показывает «Смешанный облик»); вероятная причина — устаревший HMR-модуль после регенерации `parts.ts`. Если повторится после перезагрузки страницы — разбирать отдельно.
- 🟨 **Волна владельца 2026-09-19 (вечер): Anti-Mage, Shadow Fiend, Drow Ranger** — общий рендер всех героев отложен по решению владельца.
  - **Anti-Mage:** слои базового тела + два набора оружия из бандлов items_game — «Basher Blades» (20796: Basher of Mage Skulls + Offhand) и «Arcs of Manta» (20251); у них источник даёт только `weapon` + `offhand_weapon`, остальное — по умолчанию.
  - **Shadow Fiend:** базовая строка переведена со старой монолитной `nevermore.vmdl` (без частей — сеты на неё не надеть) на актуальную `shadow_fiend.vmdl` + три дефолтных предмета (arms/head/shoulder); тёмная текстура автоэкспозицией уходила в серый — `--expose-target 0.2` (тёмный, но читаемый). Сеты по выбору владельца: Spring Lineage Eternal Harvest, Souls Tyrant, Eternal Harvest; семейства слоёв `shadow_fiend` и `shadow_fiend@arcana` (крылья арканы — оверрайд слота `back`: не предмет). **Базовый облик SF в игре изменился** (новая модель Dota вместо старой чёрной с красными рогами).
  - **Drow Ranger:** сеты по выбору владельца — Stranger in the Wandering Isles, Black Ice Constellation, Sight of the Kha-Ren Faithful; семейства `drow_ranger` и `drow_ranger@arcana` (части сетов на аркане — «refit»-модели из подмен предметов арканы). Семейство стиля `~style1` арканы отрендерено позже той же ночью (см. ниже); изначально не рендерилось — смешанный облик со стилем помечается `styleDropped`.
  - **Ограничение:** сайт на GitHub Pages (~1 ГБ), `web/public` ≈ 790 МБ до волны; пара слоёв px2+px ≈ 0.47 МБ. «Все сеты Drow на двух телах» (≈380 слоёв) вышли бы за лимит — поэтому у SF и Drow по три сета. Перед следующими волнами считать вес заранее.
- ✅ **«Крылья TB просвечивают сквозь скин» (владелец 2026-09-19).** Порядок слоёв был один на все направления: слой спины (крылья, плащ) рисовался первым, и спиной к камере броня/оружие ложились ПОВЕРХ крыльев. Теперь порядок слоя спины — по направлениям (`backOrder` в `parts.ts`: 0 — первой, 1 — перед головой/оружием, 2 — последней), его меряет `dota_part_layers.mts table` против цельного листа, рантайм рисует спину по рядам направлений (`sprites.ts drawLayers`). Промах композита: TB 3.4 → 1.1%, аркана TB 3.8 → 1.5%, аркана SF 9.1 → 5.4%, Drow 7.6 → 4.0%, Jugg 2.9 → 1.9%; у PA и MK лучший порядок — прежний. Другие слоты (оружие против плаща и т. п.) по-прежнему в общем порядке — смотреть по плейтесту.
- ✅ **Вес сайта (владелец 2026-09-19: «можешь уменьшить вес GitHub Pages?»).** Замеры: перекодирование WebP бесполезно (≈1%); клип `death` игра рисует только в направлении 0 и только у врагов → ряды смерти остальных направлений вырезаны из всех листов без потерь (`scripts/sheet_single_dir.mts`, 1504 листа: 675 → 546 МБ, **−129 МБ**), новые рендеры сразу в этом формате (`--single-dir death`), тест-страж в `arcadeSheetIds`; непиксельный набор `dota/` удалён (**−31 МБ**), `?pixel=0` рисует пиксельные листы без пиксельного прохода. Резервы (решение владельца): зеркалить 3 из 8 направлений (−37%, оружие меняет руку), пережать озвучку (97 МБ → ≈ 55 МБ), вынести спрайты на отдельный хостинг. Находки попутно: побайтные дубли `pudge@arcana~style1` = `pudge@arcana` и (px) `ogre_magi@arcana~style1` = `ogre_magi@arcana` — стиль-текстуры у них не применились, перерендерить.
- ✅ **Drow: лук сета на аркане «висит в воздухе», стиль II синеет при смешивании (владелец 2026-09-19, ночь).** (1) Аркана Dread Retribution — предмет слота `weapon`: тело `drow_arcana` несёт свой хват арбалета двумя руками, refit-луков под него в vpk нет (подмены арканы — только колчан/misc, shoulder, arms), в Dota другой лук с арканой не надеть. Новый замер `web/scripts/qa_part_attach.mts` (доля кадров, где слой не касается героя) по всем 19 семействам: лук базы и трёх сетов на аркане — 38–46%, у всех остальных слоёв ≤ 4% (4 кадра броска посоха MK — норма). Исправление — `scripts/blender/dota_part_exclusions.json` («слот закреплён за источниками», с причиной): генератор слоёв не планирует чужой слой, 8 листов удалены, гардероб не показывает слот без выбора (`choosableSlots`; заодно пропали пустые вкладки «Спина» у MK/SF), старый выбор в сейве молча уступает основе. Базовый колчан на аркане теперь тоже refit-модель (`drow_quiver_refit`). (2) Семейства стиля у арканы Drow не было — смешанный облик падал на синюю основу (`styleDropped`); отрендерено `drow_ranger@arcana~style1` (33 слоя). Аудит остальных: стиль теряется ещё только у сета MK Cult of the Demon Trickster (см. ниже).
- ⬜ **Стили сета MK «Cult of the Demon Trickster» теряются в смешанном облике** (найдено аудитом 2026-09-19). Сет — источник частей на теле `monkey_king`/арканы, а его стили 1–3 — другие текстуры тех же частей; слои отрендерены только в основном стиле, игрок видит честную подсказку `styleDropped`. Нужен «стилизованный источник» (`<сет>~<стиль>` как источник слоёв): 4 слота × 3 стиля × 5 семейств MK = 60 слоёв (≈ +14 МБ, ≈ час Blender) и развязка двух `--style` в одной строке (у стилей арканы MK `--style` уже занят телом). Заодно: слои сетов на семействах стилей побайтно совпадают со слоями базовой арканы — кандидат на общий файл вместо копий (вес).
- ✅ **SF «Spring Lineage Eternal Harvest» — перекраска.** Лист побайтно совпадал с Eternal Harvest: в Dota это те же модели с `skin 1` (материалы `_dpc`), а пайплайн искал текстуры стиля в папке модели (`shadow_fiend`), тогда как предметы лежат в `items/nevermore`. Добавлены `--style-folder` (пайплайн) и «сет-перекраска» в генераторе слоёв (хвост `--style …` у строки сета переносится на его слои).
- ⬜ **Две формы Lone Druid выброшены по QA:** `True Form of the Arctic Owlbear Clan` (12880) выбегает из кадра в беге (провал силуэта 91%), `Form of the Wolf Hunter` (6870) дрейфует во всех клипах (рывок стойки 16% против 0.3% у базы) — гашение root motion (несущая кость по весу вершин) не ловит их скелет при `--hide-base`. Починить выбор несущей кости для скина формы и вернуть.
- ⬜ Следующие герои слоёв частей волнами, больше скинов призывов (ястреб Beastmaster, фамильяры Visage, вард Juggernaut), формы остальных героев (Lycan Shapeshift у нас не форма — см. kit); порог визуального качества композита по-прежнему принимается глазами (контактный лист клеток рядом), не по проценту.


**Очередность после аудита:** A1/A2 → полный пример Jugg (hero_base арканы + другая голова + оружие, стили и spin) → MK/Lina и A3/A4 → формы/призывы → остальные герои. Порог визуального качества, целостная публикация ассетов и fallback при отсутствующем слое — критерии следующего среза из отчёта. Пресеты, отдельный выбор эффектов умений и новый layout гардероба — предложения, пока не утверждённые требования.

Владелец: «вместе с арканой или скином надевать предметы из разных сетов — меч из одного, шлем из другого; плюс скины маунтов и форм: медведь Lone Druid, волки Lycan, Метаморфоза Terrorblade». Проработка:
- **Что уже есть.** Гардероб (T13.27): один скин/аркана на героя целиком, стили аркан (`~style`), самоцветы как перекраска, косметические слоты `frame/aura/trail/death/tint`. Способ послойного рендера найден и проверен (T13.27 🟨: «разница двух листов» даёт слой части, `sprite_part_layer.mts`). Призывы — сущности сима с телом из `content/pets.ts` (`SUMMONS`: медведь, волки, …) и своими листами (`bear`, `wolf`, `spiderling`…); формы героя — отдельные листы `<hero>@<skin>@meta` (Метаморфоза, Chemical Rage).
- **Модель данных (целевая; срез 1 ниже использует более простой формат).** Косметика героя становится набором слотов **по данным Dota** (`items_game.txt`: `item_slot`, стабильный item ID, наследуемые свойства и модификаторы). Основа героя, носимые детали и стиль разрешаются совместно. Поправка 2026-09-15: у установленной Dota арканы Jugg 9059 и MK 9050 занимают `hero_base`, аркана Lina 4794 — `head`; прежнее утверждение «у Jugg аркана занимает weapon+head+back» неверно. Комплект визуальных частей не равен занятым слотам предмета. Совместимые свободные слоты доступны сетам; текущие `skins` и `loadout` мигрируют с сохранением выбранного вида.
- **Рендер: композит слоёв в рантайме.** Базовый лист (герой без сменных частей) + слои частей в порядке слотов, для каждого кадра/направления. Каждый слой — отдельный лист-разница (T13.27), Playwright-композитор кэширует итоговый лист по ключу `hero|skinBase|parts` (один раз на забег, ~50 мс на лист). Форма (Метаморфоза) — свой базовый лист и свой набор слотов (`terrorblade@meta` + `weapon_meta`…).
- **Призывы и маунты.** У `SUMMONS`/pets появляется `look: { base: "bear", skin?: cosmeticId }`; скины призывов — отдельный слот гардероба `summon:<kind>` (медведь Lone Druid, волки Lycan, паучки Brood, голем Warlock, Familiars Visage, тотем Juggernaut/Healing Ward). В vpk это `models/items/<hero>/<set>/*_bear*.vmdl`, `*_wolf*` — те же manifests, отдельные листы `bear@<set>`. В сим ничего не идёт (сим знает только `art`), детерминизм не трогается.
- **Цена.** Рендер: «голый» лист на героя + лист на каждую часть каждого сета (≈10 частей на героя) ≈ **1400 рендеров, 60+ часов Blender, +130 МБ** сверх нынешних ~280 МБ (оценка T13.27) — поэтому делать **волнами по героям с сетами в гардеробе** (сначала 5–10 героев с несколькими сетами, где смешивание реально что-то даёт), и только части, отличающиеся между сетами. Призывы дешевле: по 1–3 листа на скин.
- **UI гардероба.** Вкладки по слотам (как в Dota: Оружие / Голова / Плечи / Спина / …), в каждой — части всех сетов героя + «как в сете», предпросмотр собирается тем же композитором; отдельная вкладка «Призывы» у героев с `SUMMONS`. Сохранение в `cosmetics.loadout`, экспорт/импорт профиля (T13.69) переносит.
- **Порядок работ (DoD по срезам).** (1) данные слотов из `items_game.txt` в манифест (`dota_register_skins.mts` уже читает vpk) + миграция сейва; (2) композитор в рендере/превью и гардероб по слотам для 5 героев (Juggernaut, PA, Drow, Lina, Anti-Mage) с 2+ сетами; (3) скины призывов: медведь, волки, паучки; (4) формы (Метаморфоза) со своими слотами; (5) остальные герои волнами. Это решение владельца по объёму рендера (см. T13.27).
- ✅ **Срез 1 (2026-09-13): слоты у 5 героев + скины призывов.** Владелец: «давай приступать».
  - **Рендер слоёв** (`scripts/dota_part_layers.mts`, docs/arcade-dota-sprites.md §8): семья героя = образец (`<hero>+ref`, базовая строка), тело без сменных частей (`<hero>+body`) и по рендеру на слот каждого источника (`<hero>+<src>.<slot>`, тело + части слота); слой = разница с телом (`sprite_part_layer.mts`, теперь модуль). Две находки, без которых не работало: кадрирование (центр/высота/охват) меряется пробным рендером силуэта и у тела без плаща другое — добавлен `--fit-from <образец.json>` в `render_dota_sprites.py` (поле `fit` пишется в мету каждого листа); и разница считается по сырым листам (`RAW=1` в `dota_pipeline.sh`), иначе разные палитры pngquant дают шум по всему телу. Слот части — по имени файла, общий модуль `scripts/lib/dota_slots.mjs` (из qa_set_slots). Экспорт glb один раз на героя (`REUSE=1` + симлинки экспортных папок).
  - **Герои волны:** Juggernaut (base + Bladesrunner), Phantom Assassin (base + Dark Feather), Lina (base + Dragonfire + «Slayer»), Monkey King (base + Fiery Vajrapani + Cult of the Demon Trickster; аркана MK — на базовом теле, но только `misc`), Axe (base + Blackthorn + Wrought Legion). Не по плану: Drow и Anti-Mage выпали (у Drow аркана — другая модель тела и сетов на базе нет; у AM нет строки сета), вместо них MK и Axe с двумя сетами. Всего 140 рендеров семьи (px2 + px) ≈ 60 мин Blender на M-серии (25–30 с на лист 160 px, ≈ 15 с на 80 px) плюс 16 листов призывов; слои и тела весят 24 МБ на оба набора (18 МБ px2, 6 МБ px) (слой почти прозрачный, lossless WebP это любит).
  - **Проверка слоёв:** `build` складывает тело + все базовые слои и сравнивает с образцом: расхождение 1.5–9.8% непрозрачных пикселей в px2 (Lina 1.5, Axe 1.8, Jugg 4.3, PA 5.0, MK 9.8) и 2.4–13.5% в px — глазами это перекрытия частей друг другом (плащ MK поверх наплечников: слой считается против голого тела, а порядок «спина → броня → плечи» решает, кто сверху), тени частей и контур; на витрине и в бою композит от образца не отличить. Смотрел глазами витрину и бой (Playwright на 5273).
  - **Модель данных:** `content/parts.ts` (генерируется build): слоты героя в порядке отрисовки и источники → слоты со слоем. `cosmetics.loadout[hero][slot] = "base" | <set>` — только явные переопределения; пустой слот следует надетому облику, у источника нет части в слоте — часть по умолчанию (как Dota). `loadoutSheet()`: всё как у надетого — его цельный лист (стиль/самоцвет работают), всё как у другого сета — его лист, иначе композит `<hero>+body+<src>.<slot>+…`; `sprites.ts compositeSheet` складывает слои один раз и кэширует (мета с именем композита — геометрия кадров и скан свечения не путаются с телом). Надевание облика целиком сбрасывает слоты героя (как «надеть сет»). Арканы/персоны с другой моделью — цельные, слоты к ним не применяются (`wornIsWhole`), в гардеробе подсказка.
  - **Скины призывов:** слот `summon` в `cosmetics.ts` (`variant: "<art>@<skin>"`, `hero`), `cosmetics.summonSkins[hero][art]`, `setSummonSkin`; рендерер берёт лист призыва через `summonSheet(art)`. Листы — полная модель скина на скелете и анимациях базового призыва (`--hide-base`): медведь Lone Druid ×2 (Dark Wood, Iron Claw), паучки Brood ×2 (Amber Queen, Lycosidae), голем Warlock ×2 (Obsidian, Hellsworn). Волки Lycan (Werewolf Samurai, Snow Wolf) отрендерены и **выброшены**: у моделей волков-скинов свой скелет (`Bip01…` без пары в `lycan_wolf`), меш растягивается в вертикальную полосу — нужен ремап костей или рендер скина как самостоятельной модели с анимациями базы. Две шишки `--hide-base` в рендер-скрипте починены (сравнение с удалённым мешем и несущая кость по удалённым мешам — `ReferenceError StructRNA removed`; путь до этого не использовался). `qa_sheet_health`: у голема Obsidian срез 97% в ряду смерти (падает за кадр) — косметика, не боевое.
  - **Гардероб:** блок «Слоты облика» (вкладки слотов с меткой переопределения, плитки «как у облика» / base / сеты с миниатюрой «тело + часть», покупка непосредственно там) и «Призывы» (обычный / скины). Витрина показывает собранный облик. `data-testid`: `arcade-wardrobe-slots/slot-<slot>/parts/part-<src>/part-follow`, `arcade-wardrobe-summons/summon-<id>/summon-<art>-base`.
  - **Проверки:** `arcadeParts.test.ts` (правила слотов на подменённой таблице, стор, миграция сейва без слотов, данные ↔ манифесты слоёв ↔ листы на диске, скины призывов ↔ герои/манифесты); `arcadeSheetIds` учитывает слот `summon`. Полный vitest, typecheck, Playwright-QA на 5273.
  - ⬜ Дальше: волки Lycan (ремап костей скина); остальные герои волнами (`plan --heroes …` — 109 героев с сетами на базовом теле); слои для аркан на базовом теле со стилями (MK) — в базовом стиле; формы (Метаморфоза) со своими слотами; слои части для арканы-модели (Jugg/Drow arcana как тело + свободные слоты); больше скинов призывов (hawk Visage/Beastmaster, тотемы); items_game.txt как источник слотов вместо имени файла — если имя подведёт.

### T13.88 — Наследие Aegis: удобства вместо силы, больше веток, сброс спрятан (владелец 2026-09-13) ✅ (2026-09-13, `a0.71.0`)
Владелец: «прокачки такие же, как XP-радиус, мб скорость бега и т.д. — не прямые бафы урона, HP; сделать больше прокачек; спрятать кнопку Reset seals».
- **Ветки (5 × 4 пункта, `content/legacy.ts`):** Исследование +3%/пункт радиуса сбора опыта (как было), Лёгкий шаг +1%/пункт скорости бега, Бережливость −2%/пункт к ценам лавки (поверх скидки каравана), Прозорливость +1 бесплатный реролл уровня за забег на пункт (дальше обычная лестница цен), Припасы +10 золота на старте за пункт. Стойкость и Сила убраны; чужие ветки сейва обрезаются `clampLegacy`, вложенные в них печати возвращаются в свободные (проверено Playwright: профиль с vitality 2 / might 1 → «6 свободных из 6»). Снимок в симе `LegacyBonus { pickup, speed, shop, rerolls, gold }`, урон и HP наследие больше не трогает; реплей несёт пять чисел (`r.s.t.i.p`), старые коды с тремя не читаются как наследие (версия конфига `a0.71.0` всё равно их отсекает).
- **UI:** кнопка сброса не висит: в шапке панели у счётчика печатей ссылка «перераспределить» (только когда что-то вложено), по ней — строка подтверждения «Снять все печати?» с «Сбросить печати» / «Оставить». Подсказка панели начинается с «Только удобства, не сила».
- **Проверки:** `arcadeLegacy.test.ts` переписан (бонусы, обрезка старых веток, сим: радиус/скорость/лавка/рероллы/стартовое золото, урон и HP равны базе; реплей), `arcadeTraits` — заглушка из пяти нулей; полный vitest; Playwright на 5273 (миграция, вложение, ссылка сброса и подтверждение).
- ⬜ Идеи следующих веток (если 20 печатей мало): дальность обзора/тумана, скорость подбора предметов, +1 слот сумки, скорость каравана.

### T13.87 — Выбор ранга не запоминался и перебивался реплеем (владелец 2026-09-13) ✅ (2026-09-13)
Владелец: «прошёл на Drow, посмотрел реплей, зашёл — прогресс не стал Herald 3; может, забыл поставить Herald 2». В истории забег записан как Herald ★ — играл на Herald 1, и это не его ошибка: выбранный ранг жил только в памяти (`rank: 0` при загрузке страницы — каждый заход начинался с Herald 1), а `startReplay`/`startDaily` переписывали выбор рангом реплея/дейлика. Теперь ранг хранится (`aegis-draft.arcade.rank`, читается с зажимом по открытому), реплей и дейлик ранг игрока не трогают (герой/акт для HUD — как раньше). Тест `arcadeRankPick.test.ts`. Открытие Herald 3 требует победы на Herald 2 — на следующем забеге ранг уже будет стоять.

### T13.84 — Миникарта и указатели на редкий лут (владелец 2026-09-13) ✅ прототип (2026-09-13) P1
Владелец: «миникарта в правый нижний угол; если падают красные предметы — пометка и на карте, и с какой стороны, если ушли далеко».
- **Миникарта** (правый нижний угол, поверх HUD, рисует renderer из сима): контур арены акта, герой, места карты значками приглашений (`MARKER_TONE`), активные события (bounty, сундук, разлом, караван, патруль/знаменосец), элиты и боссы, редкий лут на земле. Без кликов; на геймпаде и мобиле — тот же рисунок, масштаб по короткой стороне экрана (~140 px), прозрачность под пальцем джойстика не мешает (правый низ — свободен, джойстик слева).
- **Указатели на лут**: `groundLoot` с редкостью rare+ («красные») — метка на миникарте и, если предмет вне экрана, стрелка по краю экрана в его сторону с расстоянием и таймером: лут лежит 90 с (`loot.lootLifetime`) и пропадает молча. Тот же контур указателей, что у приглашений (`Invitation`), — новый вид `loot`.
- **Проверка:** Playwright на 5273 — миникарта есть, метка лута появляется у дропа rare+ и исчезает с подбором; бот не трогать (косметика HUD).
- ✅ **Сделано:** `renderer.drawMinimap` — правый нижний угол, 104–150 px по ширине экрана (на узком экране поднята над строкой умений): арена, полоса реки, окно камеры, места точками в цвете приглашения (сделанные — тусклее), события с таймером кольцом с пульсом (лавка, bounty, руна, сундук, токен, святилище), редкий лут ромбом цвета редкости, элиты/боссы/охотник точками, герой с пульсирующим кольцом. `drawLootPointers` — для refined+ лута вне экрана стрелка у края в цвете редкости с дугой остатка (`loot.lootLifetime`), обычный лут не зовёт. Только рендер, сим не тронут. Проверено Playwright на 5273 (десктоп и 390 px): ошибок нет, лут-указатель и кольцо bounty видны. ⬜ Хвост: стрелки-указатели ложатся поверх миникарты в углу — если будет мешать, обходить её прямоугольник.
- **Правка по плейтесту (вечер 2026-09-13):** владелец — «видно абсолютно все вещи и направление до всех — шум; сделанные ивенты продолжают отображаться». Теперь на миникарте и в указателях только арканный («красный») лут; сделанные места (лагерь снят, аванпост взят, пруд использован, чемпион мёртв, кузня использована, разлом закрыт) с карты уходят, разлом и кузня до своего часа — тусклые. «Портал» с фиолетовым кольцом и тёмным столбом на его скриншоте — это exotic-предмет на земле со столбом света редкости, не разлом; столб в ночи читается как закрытый портал — ⬜ подумать над видом столба.

### T13.85 — Разлом пропал на Herald 2: композиции без разлома (владелец 2026-09-13) ✅ (2026-09-13, `a0.70.0`) P1
Владелец: «запустил Herald 2 — не нашёл ни одного портала (раньше были на 5-й и 12-й минуте)».
- **Причина найдена:** с `a0.65.0` (T13.78) сид выбирает композицию, и разлом есть только на «Торговом пути» (`compositions.ts`): «Дикие угодья» (лагерь, аванпост, пруд, роща, курган, логово) и «Осада леса» его не содержат. На 12-й минуте была кузня — она тоже только на Торговом пути. Игрок этого не видит до боя (чип состава — в HUD).
- **Предложение:** разлом — базовый бит акта (как пруд и аванпост), не вырезать ни из одной композиции; уникальными оставить караван + кузню (trade), курган (wilds), патрули (siege). На подготовке показывать состав по сиду: «В этом лесу: разлом 5:00, кузня 12:00, курган…». Версия конфига поднимается (меняется раскладка мест по сиду). Замер ботом `--composition` после правки — доля побед по композициям не должна разойтись больше шума.
- ✅ **Сделано:** разлом добавлен в «Дикие угодья» и «Осаду леса» (`compositions.ts`, комментарий-правило в шапке: пруд, аванпост, разлом обязательны); описания композиций и подсказка на подготовке говорят «В любом лесу: аванпост, пруд и разлом (5:00)», у Торгового пути — кузня (12:00). Версия конфига `a0.70.0` (вместе с T13.86). Тесты композиций обновлены (разлом есть везде). **Дополнение (владелец, вечер 2026-09-13: «портала на 12 минуте не появилось»):** «портал на 12-й» — это кузня (`forge.fromTick` 12:00), её не было в «Диких угодьях»; теперь кузня тоже в каждой композиции (у Торгового пути остаётся скидка каравана), из композиций вырезаются только караван, курган и патрули. Подсказка: «В любом лесу: аванпост, пруд, разлом (5:00) и кузня (12:00)». Разлом — один на забег (после испытания закрыт), второго «портала» и раньше не было. Замер ботом (seed compcal, полный акт, все места; до правки — T13.78, n=48): «Дикие угодья» Juggernaut 2.1% (было 4.2%, n=48), Sniper 33.3% (было 33.3%, n=24); «Осада леса» Juggernaut 0% (было 6.3%, n=24), Sniper 37.5% (было 31.3%, n=24) — всё в пределах шума (±10 п.п. при n=24), разлом бот делает в 6–50% забегов. Урок прогона: два одинаковых прогона бота параллельно вдвое медленнее каждого — Sniper на полном акте с n=48 ≈ 40 мин, гонять по одному. Состав по сиду на подготовке уже показывался при впечатанном сиде (`arcade-composition`); для случайного сида — общая строка.

### T13.86 — Сундук и bounty исчезают слишком быстро (владелец 2026-09-13) ✅ (2026-09-13, `a0.70.0`) P1
Владелец: «увидел активность с проклятым сундуком, подошёл, начал бить мобов — через 3 с сундука не стало; жёлтый шар с долларом тоже быстро пропал».
- **Причина:** таймер идёт с момента появления, а не с прихода героя: сундук живёт 60 с (`loot.chestLifetime`), bounty-руна 40 с (`bounty.lifetime`); пока герой дочитал приглашение и добежал (300+ px при скорости ~250 px/с плюс бой по пути), осталось несколько секунд. Остаток времени нигде не показан.
- **Предложение:** (а) «пришёл — твоё»: таймер замирает, пока герой в радиусе события или дерётся с его охраной (проклятый сундук — пока живы стражи); (б) базовые сроки: сундук 60 → 90 с, bounty 40 → 60 с; (в) остаток времени — кольцо-таймер на указателе и на миникарте (T13.84), чтобы решать, бежать ли. Все три — правки сима/HUD с бампом версии конфига; бот ценность не покажет (идёт сразу), критерий — плейтест.
- ✅ **Сделано:** `ARCADE.events { holdRadius 220, holdMin 20 с }` и `sim.holdEvent()` для bounty, сундука, руны и нейтрального токена: в радиусе от героя таймер замирает (`until` растёт с тиком), а при приходе остаток поднимается хотя бы до 20 с; вышел из радиуса — идёт дальше. Сроки: сундук 60 → 90 с, bounty 40 → 60 с. Остаток виден: дуга вокруг стрелки-указателя у края экрана (`Invitation.until/life` у лавки, bounty, руны, сундука, токена) и дуга в мире у монеты bounty и над сундуком. Тесты `arcadeEventHold.test.ts`. Не трогал: лавочника (свой срок 45 с — торговля идёт в окне) и караван (ждёт по своим правилам).

### T13.79 — Шаман поддержки: группа с временным щитом ✅ прототип (2026-09-12) P2 (аудит 2026-09-12 §4, `a0.67.0`)
- **Что:** с 4-й минуты в любом акте раз в 60 с приходит шаман (новый вид `shaman`, элита, 150 HP, лист Ogre Magi) с тремя охранниками из пула минуты (охрана держится у него — та же механика `leader`, что у патруля). Шаман не лезет в ближний бой: держит ~200 px от героя (подходит издалека, отступает вплотную) и каждые 9 с накрывает союзников в 150 px щитом на 5 с — урон по ним ×0.3 (пунктирное ледяное кольцо). Сам шаман без щита: **приоритет цели**, его смерть снимает щит со всей группы сразу. Окно без щита 4 с из 9 — AoE и мили успевают. Живых шаманов не больше двух.
- **Проверки:** `arcadeShaman.test.ts` — приход с охраной по расписанию, удержание дистанции, щит на союзниках и не на шамане, ×0.3 и истечение, смерть снимает щиты, лимит живых, до 4-й минуты нет. Playwright: группа у героя, кольца щита.
- **Замер ботом (n=48, seed modcal, short, Herald, все места):** с шаманами Juggernaut 25.0% побед (база до шаманов 18.8%), Sniper 43.8% (41.7%) — в пределах шума, шаман ни разу не добил героя. Для бота группа не «стена»: он бьёт ближайшего, и 4 с окна без щита хватает. Числа не трогаем; плейтест мили покажет, читается ли приоритет цели человеком — если нет, shieldMult 0.3 → 0.5 или shieldSec 5 → 4.
- ⬜ Дальше по §4: строй стрелков с объявленным залпом, спороносец.

### T13.78 — «Осада леса»: третья композиция с патрулями и знаменосцем ✅ прототип (2026-09-12) P2 (аудит 2026-09-12 §1, `a0.65.0`)
- **Что:** композиция `siege` (лагерь, аванпост, пруд, роща, логово, кузня; без каравана, разлома и кургана) со свойством акта «Осада леса»: с 1:00 и дальше каждые 50 с из-за края приходит патруль — **знаменосец** (новый вид `standard_bearer`: элита, 260 HP, лист `dark_troll`, знамя над головой) и 4 охранника из пула минуты. Знаменосец не гонится за героем, а ходит по маршруту между местами карты (следующая точка — случайное место дальше 120 px), охрана держится в 90 px от него, пока герой дальше 220 px, потом дерётся как обычно. **Убил знаменосца** — 25 с лес и волны спавнятся ×0.5 (`siegeMult()` на скорость спавна и размер крип-волны), охрана без вожака и берёт +50% урона 8 с; чип «Патруль разбит · волна слабее 0:45». Живых знаменосцев не больше трёх. Сид выбирает теперь одну из трёх композиций (порядок распределения сидов изменился — версия конфига поднята).
- **Проверки:** `arcadeCompositions.test.ts` — патруль по расписанию со свитой, знаменосец идёт к точке, охрана рядом, гибель даёт ослабление и деморализацию, вне «Осады» патрулей нет; все три композиции выпадают по сидам. Playwright: строка состава, патруль по хуку, знамя на экране, чип после убийства.
- **Замер ботом (n=48, seed compcal, полный акт, Herald):** первая версия (окно 45 с при патруле каждые 40 с) — Juggernaut 0%, **Sniper 45.8% при уровне p50 38 и 12 820 убийств**: дальник снимал знаменосца за секунды, и −50% спавна держалось почти постоянно — осада была самой лёгкой композицией. **Ужато (`a0.66.0`):** окно 25 с, патруль каждые 50 с (доля ослабления ≤ 50%). После: Juggernaut 6.3%, Sniper 31.3% (уровень 27) — в ряду остальных композиций (все места 4.2/39.6, Дикие угодья 4.2/33.3, Торговый путь 6.3/31.3). Урок: награда «слабее волна» должна быть короче интервала её источника, иначе ranged превращает её в постоянный режим.
- ⬜ Дальше по аудиту §4: шаман поддержки со щитом группы, строй стрелков с объявленным залпом, спороносец — отдельными срезами после плейтеста осады.

### T13.77 — Экспедиции: цели после Наследия ✅ прототип (2026-09-12) P2 (аудит 2026-09-12 §6)
- **Что:** пять экспедиций по три шага (`content/expeditions.ts`): «Первая дорога» (разминка, полный акт, лагерь), «Охотник» (контракт, контракт с Клятвой, победа с выполненным контрактом), «Торговец» (караван, кузня, победа на Торговом пути), «Дикие угодья» (аванпост, Некромант, победа в Диких угодьях), «Испытания» (победа с принятой порчей, разлом без воскрешения, Dire без единой смерти). **Правило:** в одной экспедиции герой закрывает только один шаг — три шага = три разных героя (причина играть разными); порядок свободный, стрика нет, смерть не сбрасывает. Награда — титул (Путник, Ловчий, Караванщик, Следопыт, Несломленный), без боевой силы — по условию аудита не расширять Наследие/экипировку/особенности одновременно.
- **Данные:** `ArcadeProgress.expeditions` (id → шаг → герой) и `titles`; в запись истории добавлены `composition/oath/caravan/forged/cursesTaken`, в итог сима — `composition/oathDone`. Старые профили и записи без полей — пустые шаги, ничего не ломается (санитайзер отбрасывает чужие id).
- **UI:** Штаб → Аркада → «Экспедиции» (шаги ✓/○ с именем героя, титул); на подготовке компактная панель под мастерством с подсказкой, какой шаг может закрыть выбранный герой («этот герой уже закрыл шаг здесь — нужен другой»).
- **Проверки:** `arcadeExpeditions.test.ts` — предикаты по записям, один шаг на героя, порядок свободный, титул при трёх шагах, повтор без дубля. Playwright: панели на подготовке и в Штабе.
- ⬜ Дальше по аудиту: альтернативные стартовые наборы с равным бюджетом, слот сохранённого билда, памятная рамка за титул (cosmetics `frame`), добровольные условия забега с отдельным зачётом.

### T13.76 — Бот: политика контрактов, модификаторов билда и подарка каравана ✅ (2026-09-12) P2 (аудит 2026-09-12: «улучшить бота для покупки/контрактов — основа замеров»)
- **Флаги `npm run sim:arcade`:** `--contract` — берёт первую цель контракта и с 8-го уровня идёт к её дому (`contractHome()`), бой ведёт обычная политика; `--contract oath` — с Клятвой охотника; `--build debt,ritual` — «Долг силы» в лавке раньше покупок и «Ритуал очищения» у пруда вместо снятия порчи (пруд — через `--places pond`); `--gift item|upgrade` — что брать в подарок у каравана. Итог печатает «contracts done · build mods taken». Без флагов — прежний бот.
- **Замер (n=48, seed modcal, short, Herald, композиция all; шум ±10 п.п.):**

| Политика | Juggernaut побед | Sniper побед | Примечание |
|---|---|---|---|
| база | 18.8% | 41.7% | |
| `--contract` | 22.9% | 50.0% | контракт выполнен 14.6% / 43.8% — награда сверх обычной, цель по пути |
| `--contract oath` | 16.7% | 37.5% | выполнен 10.4% / 27.1% — −15% по толпе бот чувствует, +40% по цели не выбирает: **клятва для бота — минус**, для игрока с фокусом на цель — плейтест |
| `--build debt` | 14.6% | 35.4% | взят 27% / 71% — половина дохода за одну карту exotic бот не отбивает (покупает жадно) |
| `--build debt,ritual --places pond` | 16.7% | 39.6% | ритуал возвращает часть: «Милость лавочника» ×1.5 к золоту |
| `--places caravan --gift item` | 20.8% | 52.1% | караван доведён 44% / 81% |
| `--places caravan --gift upgrade` | 22.9% | 56.3% | подъём редкости первого не-arcana предмета вместо товара; доведён 43.8% / 81.3% |

- **Вывод:** подарок каравана в любом виде — плюс (товар или подъём редкости — в пределах шума друг от друга); контракт без клятвы — чистый плюс (+4/+8 п.п.), клятва и долг силы — настоящие компромиссы, которые бот проигрывает (он не строит игру под них). Числа не трогаем: кандидаты на смягчение после плейтеста — `oath.trashMult` 0.85 → 0.9 и `debt.share` 0.5 → 0.35, если и у человека окажутся «ловушками».
- ⬜ Бот не выбирает цель контракта по награде и не сравнивает товар с подъёмом редкости — политика «первое доступное»; для замера баланса этого хватает, для замера «полезности награды под билд» — нет.

### T13.75 — Модификаторы билда: «Долг силы» и «Ритуал очищения» ✅ прототип (2026-09-12) P2 (аудит 2026-09-12 §3, `a0.64.0`)
- **«Долг силы»** — в Secret Shop раз за акт кнопка «карта школы exotic сейчас — долг N золота» (`SHOP_ACT.debt`): карта приходит отдельным экраном (`queueReward`), взамен существующая порча «Долг лавочнику» на сумму по минуте (половина дохода уходит в погашение, выплатил — порча снята). Только без другой порчи и один раз (`debtOfferTaken`); сумма и способ закрытия видны до выбора — как требовал аудит («не второй параллельный счётчик»).
- **«Ритуал очищения»** — у пруда при порче третья кнопка (`POND_RITUAL_ACT`): порча снимается и на 3 мин становится свойством билда — увядание → «Цветение» (лечение и регенерация ×1.5), долг → «Милость лавочника» (золото ×1.5; остаток долга списывается), кровавая охота → «Азарт охоты» (урон +10%). Пруд одноразовый и заражённый пруд ритуал не проводит — бесплатные награды не зациклить. Чип в HUD с таймером.
- **Вместе с «Клятвой охотника» (T13.72)** это три поворота билда из отчёта: каждый привязан к существующему событию (контракт, лавка, пруд), не требует школы и не отключает кит.
- **Проверки:** `arcadeCurses.test.ts` — долг силы: карта exotic + порча с суммой, повтор и при чужой порче недоступен; ритуал: снятие порчи, ×1.5 к лечению до истечения, золото ×1.5 и списание долга, урон ×1.1, без порчи не запускается. Playwright на 5273: кнопка в лавке → порча и карточка, кнопка ритуала → чип.
- ⬜ Плейтест трёх модификаторов на мили и дальнике; замер ботом невозможен без политики выбора (бот в лавке берёт товар, у пруда лечится).

### T13.73 — Свойство акта: взаимодействие мест внутри композиции ✅ прототип (2026-09-12) P2 (аудит 2026-09-12 §1 «одно объявленное свойство акта»)
- **Что:** у каждой композиции полного акта одно свойство (`CompositionDef.property`, `sim.actProperty()`), объявленное в описании состава на подготовке. **«Заражённый водоём»** (Дикие угодья): пока стоит лагерь порчи, пруд лечит вдвое слабее (`pondHealFrac()`) и не смывает порчу; очистил лагерь — пруд чист и `used` сбрасывается (можно и второй раз). Игрок решает: пить заражённую воду сейчас или снести лагерь ради полного пруда. **«Торговый путь»** (Торговый путь): после прибытия каравана цены кузни вдвое ниже (`forgePrice`) — сопровождение стало нужным и тем, кто не покупает. Разминка/Dire/River — без свойства.
- **UI:** подсказка в окне пруда («вода заражена…», кнопка снятия порчи выключена), в кузне — «приведи караван» / «караван привёз материалы».
- **Проверки:** `arcadeCompositions.test.ts` — заражённый пруд не снимает порчу и лечит 25%, после `clearCamp` чист и снова готов; кузня вдвое дешевле только после прибытия и только на Торговом пути. Playwright: подсказки в пруду и кузне.
- ⬜ Дальше: «Осада леса» (патрули по тропам, знаменосец ослабляет волну) — требует нового поведения врагов; сначала плейтест двух свойств.

### T13.74 — Разбор забега на экране итога ✅ (2026-09-12) P2 (аудит 2026-09-12 §8)
- **Что:** сим считает урон по источникам (`dmgSource`: автоатака, слот умения Q/W/E/R, призывы, школы и пассивки, яд/кровотечение, снаряды умений) — метка ставится в начале тика и у каждого источника, в `damageEnemy` копится только фактически снятое HP; полученный урон — по видам врагов (`takenByKind`, снаряды без владельца — `projectile`); при смерти `killer` = вид последнего ударившего. В итог (`ArcadeOutcome.killer/dealtBySource/takenByKind`, поля необязательные — старые записи не ломаются).
- **UI:** блок «Разбор» под таблицей итога: «Добил: Кентавр», «Урон нанесли: Blade Fury 54% · автоатака 31% · …» (умения — своими именами), «Урон получен от: Кобольд 40% · …» — по четыре главных источника.
- **Проверки:** `arcadeBreakdown.test.ts` — killer по последнему удару, суммы по видам, автоатака в источниках, победа без killer. Playwright: блок на экране смерти.
- ⬜ Дальше по аудиту: «какие события помогли» (уже есть строки лагерь/контракт/караван в итоге), причина смерти по месту (у какого места погиб), сохранение разбора в историю забегов.

### T13.71 — Караван: объявленное семейство товара и подарок вместо одной скидки ✅ (2026-09-12) P2 (аудит 2026-09-12 §2, `a0.63.0`)
- **Что:** у каждого предмета лавки семейство `family` (`offense`: Butterfly, Desolator, MoM, BFury; `defense`: Wand, Vladmir, Assault, Heart, BKB; `utility`: Travel Boots, Midas, Octarine). Караван получает семейство по своему seed и объявляет его до сопровождения (чип «Караван ждёт · везёт: оружие», в пути — тот же чип с прогрессом). Лавка каравана торгует только этим семейством, скидка 30% остаётся, и сверх неё **подарок**: первый товар бесплатно (`shopBuyPrice(i)` = 0) или подъём редкости одного своего предмета на ступень (`SHOP_ACT.upgradeBase + i`, кнопка в карточке предмета). Подарок один, сгорает с уходом торговца (закрыл лавку — нет). Плановый торговец — как раньше.
- **Зачем:** аудит: «одной скидки мало, когда нужного товара нет или шесть слотов заняты» — теперь сопровождение это выбор под билд (семейство известно заранее), а награда выдаётся даже при полных слотах (улучшение).
- **Проверки:** `arcadeCaravan.test.ts` — семейство детерминировано по seed, офферы только из него, первый товар бесплатно при 0 золота, подарок одноразовый, подъём редкости standard→refined и не дважды, плановый торговец без подарка. Playwright на 5273: чип ожидания с семейством, подсказка подарка в лавке, цена «бесплатно», кнопка «Поднять редкость» и результат в симе. Бот покупает через `shopBuyPrice`.
- **Замер `--places caravan`, n=48, seed placecal, short, Herald:** Sniper 62.5% побед, караван доведён в 89.6% забегов (база без мест 58.3% на n=72); Juggernaut 16.7%, доведён в 35.4% (база 13.9%). Подарок не ломает баланс: плюс в пределах шума, а у дальника сопровождение стало почти обязательным маршрутом — это и есть цель. Бот берёт первый бесплатный товар; выбор «товар vs подъём редкости» оставлен человеку (плейтест).

### T13.72 — «Клятва охотника»: один поворот билда за акт на контракте ✅ прототип (2026-09-12) P2 (аудит 2026-09-12 §3, `a0.63.0`)
- **Что:** на экране контракта переключатель «Клятва охотника» (в сим уходит `act` 6/7 = цель 1/2 с клятвой, `CONTRACT_OATH_ACT`). Пока цель жива: урон по обычной толпе ×0.85, по цели контракта ×1.4, по остальным чемпионам/боссам/строениям — без изменений (`oathMult` в `damageEnemy`, один множитель на весь исходящий урон, как Наследие). Выполнил — +1 ранг самому прокачанному из Q/W/E с запасом до конца акта плюс обычная награда контракта. Чип контракта показывает «· клятва». Не требует школы и не отключает кит — по условию аудита.
- **Проверки:** `arcadeContract.test.ts` — act 6 берёт цель с клятвой; множители 0.85/1.4/1.0 по кобольду, цели и чужому чемпиону; после убийства цели +1 ранг и множители сняты; act 1 без клятвы. Playwright: переключатель и кнопка → `contract.oath === true`, чип.
- ⬜ Дальше по аудиту: ещё 2–3 модификатора на выбор («Долг силы» через существующий долг, «Ритуал очищения» через пруд) — после плейтеста клятвы на мили и дальнике; замер ботом клятвы невозможен (бот контракты пропускает) — нужна политика бота для контрактов.

### T13.70 — Композиции полного акта: «Дикие угодья» и «Торговый путь» ✅ прототип (2026-09-12) P2 (аудит 2026-09-12 §1, `a0.62.0`)
- **Что:** полный Radiant-акт собирается из одного из двух наборов мест по seed (`content/compositions.ts`, `compositionFor(seed, act)` — чистая функция, та же на подготовке и в симе). **Дикие угодья:** лагерь, роща, курган, логово, пруд, аванпост — маршрут через чемпионов, без кузни/разлома/каравана. **Торговый путь:** караван, кузня, разлом, пруд, аванпост, роща, логово — золото и снаряжение, без лагеря и кургана. Инвариант любой композиции: пруд, аванпост и ≥2 чемпиона (иначе контракту некого предложить) — закреплён тестом. Разминка, Dire и River — как раньше (`all`); `ArcadeOptions.composition` перекрывает seed (тесты, бот `--composition`).
- **Объявление:** на подготовке под целью акта строка «Состав карты: …» (по введённому сиду; без сида — «решает сид»), в HUD чип с названием. Реплеи прежних полных актов не совпадут — версия конфига поднята.
- **Проверки:** `arcadeCompositions.test.ts` (детерминизм по seed и только для `full`, инварианты, размещение по option/seed, контракт предлагается в обеих, дайджест); тесты приглашений и контракта, которым нужны все места, явно берут `composition: "all"`. Playwright на 5273: строка на подготовке меняется с сидом, чип в HUD, места в симе соответствуют.
- **Первый замер ботом (n=12, seed `compcal`, полный акт, Herald, `--places all`, шум ±15 п.п.):** Juggernaut — все места 8.3% побед, Дикие угодья 0%, Торговый путь 0%; Sniper — все места 58.3%, Дикие угодья 58.3%, **Торговый путь 25.0%**. **Замер n=48 без `--places`:** Juggernaut — все места 4.2%, Дикие угодья 4.2%, Торговый путь 6.3% (шум); Sniper — все места 39.6% (уровень p50 32), Дикие угодья 33.3% (27), Торговый путь 31.3% (24). Композиции не ломают баланс: разница у дальника 6–8 п.п. при шуме ±10, а уровень ниже потому, что мест с опытом (охрана лагеря, курган) на карте меньше — это и есть цена маршрута через торговлю, компенсировать её должны награды каравана/кузни/разлома (следующий шаг аудита «награды, меняющие решение о маршруте»). Числа не трогаем до плейтеста. Бот: `--composition all|wilds|trade`.
- ⬜ Дальше по аудиту: одно объявленное свойство акта со взаимодействием мест («очищенный лагерь улучшает пруд», «караван объявляет семейство товара»), выбор товара у каравана, модификатор билда за акт. Сначала — замер ботом обеих композиций и ручной плейтест: выбирает ли игрок разные маршруты под разные сборки.

### T13.69 — Экспорт/импорт профиля (владелец 2026-09-12) ⬜ P3
Сейвы лежат в `localStorage` и привязаны к origin (схема + хост + порт): на dev каждый IP сети — свой профиль, на проде origin один, но разные устройства/браузеры и возможный переезд на свой домен прогресс не переносят. Сделать в настройках «Экспорт профиля» (один JSON: карьера, прогресс Аркады с наследием/мастерством/бестиарием, косметика и избранное, настройки) и «Импорт» с подтверждением и слиянием по правилу «максимум достижений, не затирать» (см. `sanitize*`/`emptyProgress` — импорт проходит через те же санитайзеры, что загрузка из хранилища; версия формата в файле). DoD: экспорт → очистка origin → импорт даёт тот же Штаб; чужой/битый файл отклоняется с понятной ошибкой; тест на round-trip.

### T13.66 — Рефактор Аркады: аудит трёх слоёв, горячие циклы, дубли, найденные баги ✅ срез 1 (2026-09-12)
Владелец: «проверь весь код — что оптимизировать/убрать/улучшить». Три параллельных аудита (сим; рендер/экран; контент/скрипты/тесты), правки — только с доказательством: бот на 3 героях × 16 сидов с `--places all` до и после совпадает бит-в-бит, полный vitest зелёный, Playwright на 5273 без ошибок консоли.
- **Сим (`sim.ts`, коммит 4c29a3c6).** Карта «вид умения → слот» строится один раз в конструкторе вместо 12 `ABILITY_KEYS.find` на тик; семь жёстко зашитых слотов (`abilities.e/w/r` у Static Field, Headshot, Presence, Omni, Counter Helix, Reincarnation) переведены на ту же карту — герой с таким умением в другом слоте больше не «молчит». Общая очередь наград `queueReward`: лагерь и курган **затирали** карточки разлома/контракта/чемпиона, поставленные раньше — теперь дописывают в хвост. Helpers `updateEngage` (6 копий гистерезиса), `returnHome` (7 копий «домой + регенерация»), `contactDamage` (4 копии) — арифметика та же. Пул спавна кэшируется по порогу минуты (`spawnPool` отдавал новый массив через `Object.values` каждый тик), ячейки сетки переиспользуются, `resetEnemy` вместо двух временных объектов на спавн, `rollRarity`/`pruneFx` без временных массивов. Tether-автокаст сравнивал индекс с `null` (всегда истина) → `>= 0`; `Math.hypot` у Marksmanship → `len` как везде. Заголовок файла честно говорит, что `sin/cos` остались в расстановке мест и орбитах (детерминизм в пределах одного движка).
- **Рендер/экран.** Найдено и починено: **звук смерти всегда был кобольдом** (`soundscape` читал вид из `value`, сим кладёт его в `y2`); **мигание неуязвимости/Aegis не рисовалось** у героев с листами Dota/LPC (`heroAlpha` не передавался в `drawDotaFrame`/`drawCharFrame`); призрачный шлейф рисовал кадр предыдущего героя после смены формы (`lastHero` не сбрасывался); **подгрузка портрета пересоздавала рендерер, ввод и звук посреди забега** (эффект зависел от `hero.picture` → теперь `renderer.setPortrait` отдельным эффектом); **`ctx.font` с `var(--font-display)` невалиден для canvas — все надписи мира молча падали в 10px sans-serif** (семейство читается из токена один раз, `this.font(weight, px)`). Упрощения: таблица цветов маркеров на уровне модуля, `canAdvance`/`prompt` вместо трёх и четырёх копий условий, стаки предметов через `useMemo`, снимки старта через ленивый `useState` (аргумент `useRef` вычислялся каждый рендер), `SFX_DEBUG` один раз, `hash`/`dot` в одном месте, слитые импорты, квадраты расстояний вместо `hypot` на порогах, мёртвые экспорты.
- **Контент/скрипты/тесты.** `ArchetypeId` и всегда-пустая проверка `arcade.arch.*` удалены; `void HEROES` в `replay.ts`; четыре doc-комментария в `config.ts` стояли над чужими ручками (camp/forge/greed/shop) — перенесены; отступы `mapgen`; бот: `--trace` парсится один раз, `SHOP_ACT.sellBase/buy1` вместо магических чисел; тест на равенство множеств `HERO_IDS`/`HEROES` (порядок `HERO_IDS` рукописный — он задаёт героя дня, `Object.keys` его бы поменял); удалён `verify_feel_r15.ts` (ноль ссылок).
- ⬜ **Хвосты аудита (не делал — крупнее одного среза):** `castAbility` ~320 строк → диспетчер по виду + общий `afterCast`; `killEnemy`/`spawnTick`/`tickPets` разбить; `enemiesWithin` со scratch-буфером (36 вызовов, аллокация на каждый); `GearLike` → `GearItem` (снимает ~20 `as GearItem`); общий `test/helpers/arcade.ts` (`step`, `entry`, `rows`, `internals` — по 4–15 копий в тестах); фабрики в `config.ts` (шесть чемпионов повторяют `ccCap/ccResist/regenPerSec`, `perAct(short, full)`); `ArcadeOverlay` для девяти одинаковых окон экрана; `DEV_FREE_SHOP` в опции сима (сейчас реплей из dev не воспроизводится в prod); пул объектов подписей в пиксельном режиме; `qa_frame_drift/gaps` — кандидаты на удаление после сверки с `qa_sheet_health`.
...
### T13.36 — Арканы всем оставшимся героям, ревизия моделей и поведение призывов (владелец 2026-09-08, 13 скриншотов) ⬜ в работе
Разобрано по трём частям.

**1. Призывы и иллюзии бегают за хозяином и бьют (сделано).** Владелец: «паучки бруды стоят на месте и бьют в гигантском радиусе… пенки фуриона так же… если где-то ещё есть такая концепция — убрать полностью; любой суммон/маунт/иллюзия бегает за хозяином и бьёт врагов, как у Naga Siren». Вид `damage_ward` был зоной урона (один зап по ближайшему врагу от точки каста каждые 15 тиков), а существо над ней — картинкой. Теперь призыв — сущность сима: `PetKind "summon"` + таблица тел `SUMMONS` в `content/pets.ts`, спавн `sim.spawnSummons`, поведение — тот же `tickPets`, что у зверинца и иллюзий. Тотемы, которые и в Dota вкопаны (`ward_nether/plague/death/serpent`, `trap_psionic`, `tombstone`), получили флаг `stationary`: стоят где поставлены, но стреляют по конкретной цели — «жжёного круга» не осталось нигде. Зоной остались только умения БЕЗ модели призыва (Macropyre, Chakram) и лечащий тотем (`ward`). Урон за удар — `<DPS> · value · every / count`, где `SUMMON_DPS = 3.2` бегающему призыву (он не простаивает у точки каста и делит урон между врагами) и `WARD_DPS = 4` вкопанному тотему — тому поправка не нужна, он стоит там же и бьёт ту же ближайшую цель, что и прежняя зона. Два урока калибровки: (1) тотем должен бить МГНОВЕННО — снаряд с земли не догоняет бегущую цель; (2) период удара нельзя брать «по весу существа»: DPS от него не зависит (делится), но редкий тяжёлый удар теряется в дороге и в оверкилле — три варда выбирают одну цель, мелочь умирает с первого удара, два остальных уходят в пустоту. Замедление/стан призывам умений НЕ добавляли — прежняя зона их не давала.

Сверка 120 забегов на сид против HEAD в отдельном worktree: Broodmother 60.8/46.7 против 60.8/45.0, Lycan 62.5/46.7 против 59.2/49.2, Lone Druid 50.0/30.8 против 50.8/30.8 — в базе. У Shadow Shaman остаток: «дошёл до Рошана» ниже на ~6–8 п.п. на обоих наборах сидов, при этом «убил Рошана» совпадает (21.7 % против 21.7 % на втором наборе) и побед больше. То есть урон в норме, разъезжается медиана времени жизни; дальше это подгонка под политику бота, а не баланс. ⬜ Перепроверить на живой игре.

**2. Пять недостающих аркан.** Список сверен по `scripts/items/items_game.txt` из vpk (`item_rarity = arcana`), а не по памяти: не хватало Monkey King «Great Sage's Reckoning» (база + голова арканы `monkey_king_arcana_head/mesh`, `--style arcana` подменяет текстуру тела), Lina «Fiery Soul of the Slayer» (`hairoftheslayerr` + `pauldrons_of_the_slayer` + `skirtoftheslayerz10` + `vambracesoftheslayerz1`), Legion Commander «Blades of Voth Domosh» (`demon_sword` вместо базового оружия), Techies «Swine of the Sunken Galley` (`bigshot` + бочка, костюмы Squee/Spleen, фитиль) и Io «Benevolent Companion» (`io_ti7` — тот самый Companion Cube). У Invoker арканы нет (Magus Apex — immortal), больше аркан в игре не осталось. Заодно исправлены неверные названия в i18n: у Rubick было «Magus Apex» вместо «The Magus Cypher», у Spectre — «Divine Anarchist» вместо «Phantom Advent», у Drow — «Frost Avalanche» вместо «Dread Retribution», FV — «Claszian Apostasy».

**Поправка к п. 2, 2026-09-15:** повторная сверка с установленным `items_game.txt` выявила ошибку Lina: перечисленные выше четыре модели принадлежат **Battle Caster**, не Fiery Soul of the Slayer. У настоящей арканы item ID 4794, модель `origins_flamehair`, слот `head`. Исправление и миграция прежнего вида открыты в **T13.80 A1/A2**; [доказательства](audits/2026-09-15-customization.md). Прежняя запись о проверке аркан не доказывает корректность этой конкретной строки манифеста.

**3. Ревизия моделей по скриншотам.** Инструмент: `qa_sheet_health.mts` (теперь не падает на листе, который не открылся) + контактные листы всех 296 листов (кадр idle/walk/attack на лист) — смотреть глазами, а не по метрикам.
- ✅ **Storm Spirit, сет Tormenta «весь белый».** В частях лежала тестовая модель `storm_spirit_tormenta_armor_test.vmdl_c` без материала — белый пластик поверх героя. Убрана.
- ✅ **Mirana, персона «едет на пустоте».** В частях не было `mirana_persona_mount.vmdl_c`.
- ✅ **Rubick, аркана без головы.** Тело арканы `rubick_arcana_base` — только торс с плащом; голова, посох и наплечник у арканы остаются базовыми (`rubick_head`, `rubick_staff`, `shoulder`).
- ✅ **Queen of Pain, аркана «летает в стойке».** В `--anims` стоял клип `qop_arc_new_rig2_fly`; в модели есть `qop_arc_new_rig2_idle` — теперь он (и `..._attack` вместо базового `attack`).
- ✅ **Razor, аркана без свечения.** Материал тела `razor_arcana_base` — шейдерный, без color-текстуры (только specular и spiral overlay), поэтому модель выходила чёрным пятном. Лечится `--glow-mask` по папке арканы + `--glow-mat razor_arcana_base` + `--light 2.0 --autoexpose 0.32`. Вторая «персона» — набор текстур с токеном `alt` в vpk: лист `razor@arcana~style1` через `--style alt`, в косметике стиль зарегистрирован.
- ✅ **Метаморфоза Terrorblade серая.** У листа `@meta` не было ни `--glow-*`, ни автоэкспозиции (в `demon_detailmask` маска пуста — см. T13.33), поэтому серо. Теперь тело демона красится `--glow-mat demon_color`, а у арканы есть свой лист `terrorblade@arcana@meta` (красный): рендер выбирает `<hero>@<skin>@meta` раньше `<hero>@meta` и красит его самоцветом — раньше форма самоцвет игнорировала.
- ✅ **Io «непонятный шар».** Модель `wisp` действительно шар, весь вид Io в Dota — частицы (это было записано в T13.33). Сделано так, как там и планировалось: `HERO_AURA` в `sprites.ts` — встроенное свечение героя, рисуется слотом `aura` и в бою, и в превью гардероба.
- ✅ **Превью героя маленькое и режет модель.** Масштаб превью считался по росту героя и мог выйти за холст (лист с `margin 1.4` рисовался на 280 px в холсте 220 px — оружие и крылья обрезала рамка). Теперь потолок масштаба — целый кадр листа, кадр центрируется по якорю, холст 320 px и ужимается по ширине экрана.
- ✅ **Crystal Maiden, аркана** — к телу арканы не были пришиты ледяной плащ `crystal_maiden_arcana_back` и базовые манжеты/наплечники/посох/головной убор.
- ✅ **Techies без гоблинов** — в базовом листе была только тележка, без костюмов Squee и Spleen.
- ✅ **Маунты пропали у всадников (владелец 2026-09-08: «chaos knight сет — тоже нет лошади под ним»).** Системная дыра манифеста: строка сета перечисляет только части сета, а слоты, которых сет не трогает, остаются пустыми — конь/пантера/нетопырь просто не пришивались. Починены `chaos_knight` (базовый ряд тоже был без `chaos_knight_mount`), `chaos_knight@oda_nobunaga`, `batrider`(+`@fiery_heart`), `luna`(+`@sets_servant`), `snapfire`(+`@snailfire`), `abaddon@alliance_abba`, `disruptor@ragethree`, `techies@mad_tex`, `keeper_of_the_light@cradle_of_lights`.
- ✅ **Legion Commander, аркана: «по сломанному держит меч» (владелец 2026-09-08).** «Blades of Voth Domosh» — парные клинки в одной модели: главный на кости `weapon_0`, второй на `Offhand0_JNT`. В скелете LC кость офф-хенда есть, но клипы её не двигают (у героя по умолчанию нет предмета в левой руке), поэтому второй клинок оставался в позе покоя и лежал на земле «палкой». Второй клинок снят `--drop-vgroup offhand`.
- **Облики героям без косметики (владелец 2026-09-12: «сеты и арканы из vpk»).** Списком vpk (`scripts/dota_vpk_list.mts` — свой парсер директории VPK v2, 384 004 путей за секунду; шишка: длина preload лежит по смещению 4, не 8 — с ошибкой дерево обрывалось на 596 записях) найдены и отрендерены в `dota_px2`+`dota_px`: **Sniper** — сет Monster Hunter «Odogaron» (5 частей: bracer/cape/head/shoulder/weapon, перекрывает все 5 базовых), **Winter Wyvern** — «Wyvern of the Sea» (head+back = оба базовых слота), **Visage** — «Soul Reaper» (head+armor; фамильяр из сета не берём), **Lifestealer** — единственный предмет в vpk, шлем (`lifestealer_helmet`) поверх базы. **Kez** — в vpk нет ни сетов, ни предметов, остаётся базовым. Регистрация — `scripts/dota_register_skins.mts <px2.tsv> <px.tsv>` (строки обоих манифестов, `COSMETICS`, RU/EN i18n, идемпотентно). Проверки: `qa_sheet_health` — метрики на уровне базовых листов (у WW «срез» даже ниже: 30% против 82%); листы просмотрены; гардероб на 5273 показывает карточку и превью каждого. Тесты манифестов/гардероба зелёные.
- **Вторые сеты (в работе 2026-09-12).** `scripts/dota_pick_sets.mts` — автоподбор по списку vpk: группы частей `<set>_<слот>` в `models/items/<герой>/`, ≥4 слота, `_high` предпочтительнее, событийные/служебные имена (`ti\d`, `dotapit`, `aghsbp`, `frostivus`, годы, `dplus`, `cavern_crawl`, `persona`, `arcana`, `fx`, `low`…) отброшены, уже заведённые сеты пропущены. Для 96 героев с одним обликом нашлось 87 сетов (без сета: Storm Spirit, Necrophos, Viper, Jakiro, Enigma, Phoenix, Puck, Underlord, Undying — там части лежат иначе). Рендер тремя чанками (`awk NR%3`) в фоне: 87 листов px2 за ~1 ч, px с `REUSE=1` за ~35 мин. **Готово 2026-09-12:** 87 сетов заведены (`dota_register_skins.mts`), имена — Title Case из id сета; `qa_sheet_health` по всем 87 — пустых кадров нет, яркость 0.16–0.26 (в норме базовых); `riki@frosty_blow` первый раз не декодировался (webp повреждён при параллельной записи?) — перерендерен. Гардероб на 5273: у Lich, Ringmaster, Riki по три облика, превью открываются. Итого обликов 224 на 121 героя; без облика только Kez.
- **Zeus arcana «голый» и Io «не так выглядит» (владелец 2026-09-12, два скриншота).** По `items_game.txt` аркана Zeus — `zeus_arcana.vmdl` + скрытые базовые `vest`/`bracers`, остальные части базы остаются; в манифесте у строки не было частей вовсе — тело без шлема, пояса и сигила. Добавлены `zeus_hair_arcana` (шлем Громовержца) + базовые `zeus_belt`, `zeus_sigil`; лист перерендерен (px2+px), в гардеробе — шлем, синяя юбка, светящиеся кисти, как на референсе; колесница-дым — частицы, в меше их нет. Io: в Dota это шар света с усиками-частицами, меш `wisp.vmdl` — плоский бирюзовый шар; `wisp_additive` как часть встаёт вторым шаром выше тела (не годится). Сделано: ядро отрендерено белым (`--glow-mat .*=225,245,255,1.0 --light 2.0`), а усики и сияние — новый эффект `wisp` в `effects.ts` (два кольца сияния + 5 вращающихся усиков, спереди искры), `HERO_AURA.io = "wisp"` (раньше `frost`). Превью в гардеробе проверено.
- **Alchemist: мечи всегда в руках, а бьёт кулаками (владелец 2026-09-12).** В Dota мечи в ножнах за спиной и достаются только в Chemical Rage (клипы `ability_4_*`: `pullout`, `idle`, `run`, `attack1-3`, `putback`). **Второй заход (владелец: «вырезал мечи полностью»):** кости мечей `Rhand_weapon_2`/`Lhand_weapon_1` висят на кости ножен `sheath5_0` — в базовых клипах они лежат на спине (локальный сдвиг постоянный), в `ability_4_*` уходят в руки; значит модель мечей нужна во всех строках, а не только в форме — убирать её из базы было ошибкой (после отката листы базы и сетов показывают мечи за спиной и кулаки). Добавлены строки формы `alchemist@meta`, `alchemist@frankenstein@meta`, `alchemist@jungle_chief@meta` с мечами и клипами ярости; у R (`frenzy`) появился `form` — `formNow()` теперь берёт любую способность с `form`, а не только `metamorphosis`, и `frenzy` с формой ставит `formUntil` на время бафа; тип атаки и дальность не меняются (форма с `range` = база 88). Гардероб показывает кнопку «Форма» по наличию `form`. Тест в `arcadeForm.test.ts` + проверка манифестов (мечи есть во всех строках Alchemist). **«Застывает при ударе в ульте» (владелец, третий заход) — причина в рендере, не в листах:** сим при Frenzy/Fiery Soul ставит перезарядку удара короче базового интервала (`attackInterval × (1 − frenzyMult)`), а рендер вёл прогресс анимации удара от полного интервала — `atkT` начинался у 0.8 и кадр удара сразу был последним, герой «замирал» в стойке с мечами (то же у Ursa, Io Overcharge и любого ускорения атаки). Теперь сим пишет фактическую длину перезарядки в `player.attackCdMax`, рендер ведёт анимацию по ней; тест в `arcadeForm.test.ts` (в ярости `attackCdMax` < базового интервала). **Четвёртый заход (владелец: «в обычной форме всё равно мечи в руках»):** модель `alchemist_sword` у кости `Rhand_weapon_2` в базовых клипах стоит сбоку на уровне рук и читается как «держит», в Dota она вне Chemical Rage скрыта, а «мечи за спиной» даёт `alchemist_scabbard`. Уточнение по мешу ножен: у `alchemist_scabbard` 358 из 612 вершин (клинки) привязаны к костям `Rhand_weapon_2`/`Lhand_weapon_1` — в базовых клипах они в ножнах, в ярости сами уходят в руки; отдельная модель `alchemist_sword` только дублирует их. А с камеры сверху клинки в ножнах проецируются на уровень рук и читаются как «держит» (владелец видел это и на ножнах без модели мечей). **Итог (пятый заход, владелец: «мечи на спине, руки пустые; в ульте эти же мечи в руках»):** база и сеты вне ярости — ножны `alchemist_scabbard` с клинками (на костях hand_weapon они в ножнах), без оружия в руках; `alchemist@meta` — те же ножны (клинки уходят в руки по костям); сеты `@meta` — оружие сета в руках. Отдельная модель `alchemist_sword` нигде не используется (дублировала клинки). Тест манифестов закреплён под это правило.
- **Настоящая причина «бегает с мечами» (шестой заход, по кадрам бега):** у Alchemist нет клипа `run`, и `pick_action` по подстроке выбирал `ability_4_run` — бег Chemical Rage с мечами в руках (клипы `run_alt`/`run_haste`/`injured_run` штрафовались «мягкими» токенами, `ability_4_run` — нет). Стойка и удар при этом были базовыми — отсюда «бегает с мечами, бьёт руками» с самого первого сообщения владельца. Правка: в строках Alchemist `walk=run_alt` явно, а в `pick_action` токен `ability` добавлен в штрафные — реплика выбора по всем экспортированным героям других жертв не нашла. **Инструмент закреплён:** `web/scripts/qa_clip_pick.mjs [manifest] [--all]` — реплика `pick_action` по клипам glb из `~/dota-export`, печатает клип на каждый ряд и помечает подозрительные (ability/haste/injured/versus/…); списки токенов держать в синхроне с `render_dota_sprites.py`. Прогон по всем 126 героям (недостающие 40 базовых моделей экспортированы S2V): подозрительных рядов 9, все объяснимые — `cast_ability_6` у Keeper of the Light (это клип каста), `whirlingDeath` у Timbersaw (его вихрь), `run_haste` у сета Elder Titan (вариация бега), нет клипа стойки у `ward_death`/`trap_psionic`/`mine_techies`/`beetle` и бега у `io@arcana` (как и раньше — статичные модели). Alchemist был единственным.
- **Кит Io (владелец 2026-09-12, скриншот «крутилки»).** Сверено с Dota: Tether привязывает Io к союзному юниту (луч, лечение/скорость обоим), Spirits — пять духов кружат по орбите и бьют тех, в кого врезались, Overcharge — темп атаки, Relocate — перенос. У нас Q был обычным `ward`, W — `spin` Juggernaut (кольцо урона, автоатака заблокирована). Сделано: виды `tether` и `spirits` (`heroes.ts`, `sim.ts`): **Tether** срабатывает только когда рядом свой юнит — иллюзия, призыв, питомец (`tetherTarget`, радиус 420); без него каст не проходит и перезарядка не тратится; связь 8 с даёт лечение 14–32/с (пока юнит в радиусе) и скорость, рвётся, когда юнит пропал; рендер рисует луч с бегущими бусинами. **Spirits** — 5 шаров на орбите 130 px, оборот 3.5 с (`ARCADE.io.orbitSec`), урон 40–85/с только тем, кого шар касается (`spiritOrbs`, `orbR` 16), 12 с; автоатака не блокируется, вихря нет. Автокаст: Tether при HP < 70% и юните рядом, Spirits при 2+ врагах или боссе. Тесты `arcadeIo.test.ts`; Playwright на 5273: с волком рядом луч и 5 шаров, `spinUntil` не ставится.
- ✅ **Аудит слотов сетов (2026-09-12).** `web/scripts/qa_set_slots.mjs [manifest] [--apply]`: слот части — по токенам имени файла без префикса героя (точное совпадение, затем составные слова от 4 символов: `headitem`, `shoulderpads`, `lweapon`; `mount` раньше `armor`), неизвестный токен = уникальный слот (foliage, abdomen, thorn). Правило как в Dota: слот без предмета сета показывает базовый предмет — базовые части, чей слот сет не перекрыл, дописаны в строку сета. Затронуто 75 строк в обоих пиксельных манифестах (гоблин-наездник и головы у сетов Alchemist, пушки Gyrocopter, ножны/плащи/маунты/тотемы у прочих), перерендерены тремя чанками. Ложные срабатывания прежней эвристики (`mars_spear` → head из-за «ear», `axe_belt` → weapon из-за «axe») сняты токенизацией.
- ✅ **Вечный цикл в `sim_arcade.ts` (найден заодно).** Пока висит лавка или добыча, `sim.step` возвращает управление, НЕ двигая тик, — значит любое «не то» действие бота это бесконечный цикл в прогоне, а не медленный забег. Бот проверял `nearLoot` раньше `shopOpen` и слал в открытую лавку `PICKUP_ACT` (61), которого `shopAction` не знает: seed `cmp-76` на Shadow Shaman крутился 22 минуты на тике 21706. Порядок веток в `botInput` переставлен — открытые окна первыми. Правило на будущее: в `botInput` любое модальное состояние сима обрабатывается раньше «мировых» действий.
- ✅ **Кандидаты на проверку глазами (2026-09-12, кадры стойки вырезаны крупно рядом с базой).** Недостающих текстур (белые/маджента-заплатки, чёрные силуэты) нет ни у одного: `jakiro@ti7_immortal` — тёмно-лиловый дракон, `lich@rime_lord` — ледяной бело-голубой (так и задуман), `medusa@blueice`, `earth_spirit`, `lifestealer`, `hoodwink`, `ward_death` — целые. `vengeful_spirit@arcana` и `mirana@persona` остаются блёклыми относительно референса — это тон текстур при Workbench-рендере, а не дефект сборки; поправлять только по референсу владельца (`--light`/`--autoexpose` на строку), сами не гадаем.
- ⬜ **Кандидаты на проверку глазами (старый список, закрыт выше).** `qa_sheet_health.mts` ранжирует, но не выносит вердикт: «силуэт из N кусков» законен у героя с летающими частями (у `ringmaster`, `muerta`, `wraith_king@arcana` строки манифеста полные — метрика ловит парящие предметы, а не дыру). Смотреть по одному: `ward_death` (8 кусков), `hoodwink*`, `lifestealer`, `jakiro@ti7_immortal` и `lich@rime_lord` (серые/белёсые — похоже на недостающие текстуры), `medusa@blueice`, `earth_spirit`, `vengeful_spirit@arcana` (обесцвечена против референса), `mirana@persona` (маунт вернули, но набор всё ещё блёклый).

### T13.35 — Скин у каждого героя свой (владелец 2026-09-08: «надеваю аркану на одном персонаже, а на другом она снимается») ✅ (2026-09-08)
Был один слот `cosmetics.equipped.skin` на всех героев. Теперь правда — `cosmetics.skins: {hero → id}`, а `equipped.skin` — производное для выбранного героя (`withHeroSkin` в arcadeStore при `equip`/`setHero`/старте), чтобы все читатели слота (рендер, озвучка `skinnedHero`, гардероб) работали без правок. Снятие — у выбранного героя. Старые сейвы: единственный скин переносится герою-владельцу. Бейджи в списке героев — по `skins[id]`. Тест в `arcadeWardrobe.test.ts`.

### T13.33 — Верность моделей Dota: неверные сборки и отсутствующее свечение (владелец 2026-09-07: «аркана Jugg не та», «SF не сходится», «Io — шар», «аркана PA голая», «TB чёрный без свечения») ⬜ разобрано, не начато
Ответ на вопрос «модели нет или мы не так рисуем»: **модели есть, мы собираем и освещаем не так.** Проверено по списку vpk (`Source2Viewer-CLI --vpk_list`, 384 тыс. файлов):
- ⬜ **Shadow Fiend, база.** Манифест берёт `models/heroes/nevermore/nevermore.vmdl_c`. Пробовали `models/heroes/shadow_fiend/shadow_fiend.vmdl_c` + части `shadow_fiend_{arms,head,shoulders,wings}` — получился SF с распахнутыми крыльями, владелец: «это вообще не то, оставь как было» — откачено (2026-09-07). Расхождение с картинкой владельца (сгорбленный демон, шипы на плечах, когти, красные глаза, дым) — не сборка, а свет и материалы: тёмная модель с автоэкспозицией теряет объём, нет selfillum глаз/груди, нет дыма-частиц. Если возвращаться к новой модели — без `wings` (это слот косметики) и с проверкой позы idle.
- ✅ **Juggernaut, аркана (2026-09-07).** Две ошибки сборки. (1) Манифест пришивал к арканe **базовые** маску и меховой плащ (`jugg_mask`, `jugg_cape`) — бурый мех и базовая маска поверх арканы. (2) Тело `juggernaut_arcana.vmdl_c` в vpk — **только торс с руками** (z 1.7–3.6, без ног: у Jugg ноги всегда отдельная модель `juggernaut_pants`), а слоты меча и наручей с арканой в Dota остаются базовыми. Итоговые части: `juggernaut_pants` + `jugg_sword` + `jugg_bracers` + маска арканы `models/items/juggernaut/arcana/juggernaut_arcana_mask.vmdl_c` (в ней же огненная грива `tail_core` и цепи `chain_0..6`). Грива в Dota — additive-шейдер (`*_mask_tail`, текстура с чёрным фоном): в Workbench выходило чёрное знамя 4 единицы длиной → новые флаги рендера `--drop-mat <regex>` (грани материала снимаются) и `--drop-vgroup <regex>` (грани по кости: `chain_[3-9]` укорачивает гриву вдвое, физики ткани нет); сердцевина гривы (`tail_core`, листья на альфе) красится `--glow-from-alpha`, руны на теле и маске светятся через `--glow-mask` (detailmask-альфа). Стили: базовый = бирюзовый (`*_color`), `~style1` = красный (`*_v2_*`, `--style v2`) — как в Dota. Кадр/world 160 как у базы. Диагностика в Blender: `evaluated_get` показал, что тело деформируется, но bbox без ног — так и нашли торс.
- ✅ **Phantom Assassin, аркана «голая» (2026-09-07).** Сборка верная (тело + оружие), а «голая» она потому, что в модели **нет** капюшона и плаща: кости `cape_*` есть, вершин на них ноль — в Dota плащ-дым и шипастый капюшон это частицы (`pa_arcana_elder_ambient_skined.vpcf` и др.), спрайт их взять не может. Узор платья — прозрачные области color-текстуры (16%) красятся бирюзовым свечением `--glow-from-alpha 0.2,0.85,0.85`, `--light 1.5`. Пробовали заменить частицы своим эффектом скина (корона из шипов, капюшон, дым — `fx.aura: "veil"`), владелец 2026-09-08: «выглядит ужасно, пусть будет опять лысой» — эффект удалён, механизм `fx.aura` на скине оставлен (рисуется в бою и в превью, привязан к герою скина) для будущих замен частиц (Io, SF).
- ✅ **Terrorblade: свечение груди и клинков (2026-09-07).** Ядро груди — selfillum в альфе `terrorblade_detailmask` (0.7% текстуры): в 160-px кадре точка терялась → `--glow-grow N` расширяет маску max-фильтром (18 текселей). Клинки в Dota красит градиент `weapon_grad`, а `weapon_detailmask` = 1 всюду → `--glow-mat weapon=R,G,B,сила`: светлые места материала тянутся к цвету свечения; у арканы так же подсвечены тело и броня (`terrorblade_color|armor_color`, сила 0.55) — красные линии по рукам и груди, как на референсе. База — бирюзовая, аркана — красная по умолчанию. Плюс `--light 1.6 --autoexpose 0.3 --expose-target 0.5`.
- ✅ **Самоцветы красят не то (владелец: «зелёные пятна на всех самоцветах должны стать цвета самоцвета»).** Рендер пишет тон свечения в мету листа (`glow`, градусы, из `--glow-color`/`--glow-from-alpha`); `gemSheet` берёт его вместо скана доминирующего тона и с мягкими порогами (s ≥ 0.16, v ≥ 0.38, ±55°) — блёклые пиксели ядра тоже перекрашиваются; при перекраске насыщенность и яркость подтягиваются мягко (s ≥ 0.45, v ≥ 0.6 — сильнее выходило «кислотно», владелец просил ослабить дважды). Свечение TB: клинки сила 0.75, тело арканы 0.4. Корона/капюшон «вуали» привязаны к макушке в центральной колонке контура, а не к верху рамки (в стойке PA руки выше головы); в превью гардероба эффект скина рисуется вместе с надетым свечением, как в бою; рендер хранит `fx` скина по определению косметики (карта слотов держит варианты листов, не id). Метаморфоза TB не тронута (`demon_detailmask` пуста, ⬜).
- ⬜ **Io.** Модель `wisp` — шарик, всё остальное в Dota — частицы. Спрайт честный; «как в Dota» получится только собственным эффектом (кольца-орбиты и свечение в `effects.ts`, по слоту `aura` героя).
- ⬜ **Системно:** конвейер — Workbench без шейдеров Dota: нет selfillum/emissive, нет rim/fresnel, нет частиц, палитра 48 цветов, автоэкспозиция сдвигает тон. План повышения верности: (1) аудит манифеста скриптом — у каждого `@skin` части должны быть из папки скина, а не базы; (2) emissive из масок для всех; (3) Eevee вместо Workbench с emission-нодами; (4) палитра ≥ 96 для аркан; (5) партикл-заглушки по слотам (огонь Jugg, орбиты Io, пламя SF). После этого — сеты героев (`models/items/<hero>/<set>_{arms,back,head,legs,weapon}`): у Jugg в vpk 12+ сетов, у SF 7+.

### T13.34 — Геймпад DualSense (владелец 2026-09-07) ✅ (2026-09-12)
Gamepad API поверх `input.ts` — геймпад даёт тот же `ArcadeInput`, в лог попадают те же `cast`/`act`, детерминизм и реплеи не тронуты (в реплее пад не опрашивается).
- **Чтение пада** — чистый `features/arcade/gamepad.ts`: `readPad` (радиальная мёртвая зона 0.2 с перемасштабированием, D-pad как запасной стик, битмаски `held`/`edges` по 18 кнопкам стандартной раскладки), `PadNav` (один шаг меню на наклон стика или D-pad, повтор — после возврата стика к центру). Тесты — `test/arcadeGamepad.test.ts`.
- **Раскладка** — левый стик/D-pad движение, ✕ ○ □ △ → Q W E R, R2 — атака (тот же бит, что F), R1 — подобрать, L1 — сборка, Options — пауза, тач-пад/PS — вспышка. Правый стик не задействован: направление атаки и так автоматическое.
- **Меню** — пад опрашивается каждый кадр экрана (`pollPad`), а не только на тике сима: в окне карточек стик двигает фокус (`data-pad-focus` на `.arcade-offer-wrap`), ✕ выбирает; ○ закрывает лавку/добычу/пруд/кузню/разлом/контракт/нейтрала (`SHOP_ACT.close`) и экран сборки; ✕ на паузе — продолжить. В меню касты и движение с пада не копятся — ✕ на карточке не выстрелит умением после закрытия окна.
- **Автопереключение как в Steam Input** — первое нажатие/наклон на паде (`onGamepad`) переводит подсказки HUD на глифы (`PAD_GLYPH`: R1 у подбора, R2 у атаки, ✕○□△ у умений); первая клавиша или указатель (`onKeyboard`) возвращает клавиатурные буквы. Событие `gamepadconnected` не нужно — определяем по вводу, так же переключаемся и обратно. Глифы — текст (✕○□△ и R1/R2), отдельные SVG из `ui/` не заводились: символов хватает, а подсказки уже текстовые чипы.
- **Проверка** — Playwright с виртуальным `navigator.getGamepads` на 5273: стик двигает героя, подсказки F→R2 и обратно после клавиши, фокус карточки 0→1 стиком и выбор по ✕, Options ставит паузу. Живой DualSense у бота нет — владельцу проверить тач-пад и вибрацию (вибрация не делалась).
- ⬜ Хвосты: вибрация на попадании/смерти элиты (`GamepadHapticActuator`), навигация по лавке стиком (сейчас в лавке пад закрывает окно, покупки — курсором), подсказки пада на экране подготовки.

### T13.32 — Руны у реки, иллюзии-питомцы, «жирные» следы (владелец 2026-09-07) ✅ (2026-09-07)
- **Руны** (`ARCADE.rune`, a0.39.0): первая на 1:30, дальше каждые 2 минуты, лежит 45 с у реки (точка реки или кольцо вокруг героя); вид — из `RUNE_KINDS` по rng сима, подбор касанием (в лог попадает как движение). Модели — настоящие руны Dota (`props_gameplay/rune_*.vmdl_c`, листы `rune_dd/shield/arcane/illusion` в обоих наборах, 4 КБ каждый, отрисовка с покачиванием и цветной подложкой). Двойной урон — ×2 удара 45 с; щит — половина максимума здоровья, урон снимается с него первым (`damagePlayer`), 45 с; магия — перезарядка ×0.7 на 50 с; иллюзии — две копии героя на 75 с. HUD: чипы «Double damage 0:43» / «Shield 310» / «Arcane». Бот считает руну целью (`spots`).
- **Иллюзии как питомцы.** Новый вид `illusion` в `content/pets.ts`: скорость/интервал/дальность берутся у героя в момент шага (`rangedNow()` — в Метаморфозе Terrorblade копии стреляют «стрелой», иначе бьют вблизи), срок `until` и урон `dmg` в самом питомце, `expirePets()` в начале `tickPets`. Умения с `summon.art === "illusion"` (Terrorblade Conjure Image, Naga Mirror Image, Phantom Lancer Juxtapose, Chaos Knight Phantasm) теперь зовут `spawnIllusions`, а не ставят тотем. Рисуются листом героя (та же форма/скин) с прозрачностью 0.62, гаснут за последнюю секунду. `syncPets` их не трогает. Бот, 12 прогонов: TB 33%, Naga 42%, PL 50%, CK 75% (⬜ CK выше полосы — урезать `dmgFrac` Phantasm).
- **Следы.** Все следы пожирнели (кучки 2–3 точек на шаг, живут 0.8 с); новые — кровавый (standard), листва (refined), пустотный (exotic), призрачный (arcana: полупрозрачные силуэты героя за спиной, 0.4 с). Свечение героя владелец принял («отличное, мне нравится»).
- **Почему Jugg-аркана / SF не «как в Dota»** (вопрос владельца): модели те же, что в клиенте; расхождение — в рендере. Workbench-освещение без шейдеров Dota (нет selfillum/rim/emissive-масок — «горящие» части выглядят плоскими), нет партиклов (пламя SF, огни арканы Jugg — это частицы, не меш), палитра 48 цветов и кадр 160 px гасят градиенты, автоэкспозиция тёмных моделей сдвигает тон. Значит, **сеты добавлять можно** (геометрия/текстуры есть в vpk, `models/items/<hero>/<set>/`), но «как в Dota» они станут только после работы над конвейером: Eevee + emission из masks2, палитра ≥96, партикл-заглушки в effects.ts по слотам (факел/пламя/дым на кости).
- **Панель баффов (владелец: «нет индикации, сколько действует руна»).** Под слотами экипировки — плитки рун: иконка первого кадра модели (`RuneIcon`, лист `rune_<вид>`), имя, остаток `0:42`, у щита ещё запас HP, у иллюзий число копий; полоска тает по времени; цвет — токены `--arcade-rune-*` (те же в `runeColor` рендера). Старые чипы в верхней строке убраны.
- **Аркана — красная, не жёлтая (владелец).** Все `data-rarity="arcana"` в arcade.css → `--card-tier-arcana` (#ff4d5e), кольцо добычи на земле — `--arcade-arcana`; золото осталось только у легендарных карточек и Aegis.
- **Добыча на земле по редкости (владелец: «фиолетовый на земле кажется обычным предметом»).** Кольцо толще, светящаяся подложка и столб света, ярче с редкостью; цвета — `--arcade-refined` (синий), `--arcade-exotic` (насыщенный фиолетовый вместо бледного `--arcade-lightning`), `--arcade-arcana` (красный); обычный — тонкое серое кольцо.
- **Смысл цвета карточки (владелец: «фиолетовый навык реально сильнее или цвет просто так?»).** Реально сильнее: ранг, взятый редким вариантом, входит в силу с множителем `ARCADE.rarity.mult` (refined ×1.35, exotic ×1.8, arcana ×2.4) и поднимает потолок рангов (`rankBonus`); цифры «сейчас → после» на карточке это уже учитывали, теперь строка под названием говорит прямо: «Refined · сила ранга ×1.35 · ранг 1/3».
- ⬜ **Разнообразие режима (идеи, не начаты).** *Карты:* другие ландшафты из vpk (jungle/summer/cavern/reef/journey/ti10), Dire-сторона с своей палитрой и деревьями, день/ночь (с ночным зрением у NS-подобных), река с бродами, аванпосты и святилища как точки. *Активности:* захват аванпоста (канал 6 с под давлением → буст опыта), лотосовые пруды (лечение раз в N мин), руна щедрости (золото), курьер с посылкой (проводить до точки — награда), стакинг лагерей по таймеру (двойной лагерь = двойной лут), Тормнентор как мини-босс с отражением урона, Рошан на 20-й минуте с Aegis (второе дыхание). *Косметика:* курьеры как спутники-косметика (модели `models/courier/*`), скины вардов-тотемов, тонты по клипам Dota (`taunt`), погода (дождь/снег/пепел/лунный луч — оверлей), музыкальные и голосовые паки (анонсеры), эффекты убийства (пикселизация/рассыпание врага), рамки HUD и эмблемы профиля. Сеты героев — после конвейера выше.

### T13.31 — Подбор по кнопке, экран сборки, стрелы, косметические эффекты и бесплатная косметика (владелец 2026-09-07, семь пунктов) ✅ (2026-09-07)
- **Подбор добычи по кнопке.** Сундук и предмет на земле только помечаются как «рядом» (`sim.nearLoot`), HUD показывает подсказку «Подобрать · G» (кнопка работает и тачем); подбор — `PICKUP_ACT` через `step()`, чтобы попадал в input-лог и реплей. Бот и тесты жмут сами. «Оставить» на экране добычи кладёт предмет к ногам (лежит `lootLifetime`), а не удаляет.
- **Экран сборки** (`sim.buildOpen`, Tab/I, кнопка «Сборка» в HUD) — как карта прокачки в Death Must Die: экипировка по слотам, сумка с «надеть»/«выбросить» (выброшенное ложится на землю и уходит из «подобранного за забег»), взятые умения с рангами, легендарки, таланты, предметы лавки, нейтралка. Мир стоит, как в лавке. С полной сумкой экран добычи показывает её список с выбросом — раньше при 12/12 оставался только «Leave».
- **Стрелы.** `drawHeroProjectile`: древко в 9 арт-пикселей, наконечник и оперение; клинок и пуля перерисованы. Пять точек в кучке читались белым кружочком.
- **Косметические эффекты** (`features/arcade/effects.ts`, общие для боя и превью гардероба): слот «эффект под ногами» (кольцо углей, ледяное, золотое Aegis, пустотный вихрь — id прежних «рамок» сохранены ради сейвов), новый слот «свечение героя» (пламя, иней, молнии, сияние Aegis с нимбом), следы и эффект смерти врагов перерисованы. Клавиша **T** — вспышка на секунду. Превью гардероба рисует всё надетое.
- **Косметика бесплатна** во всех сборках (`SHARD_PRICE` = 0, полные цены сохранены в `SHARD_PRICE_FULL`). Lotus → иконка Lotus Orb.
- **Обрезанные кадры.** Метрика «срез» в `qa_sheet_health.mts` показала 60+ рядов, где оружие/плащ упираются в край кадра (арбалет арканы Drow в ударе, пила Timbersaw, шляпа Muerta). Герои переведены на кадр 160 с запасом 1.4 (`--margin`), размер героя на экране прежний (world вырос вместе с кадром). Листы весят на ~50% больше.
- **Цифры прокачки (владелец 2026-09-07: «непонятно, как растёт урон»).** `upgradeFigures` в content/schools.ts зеркалит формулы sim.ts: карточка школы показывает «сейчас → после» (урон ауры 8 → 10.8, шанс молнии 33% → 44%…), новое значение подсвечено; лавка и нейтралка показывают итог статов «172 → 189»; экран сборки — панель характеристик (здоровье, регенерация, броня, урон, ударов/с, крит, вампиризм, перезарядка, скорость, радиус сбора, золото, опыт). Тест `arcadeFigures.test.ts` держит формулы в согласии с симом.
- **Свечение по контуру (владелец: «эффект должен окружать персонажа, как гем Terrorblade / горящий Undying», «корона не на месте»).** `frameGeometry` в sprites.ts считает рамку и контур силуэта кадра (кэш на кадр); свечение рисуется в два слоя — ореол по контуру и пламя/пар/пыльца за спиной до спрайта, искры/кристаллы на контуре после; нимб Aegis сидит на макушке силуэта. То же в превью гардероба.
- ⬜ Кнопка подбора и сборки на тач-панели — сейчас подсказка подбора нажимается, сборка — кнопкой рядом с паузой; отдельной кнопки вспышки (T) на таче нет.

### T13.30 — Визуальная каша листов и самоцветы «как в Dota» (владелец 2026-09-07: семь скриншотов — Drow без рук, Muerta дёргается, PA/TB чёрные, Razor серый, QoP белая; «гемы сделал не то») ✅ конвейер + перерендер (2026-09-07)
- **Метод.** Прошлые «исправлено» смотрели один ряд `idle` под одним углом. Теперь `scripts/qa_sheet_health.mts` меряет все кадры × все 8 направлений каждого листа: строб (чередование двух поз), рывок центроида, провал площади силуэта, куски, яркость/насыщенность стойки, пустые кадры — и ранжирует худших по каждому признаку. Каждый дефект со скриншотов нашёл в нём число и системный корень.
- **Корни (render_dota_sprites.py / dota_pipeline.sh).** (1) Клипы для рядов брались из ВСЕХ glb, включая части: у плаща арканы Drow `cloth_IDLE`/`cloth_DEATH` выигрывали как более короткие имена — тело в bind-позе, «размазня»; теперь только клипы основной модели, и аркана Drow вернулась на своё тело `drow_arcana.vmdl_c` (T13.28 закрыт). (2) Длинные клипы бега (у Muerta `run` 7 с) сэмплировались 12 кадрами на весь клип — шаг 0.58 с при цикле 0.5 с, чередование двух поз; период цикла ищется автокорреляцией позы, берётся один период. (3) `drop_dupes` выкидывал симметричные части (наручи, ноги) как «дубли» по числу вершин и габариту — сравнение теперь по геометрии. (4) Root motion гасится по несущей кости (самая тяжёлая по весу вершин кость туловища) относительно позы кадрирования, без порогов: корень у Vengeful Spirit и персоны Mirana стоит на месте (провал бега 99%), пороги оставляли Meepo/персону DK с пустым кастом; служебные клипы (`mvp_screen`, `cast_dig`) в чёрном списке. (5) Тёмные текстуры со светлыми деталями (PA, TB, плащ Drow) автоэкспозиция пропускала целиком — мягкая гамма к 0.3; узор свечения из альфы цвета (`--glow-from-alpha`, PA) и из масок selfillum (`--glow-mask`, грудь TB). (6) Ремодель арканы QoP ссылается на `materials/models/heroes_staging/…`, которых в vpk нет — меш без материалов, серый; `--fix-tex` + `--mat-map` по примитивам, у `~style1` не было `--style`.
- **Самоцветы.** Поворот тона всего листа красил кожу и одежду. Теперь `gemSheet` (`features/arcade/sprites.ts`): акцентные пиксели (яркие, насыщенные, в тоне доминирующего кластера гистограммы) получают тон самоцвета, вокруг — ореол; облик без свечения (Razor) самоцветов не показывает, как и Dota не даёт гнёзд предметам без эффектов. Пять цветов призматических самоцветов Dota (Golden, Emerald, Cerulean, Brilliant Purple, Ruby). `items_game.txt` гнёзд у аркан не задаёт (они ставятся инструментами) — источника «у кого есть гемы» в клиенте нет, поэтому критерий — свечение на листе.
- **Перерендер.** Все 293 листа `dota_px2` и `dota_px` перерендерены обновлённым конвейером (3 чанка параллельно, `REUSE=1`), сравнение до/после — тем же `qa_sheet_health.mts`.
- ⬜ Ряд `death` у части героев (Drow, Sniper, Invoker, Monkey King, …) ложится ниже кадра: кадрирование теперь честно по позе бега, без люфта bind-позы, и труп, лежащий к камере, режется снизу (в замере — «провал смерти»). Лечить не якорем (пробовали: туловище тянет труп за кадр, без гашения тело уезжает своим root motion), а кадрированием: учитывать последний кадр смерти при расчёте охвата или сдвигать центр вниз только для ряда смерти. Прыжковые касты (Monkey King, Meepo, Winter Wyvern) вылетают за верх кадра — то же. Смотреть по ранжированию `qa_sheet_health.mts`, а не по одному листу.

### T13.29 — Писк вместо звука вихря у Juggernaut (владелец 2026-09-06) ✅ шов петли (2026-09-12)
- **Что сообщено.** «Во время bladefury джаггернаута есть какой-то непонятный писк вместо нормального звука раскрутки»; уточнение: «первый звук раскрутки есть, а потом уже идёт этот писк».
- **Что проверено и ИСКЛЮЧЕНО** (2026-09-06): клип петли цел — `juggernaut/spin_loop.m4a` 1.118 с, офлайн-рендер трёх оборотов с `loop = true` даёт на стыке −0.0059 → 0, щелчка нет; в клипе нет узкого тона — энергия широкополосная с пиком на 125 Гц, как у соседних клипов удара; поток попаданий ни при чём — вихрь бьёт по площади каждые 6 тиков через `damageEnemy(..., "spin")`, но `events.hits` при этом не растёт (замер: 240 тиков вихря, 0 приростов), так что звук ударов в вихре не играется вовсе; синтетика тоже под гейтами (`ARCADE_MIN_GAP_MS`: hit 70 мс, kill 90 мс).
- **Почему не пойман.** В headless-прогоне (70 с, автовыбор первого оффера) вихрь так и не включился: без игрового ввода герой стоит и Q не срабатывает. Все три пойманные зациклённые источника оказались синтетическим шумом (буфер ровно 1.000 с), а не `spin_loop` (1.118 с) и не `radianceLoop` (4.35 с).
- **Что сузит поиск за минуту.** Открыть забег с `?sfxdebug=1`, сделать вихрь и посмотреть в панели последние 12 записей журнала: там видно url клипа и статус (`played` / `pending` / `failed` / `suspended`). Если в момент писка идёт поток одного и того же url — это наложение; если `failed` — сэмпл не декодировался и играет синтетический фолбэк.
- **Воспроизведено headless и исправлено (владелец 2026-09-12: «первый раз звук есть, дальше писк до конца крутилки»).** Playwright с перехватом `AudioBufferSourceNode.start`/`OscillatorNode.start`: за 4 с вихря стартует ровно один зацикленный источник (`spin_loop.m4a`, 1.118 с), один пинг каста (triangle 440→880, 90 мс) и `spin_stop` в конце — наложения и синтетического фолбэка нет. Значит писк — сам оборот петли: `loop = true` на сыром декодированном AAC крутит и края клипа (priming/padding кодека), первый проход чистый, каждый следующий начинается со шва. Фикс в `ui/sound.ts`: `makeLoopReady` — обрезка тишины по краям до восходящих нулевых переходов и равномощный кроссфейд хвоста в голову (40 мс), голова, уже прозвучавшая в хвосте, отбрасывается; буфер петли кэшируется на декодированный сэмпл. `sfxLoop` теперь пишет в журнал `?sfxdebug=1` и не стартует на приостановленном контексте. Тест `sound.test.ts`: тишина срезана, скачок на шве < 0.03 при сыром 0.47, энергия сохранена. Слух владельца — финальная проверка.

### T13.28 — Аркана Drow: лист собирается на базовом теле (владелец 2026-09-06: «скин drow, который был с арбалетом, нужно вернуть») ✅ закрыт T13.30 (2026-09-07): причина была в клипах частей (`cloth_*`), не в модели — лист снова на теле `drow_arcana.vmdl_c`
- **Что не так.** Косметика арканы (`models/items/drow/drow_arcana/drow_arcana_{arms,back,head,legs,quiver,shoulder,weapon}`) деформируется мимо тела `drow_arcana.vmdl_c`: плащ вырастает примерно вдвое, причёска расходится веером, лук висит отдельно. На листе это месиво, которое владелец и увидел в гардеробе.
- **Что уже исключено** (2026-09-06, коммит f977e879): не выбор glb — конвейер брал соседний `drow_base.glb` вместо `drow_arcana.glb`, починено; не дубли мешей — CLI выгружал по три совпадающих копии (`._dummy`, базовый, `_style1`), схлопнуты; не способ пришивания — Copy Transforms по костям и пересадка мешей на скелет тела дают побайтово одинаковый кадр; не выбор клипа — берётся `drow_arc_idle`, и на теле без косметики поза правильная; не rest-поза — расхождение частей и тела 0.000.
- **Куда копать дальше.** У этой модели клипы названы `drow_arc_*`, а не `idle`/`run`/`attack`/`death`, и в скелете 251 кость против ~100 у базового тела — то есть косметические кости у неё анимируются, а не висят на родителе, как у обычных плащей. Проверить, что несут ключи костей `drow_arcana_cape_*` / `ponytail_*` / `weapon_*` в `drow_arc_idle`, и при необходимости морозить их в rest.
- **Как сейчас.** Облик ВКЛЮЧЁН и собирается на базовом теле `drow_base` с косметикой арканы поверх — фигура стоит прямо, плащ и арбалет на месте. Раньше это выходило случайно (конвейер брал первый попавшийся glb), теперь зафиксировано в обоих манифестах явной строкой и комментарием. С «правильным» телом `drow_arcana.vmdl_c` лист выходит размазнёй, поэтому чинить выбор модели обратно нельзя без перепроверки картинки.
...
- **Превью облика тянулось дробным масштабом (2026-09-06, a0.27.0).** Стенд гардероба 230 css при листе 128 арт-пикселей — это 1.8, и nearest-neighbour растягивал часть пикселей вдвое, часть нет: облик выглядел кашей. `previewScale` держит целое число ФИЗИЧЕСКИХ пикселей на арт-пиксель при увеличении (округление вверх по обычному правилу — у кадра есть поля вокруг силуэта, лишние 10–15% срезают пустоту, а не героя) и оставляет дробный масштаб при уменьшении: миниатюрам 48 px целых чисел не бывает вовсе, и на целом они показывали кроп в упор.
- **Самоцветы аркан** (владелец: «в арканы вставляются гемы, аркана может быть любого цвета»). Первая версия — поворот тона готового листа (`hueSheet`) — красила и кожу, и одежду; заменена на перекраску свечения `gemSheet`, см. T13.30. Было: поворот тона листа (кэш на пару «лист + градусы»), поэтому доступен любой арканe без перерендера. Пять самоцветов у каждой арканы; превью в гардеробе показывает настоящий результат.
...
- **Два облика были не в масштабе — починены (2026-09-06).** `snapfire@snailfire` (×2.23): панцирь улитки раздувал габарит модели, часть убрана из строки манифеста — стало ×0.98. `ancient_apparition@frost_djin` (×0.18): две причины сразу. Первая — автоподбор клипов взял вырожденные `aa_idle`/`aa_attack` (силуэт 24 px кадра вместо 110, при том что `ancient_run` и `aa_death` рисовались нормально); клипы запинены в `--anims`. Вторая — набор шире базовой модели (охват камеры 6.65 против 4.90), поэтому герой в кадре выходил на четверть мельче; `world` в манифесте поднят со 128 до 174 пропорционально охвату. Итог ×1.25 в px2 и ×0.91 в px.
- **`qa_sprite_sheets.mts` научился учитывать `world`.** Он сравнивал число непрозрачных пикселей в кадре, а на экране размер решает `world / frame`: облик с более широким набором честно выглядел «мелким» для проверки. Теперь сравнение идёт по площади на экране.
- **Лечащая зона перестала быть «шариком» (2026-09-06, a0.25.0).** Тот же дефект, на который владелец указал у варда Juggernaut, только у остальных: Shadow Wave, Purification, Purifying Flames, Cold Embrace, Voodoo Restoration, Tether, Nature's Attendants и Hand of God рисовались кружком радиусом 7 px на полу. В Dota это не тотем, а свечение вокруг цели — теперь `drawHealAura` рисует всплывающую пыльцу по радиусу (плотность от радиуса, цвет `--arcade-heal`). Бьющий вард (`damage_ward`) без листа пока остаётся кружком: ему нужна своя модель, см. ниже.
- **Настоящие модели вардов и призывов (2026-09-06, a0.28.0).** Отрендерены десять новых листов из моделей Dota: Nether Ward (Pugna), Plague Ward ×2 (Venomancer), Death Ward (Witch Doctor), Mass Serpent Ward ×3 (Shadow Shaman), Psionic Trap ×2 (TA), Proximity Mine (Techies), Tombstone (Undying), паучки ×3 (Broodmother), жуки ×3 (Weaver), Голем (Warlock). Rain of Chaos заодно перестал быть падающей зоной и стал тем, чем является в Dota — призванным големом (`damage_ward` + `summon`), что попутно снимает верхний край: Warlock мерился 60% при полосе 30–50%. Заглушка-тотем осталась для тех, у кого модели в vpk нет. **Грабли:** в `--anims` нельзя писать `attack=attack@0.1-0.75|idle` — парсер режет по `@` раньше, чем по `|`, и падает на `0.75|idle`; у `witchdoctor_ward` и `weaver_beetle` в vpk вообще нет клипов (ряд один, `drawDotaFrame` сам падает на idle).
- **Бьющий вард получил пиксельный тотем-заглушку (2026-09-06, a0.26.0).** Пока нет настоящих моделей (блокер ниже), Nether Ward, Plague Ward, Death Ward, Mass Serpent Ward, Psionic Trap, Tombstone и Spawn Spiderlings рисуются `drawWardTotem` — тень, каменное основание, древко, голова с пульсирующим оком, высотой примерно в 2/3 героя. Читается как поставленный объект, а не как «шарик на полу».
- 🟨 **Смешивание предметов по слотам** (надеть шлем из одного сета и оружие из другого). Пре-рендеренные листы этого не дают: комбинаций больше, чем листов. **Способ найден и проверен на срезе (2026-09-06).**
  - **Holdout не подходит:** мы рендерим Workbench'ем (так сделан весь пиксельный конвейер), а holdout — свойство Cycles/EEVEE. Вместо него — **разница двух листов**: «герой без части» и «герой с частью». Разница даёт ровно видимые пиксели части, уже с правильным перекрытием телом, и не требует менять движок рендера. Инструмент: `web/scripts/sprite_part_layer.mts` (Playwright-canvas, как остальные наши растровые тулзы).
  - **Проверено:** Juggernaut, часть `jugg_sword`. Слой вышел на 3.3% площади листа и содержит именно катану; по краям остаются редкие одиночные пиксели — это разница затенения тела от присутствия части, лечится допуском (`tol`, по умолчанию 12) или отсевом одиночек.
  - ⬜ **Что осталось и чего это стоит.** Рантайм-композит (рисовать базовый лист, потом слои частей по порядку) — небольшой кусок в `renderer.ts`. Основная цена — рендер: нужен «голый» лист на каждого героя плюс лист на каждую часть каждого сета, это примерно 10 частей на героя → **~1400 рендеров ≈ 60+ часов Blender и ~130 МБ** сверх нынешних 279 МБ. Это решение владельца: делать сразу всех, делать по нескольким любимым героям или не делать вовсе.
...
- **Анимации умений ✅ (2026-09-06).** Ряд `cast` есть во всех 283 листах (`pick_action` падает на любой неслужебный клип — Valve называет клипы каста по способности, `nevermore_Requiem`), рендерер проигрывает его 420 мс по росту счётчика `events.casts`/`ults` (`castUntil`/`castFrom` в renderer.ts). Пункт был просрочен в статусе: перерендер и проигрывание сделаны в ту же сессию.
...
### T13.16 — Звук героев: последние пять дыр закрыты (2026-09-06)
- **Пять героев молчали, и причина была не в Dota, а в именах папок Valve.** Удары: у Keeper of the Light они лежат в `sounds/weapons/hero/keeper`, а скрипт искал `keeper_of_the_light`; у Lifestealer — в `lifestealer`, хотя озвучка у него, наоборот, в `life_stealer`. Реплики: Phoenix, Marci и Io в списке `dota_voice.sh` отсутствовали вовсе, потому что определитель базового префикса ищет файлы вида `*_move_01`, а у них номера в именах нет совсем (`marci_move`, `phoenix_bird_attack`), у Io же имена вообще без префикса (`attack`, `move`). Скрипты поправлены: номер в имени стал необязательным, категория может стоять в начале имени, три героя добавлены в список.
- **Ещё десять героев были без звуков умений** — та же природа. Трёх (Keeper of the Light, Underlord, Vengeful Spirit) вообще не было в таблице `dota_sfx_pack.py`, у семи имена звуков в таблице не совпадали с файлами Valve: у Phantom Lancer `lance_cast` вместо `spirit_lance_cast`, у Chen `persuasion_cast` вместо `holy_persuasion_cast`, у AA `coldfeet_cast`/`iceblast_cast` вместо `cold_feet`/`iceblast_release`, у Monkey King `boundless_strike` вместо `strike_cast01`, и так далее — скрипт молча пропускал отсутствующий файл. Теперь звуки умений есть у всех 126, а пак вырос с 522 до 559 ссылок.
- **Два вида врагов тоже молчали:** Торментор и Древний. У Торментора в vpk нет ни модели, ни ударных звуков — есть только его «отражение» в `sounds/ambient/tormentor_reflect0N`, а это ровно то, что слышно, когда по нему бьют. Древний — постройка, поэтому удары по нему взяты из `physics/damage/building/`, а падение — звук разрушения Древнего.
- **Теперь 126 из 126** и с ударами, и с репликами; `test/sound.test.ts` держит это и заодно проверяет, что ни один индекс не ссылается на несуществующий файл. Marci ожидаемо получила только `move` — она в Dota не говорит, а хмыкает.
...
- **Баг переноса опыта (2026-09-11, найден ботом: уровень 1224, xp 1e24 на placecal-23).** Остаток опыта после уровня (`carry` в `applyOffer`) шёл в `gainXp` и умножался заново на жадность/xpMult — при большом запасе экспонента. Теперь остаток переносится без множителей (`gainXp(carry, true)`); тест в `arcadeLevelupActions.test.ts` — потрачено ровно столько, сколько дали. Бот получил `--only N` и трассу скачков уровня.
- **Перезамер аутсайдеров 2026-09-11 (a0.58.0, 30 прогонов, база `sweep`, разминка Herald):** Arc Warden 23.3%, Medusa 20.0%, Ringmaster 23.3% — после срезов a0.40–a0.58 (яд, места, апгрейды arcana, особенности) все трое в полосе; при этом Juggernaut на той же базе 6.7% против 29.2% на базе `traitcal2` (n=96) — разброс между базами сидов больше, чем между героями, так что хвост «Arc Warden/Medusa/Ringmaster» закрыт, а любой новый вердикт по герою требует двух баз по 30+.
...
- **Крайности свипа, партия 2 (2026-09-06, a0.30.0).** Замеры по 20 прогонов на двух базах:
  - **Rubick 5% → 50.0 / 25.0%.** Telekinesis был одноцелевым, хотя в Dota роняет цель и задевает всех вокруг места падения — стал вспышкой.
  - **Meepo 5% → 20.0 / 35.0%.** Divided We Stand — это несколько Meepo, которые бьют, а сим считает один источник: его усилили. **Промежуточная версия улетела на 50/60%** (усиление ульта И прибавка к атаке разом), статы вернули назад — в таблице осталось только усиление ульта.
  - **Ancient Apparition 70% → 45.0 / 60.0%.** Это мой же побочный эффект: `mass_freeze` научился бить в a0.21.0, а у Ice Blast в таблице уже лежало 260–520 при радиусе 460 — самый широкий ульт в игре, и он молча ожил. Урезали дважды: радиус 460 → 320, перезарядка 55 → 65, урон −25%, «жажда» 0.32/600 → 0.2/460.
  - ⬜ **Arc Warden 5% → 10.0 / 15.0% — три попытки не сдвинули.** Прибавка к статам (дальность 330 → 400, интервал 0.8, урон 31) дала +5 п.п.; замена Magnetic Field на скорость атаки не дала ничего и была откачена — она отнимала у него ЕДИНСТВЕННУЮ защиту (+25 брони на 5 с). Пробой сима: умения кастуются (16 Q и 11 E за забег) и убивают 223 врага против 194 у Rubick, то есть дело не в мёртвых слотах и не в боте. Копать дальше: почему при таком же числе убийств он не доживает.
- **Второй слот, который бот качает всегда, — R, и у 21 героя он не давал урона (2026-09-06, a0.33.0).** Механическая сверка «что стоит в R» и свип совпали почти один в один: внизу таблицы стоят именно они — Rubick 5%, Arc Warden 5%, Dazzle 10%, Tinker 10%, Grimstroke 10%, Keeper of the Light 10%, Ogre Magi 15%. У четверых ульт был `mass_freeze` с нулевым значением, то есть чистый контроль; после того как вид научился бить (a0.21.0), достаточно проставить число:
  - **Grimstroke 10% → 25.0%**, **Keeper of the Light 10% → 25.0%** — обоим Soulbind и Will-O-Wisp дали 220–440.
  - **Medusa 20% → 20.0%** и **Ringmaster 20% → 20.0%** — не сдвинулось. Значит у них дело не в ульте; урон оставлен (ульт, который ничего не делает, — дефект сам по себе), но обоих оставляю кандидатами.
  - Не трогал Chronosphere (Faceless Void 25%), Song of the Siren (Naga 25%), Overgrowth (Treant 30%) и Global Silence (Silencer 55%): они в Dota тоже без урона, а по свипу не внизу — то есть чистый контроль им не мешает.
- **Storm Spirit 15% → 20.0 / 25.0% (a0.34.0).** У него был самый маленький запас HP в игре (470) при зачистке «одна мина за каст»: 540 HP и усиленный ульт (150–320, перезарядка 26 → 22) — ульт бот качает всегда, поэтому только через него его и можно было усилить измеримо.
- ⬜ **Grimstroke, Keeper of the Light, Dazzle, Medusa, Ringmaster, Storm Spirit, Tinker, Ogre Magi** — остаются ниже полосы после всех правок Q и R. У большинства из них оставшиеся слоты (W и E) — щиты и пассивные ауры, а их бот не качает вовсе, поэтому и померить их вклад нечем: сначала политика бота, потом эти герои.
- **Правило «Q несёт зачистку» разошлось по шести героям (2026-09-06, a0.32.0).** Механическая проверка «какой вид умения стоит в Q у каждого героя» дала 50 кандидатов; пересечение с теми, кто в свипе ниже полосы, дало шесть — и каждому вернули зачистку в Q по мотивам Dota. Замер: 20 прогонов на одной базе `sweep2` (направление, а не точное число).
  - **Bane 10% → 50.0%** (уровень p50 30): Enfeeble из точечного дота стал волной слабости вокруг.
  - **Skywrath Mage 10% → 45.0%**: Arcane Bolt с перезарядкой 3 с бил одну цель — теперь прыгает по 3–5, как и положено магу.
  - **Slardar 10% → 35.0%**: в Q стоял разгон без урона, а Slithereen Crush — в W; поменяли местами.
  - **Bristleback 20% → 55.0%**: Quill Spray стоял в W, а в Q — Nasal Goo с самыми слабыми числами в игре (40–100); поменяли местами.
  - **Shadow Demon 10% → 30.0%**: Shadow Poison стоял в E, Disruption в Q; поменяли местами.
  - **Dazzle 10% → 20.0%**: Poison Touch стал конусом (в Dota он и бьёт конусом), но герой всё ещё ниже полосы — у него W это щит, а R пассивная аура, так что работают только Q и лечение. ⬜ Остаётся кандидатом.
- **Верх свипа: три нерфа из пяти сработали (2026-09-06, a0.35.0).** Верхние в таблице — это танки с сильной зачисткой в Q, зеркало нижних. Резал по одному параметру у каждого, замер 20 прогонов на базе `sweep2`:
  - **Timbersaw 70% → 60.0%** (Reactive Armor 4–10 → 3–7, `tough` 3 → 2), **Elder Titan 70% → 55.0%** (Echo Stomp 11 → 14 с), **Mars 65% → 55.0%** (Spear 130–320 → 110–270 и перезарядка 7 → 8, `tough` 7 → 5). Все трое сошли с явного верха к верхнему краю полосы.
  - **Phoenix 70% → 65.0%** (Supernova 120 → 150 с и 50–90% → 40–70% HP). Сдвиг в пределах шума, но правку оставил: возрождение раз в две минуты с почти полным HP — сильнейший инструмент выживания в игре, и направление верное.
  - **Kez 65% → 70%: правку откатил.** Диагноз «двойной крит» (пассивка в E плюс фирменный крит) не подтвердился — после урезки число ушло ВВЕРХ. Значит его держат статы: 760 HP, броня 5, скорость 180 и интервал атаки 0.85 — лучший набор среди ближников. ⬜ Кандидат, но лечить надо статы, а не крит.
  - **Замечание к методу:** на этом уровне (5–10 п.п. при шуме ±11%) одиночный замер на одной базе уже не различает правки. Дальше либо 60+ прогонов на 2–3 базах на каждого героя, либо сначала чинить политику бота — иначе это подгонка под шум.
- ⬜ **Полный акт после дня правок: медиана выше своей полосы, но разброс важнее (замер 2026-09-06, 20 прогонов, база `sweep2`).** Целевая полоса полного акта — 15–25%, замерено: Slardar 5% · Axe 25% · Juggernaut 30% · Rubick 40% · Sniper 45% · Bane 45%. Медиана 35%, то есть акт стал легче нормы, **но разброс между героями (5–45%) шире самой полосы** — и это проблема крупнее. Глобальный рычаг (HP Древнего 9000, `full.hpMult` — у полного акта его вообще нет, в отличие от `dire` 1.15 и `river` 1.3) я трогать не стал: он опустит и без того нижних (Slardar с 5% ушёл бы в ноль), а верхних не приблизит к нижним. Сначала сузить разброс на коротком акте, потом мерить полный. Замечание к методу: полный акт считается в 5–10 раз дольше короткого (20 минут игрового времени на прогон), так что сплошной свип по нему нереален — только выборка из 5–6 героев по краям и середине.
- **Обход слепоты бота: проба вклада умения (2026-09-06, a0.36.0).** Полноценный второй бот не получился — наивная политика движения умирает на 4-м уровне (проверено двумя вариантами), а хорошая политика это и есть тот бот, чей файл я не трогаю. Зато вопрос «работает ли слот вообще» решается без бота: `web/scripts/sim_kit.ts` гоняет один и тот же сид с одним и тем же вводом (стоим на месте, автокаст включён) и меняет ТОЛЬКО уровень проверяемого слота — 0 против 4. Сравниваются убийства за 90 секунд. Абсолютные проценты с полосой 30–50% не сравнимы, но вклад слота виден сразу.
- ⚠️ **Поправка к трём предыдущим пунктам: у пробы есть шумовой порог, и я его сначала не измерил.** Перепроверка на ТРЁХ базах сидов по 20 прогонов: у Elder Titan один и тот же слот дал −13% / +7% / +2%. То есть **значения в пределах ±20% — шум**, и «отрицательный вклад» у Ogre Magi (−14%), Storm Spirit (−19%), Dazzle (+13%) и Invoker (−23%) был одиночным замером на одной базе, а не установленным дефектом. Так писать в BACKLOG и в сообщения коммитов было преждевременно.
  - **Что подтвердилось на трёх базах после правок:** Ogre Magi W **124 / 93 / 207%** — эффект огромный и настоящий, тут правка бесспорна. Invoker E **42 / 14 / 22%**, Storm Spirit W **7 / 22 / 14%**, Dazzle E **50 / −2 / 12%** — скромно, на грани порога; правки оставлены, потому что по существу верны (одноцелевой дот и захват одной цели в survivors-игре — плохой инструмент), но выдавать их за измеренные улучшения нельзя.
  - **Рабочее правило для пробы:** доверять только эффектам от ~50% и выше и обязательно на 2–3 базах сидов. Всё, что меньше, — повод посмотреть глазами на кит, а не основание для правки.
- **Сплошной аудит W и E по всем 126 героям пробой (2026-09-06, a0.37.0, 252 замера ≈ 2.5 часа).** Активных умений с уроном и вкладом ≤ 3% набралось 44. **Но у метрики есть насыщение, и без этой оговорки числа врут:** если герой и так выкашивает весь спавн (Elder Titan, Phoenix, Leshrac, Warlock), лишний урон не добавляет убийств, и слот выглядит «мёртвым», не будучи им. Действовать можно только там, где вклад ОТРИЦАТЕЛЬНЫЙ (каст стоит времени, а отдача не окупает) И герой ниже полосы. Такой нашёлся один: **Invoker (20% по свипу), E = метеор −23%** — узкая зона радиусом 130 с перезарядкой 10 с; расширил до 190 и ускорил до 8 с, стало **+20%**. Остальные заметные минусы (Dawnbreaker 35%, Morphling 35%, Outworld Destroyer 40%) — у героев В полосе, то есть насыщение, а не дефект.
- **Проверка бота подтвердила диагноз о нём самом.** После правок W/E у Invoker, Ogre Magi, Dazzle и Storm Spirit бот показывает РОВНО те же 20 / 15 / 20 / 20%: он эти слоты не качает и разницы не видит. Правки реальны (проба это показывает), но текущим калибратором не измеримы — это и есть главный аргумент за правку `pickOffer`.
- **Первый же прогон пробы по восьми героям ниже полосы нашёл ТРИ слота с ОТРИЦАТЕЛЬНЫМ вкладом** — прокачанное умение снижало число убийств, потому что каст стоит времени, а отдача одноцелевая:
  - **Ogre Magi W (Ignite) −14% → +107%:** точечный дот стал вспышкой вокруг. У Ogre вообще не было площади ни в одном слоте.
  - **Storm Spirit W (Electric Vortex) −19% → +4%:** одноцелевой захват стал вихрем по площади. Всё ещё почти нулевой вклад — герой остаётся кандидатом.
  - **Dazzle E (Shadow Wave) +13% → +73%:** в Dota волна бьёт врагов и лечит союзников, а у нас только лечила; `life_drain` делает ровно и то, и другое.
  - Для сравнения, здоровые слоты той же пробы: Keeper of the Light W +187%, Tinker W +159%, Ringmaster W +87%, Grimstroke W +77%, Io W +142%. То есть проба различает живое и мёртвое уверенно.
- ⚠️ **Важное про сам инструмент: бот почти не качает W и E (2026-09-06).** `pickOffer` в `scripts/sim_arcade.ts` оценивает предложения так: ульт 60, Q 40, W 30, E 20 — а любое улучшение школы 50–53. Значит из четырёх слотов бот стабильно качает только R (берёт всегда) и Q (он и так на первом уровне с начала забега), а W и E у него остаются на нуле почти весь забег. **Следствие: свип и все точечные замеры меряют героя через Q, R, пассивку и предметы, но почти не трогают W и E.** Это объясняет, почему все удачные правки сегодня пришлись на Q или R, и почему у Arc Warden замена умения в E не сдвинула НИ ОДНОГО числа (10.0/15.0 · уровень 12 · 617 убийств — до и после совпали до единицы). Правка политики бота — в файле другой сессии, поэтому не трогал; но пока она не сделана, ⬜ **балансу W/E верить нельзя**.
- **Диагноз Arc Warden (следствие того же).** Профиль против Rubick на тех же сидах: уровень p50 12 против 26, убийств 617 против 2525, смерть на 298 с против 540 с. То есть дело не в живучести, а в скорости зачистки: в Q у него стоял Flux — точечный дот с узким условием каста, и это было ЕДИНСТВЕННОЕ умение, которое бот качает. Поменял местами с искрой-цепью: Q — Spark Wraith, E — Flux. Слот Q должен нести основную зачистку у всех героев (так и есть у Drow, CK, Oracle, Mirana, Rubick, NP, Hoodwink, Clinkz) — Arc Warden был исключением. **Итог: 10.0/15.0% → 30.0/25.0%,** уровень 12 → 18, убийств 617 → 1184.
- **Полный свип 126 героев на базе `sweep2` (2026-09-06, 20 прогонов):** 60 в полосе 30–50%, 46 ниже, 19 выше, один таймаут (Razor — прогоны упираются в `MAX_TICKS`). **Снимок смешанный:** героев мерили по мере того, как правки уезжали в код, поэтому у Windranger, Clinkz, Lina, Zeus и Warlock здесь ещё старые числа; их собственные замеры — выше. Ценность снимка — в нетронутых героях.
- **Два героя с НУЛЁМ побед на 20 прогонах (2026-09-06, a0.29.0)** — это уже не шум:
  - **Nature's Prophet 0% → 30.0 / 20.0%.** Из четырёх слотов урон был только в энтах и в ульте раз в 50 с (Sprout и Teleportation — без урона вовсе) при 560 HP. Энты стали втроём и бьют сильнее (cd 16 → 14), Wrath of Nature 50 → 36 с и на цель больше, Sprout теперь и бьёт: деревья вокруг цели — это `mass_freeze` с ненулевым значением.
  - **Hoodwink 0% → 36.7 / 20.0%.** Bushwhack был одноцелевым, хотя в Dota стягивает и оглушает область — стал вспышкой. Sharpshooter был добиванием одной цели раз в 45 с, хотя в Dota это сквозной выстрел — стал линией из пяти зон с перезарядкой 26 с.
- **Партия аутсайдеров по свипу (2026-09-06, правки уехали вместе с коммитом 9e179d6c — в его сообщении их нет, поэтому фиксирую здесь).** Свип всех 126 на базе `sweep2` показал новые крайности; каждая правка проверена на двух базах по 30 прогонов:
  - **Clinkz 10% → 30.0 / 26.7%.** У него все четыре слота были бафами — активного урона ноль при 470 HP (Omniknight с тем же нулём держит 55%, но он танк). Strafe заменён на Burning Barrage (`multishot`) — залп горящих стрел, как в Dota.
  - **Mirana 15% → 33.3 / 40.0%.** Starstorm был самой слабой новой в игре (50–125, радиус 200) и единственной зоной в ките — стал 70–180 при радиусе 280.
  - **Lion 15% → 43.3 / 13.3%, на повторе 50.0 / 15.0%.** Earth Spike был шириной 60 — самая узкая линия в игре при том, что в Dota это ряд шипов. Разброс между базами сидов у него шире всей полосы (35 п.п.), то есть его судьбу решает карта, а не кит: среднее 30%, но одиночному замеру у Lion верить нельзя.
  - **Windranger 5%, Lina 10%:** Powershot был шириной 55, Dragon Slave 60, Light Strike Array 95 — все три уже, чем у любого другого героя, хотя в Dota это сквозной выстрел, конус и круг. Расширены до 95 / 95 / 130.
  - **Zeus 70% → перезарядка цепи 2.2 → 3.2 с.** Цепь на 10 целей раз в две секунды — это пулемёт, и он один давал верхний край свипа.
- **Дольше живёт ≠ лучше играет.** Попытка добить Lina перезарядкой Dragon Slave 7 → 5.5 с и четвёртой зоной подняла не побед, а длительность: прогон вместо 1–2 с стал упираться в `MAX_TICKS` (30 прогонов считались 7 минут вместо 40 секунд), потому что герой выживает, но не успевает добить Древнего, а толпа копится. Правка откачена, расширение зоны оставлено. То же самое видно у Razor — его замер в свипе ушёл в таймаут. **Вывод для калибровки: время прогона — второй показатель рядом с процентом побед. Выросло время И процент стоит на месте — забег патовый, правку откатывать; выросло время, а процент поднялся (так вышло у Nature's Prophet: 0% → 30%) — герой просто стал доживать до Древнего.**
...
### T13.10 — Juice, звук, тач/TMA ✅ срез (2026-09-05) · ✅ музыка-луп (2026-09-06) · ⬜ проверка в реальном TMA
- **Крит как в Dota (владелец 2026-09-12).** Цифра крита крупнее (20 px, в первые кадры «выпрыгивает» до 28) и красная (`--arcade-crit-text`), поднимается выше обычной; звук — отдельный удар Dota `sounds/weapons/crit1-4` (`shared/crit_N.m4a`, 4 клипа по кругу) поверх обычного попадания, играет до троттла ударов, синтетический «крит» остаётся только у героев без клипов; «спаттер» PA из пака больше не играет (два звука на один крит). Заодно выяснилось, что все цифры урона до этого рисовались 10px sans-serif — см. T13.66. **Владелец: «критуем постоянно»** — это была Drow: Marksmanship (пассивная прибавка за дистанцию) красился в крит на каждый выстрел, отсюда и «звук не поменялся» (он звучал на каждом попадании). Теперь прибавка остаётся, но критом не считается — красная цифра и звук только у настоящих критов (Daedalus, Blade Dance, Time Lock). **Владелец: «звук крита тот же, что у выстрела»** — клипы `weapons/crit1-4` в vpk лежат на −25 dB RMS и длятся 2,6 с, под ударом их не слышно; нормализованы к −1 dB, обрезаны до 1,1 с, играют с gain 1.0, а сам удар при крите приглушён (0.4); предзагружаются на старте забега — иначе первый крит молчал (`sfxSample` на незагруженный буфер только ставит загрузку).
- **Смена боевых тем без провала (2026-09-06).** Три темы шли по кругу, но следующая заводилась по событию `ended` — то есть кроссфейд начинался уже на тишине и между треками провисала пауза в полторы секунды. Теперь следующая тема заводится за `FADE_MS` до конца текущей (проверка в `ensureMusic`, который и так зовётся каждый кадр), а `ended` остался запасным путём. Тесты `test/arcadeMusic.test.ts` на подменённом `Audio`. **Больше боевых тем в vpk нет:** `battle_0N_1` и `battle_0N_2` — это ссылки на те же три mp3 (проверено распаковкой), а `laning_*` и `ganked_*` — другой тон, их добавлять только по решению владельца.
...
### T13.37 — Постоянные открытия Arcade вне последних 50 забегов ✅ (2026-09-10) P1 (аудит 2026-09-08)

- **Проблема:** `HISTORY_CAP = 50`, а `hasActVictory`, `maxUnlockedRank` и lifetime-трофеи вычисляются только из этого массива. Победа `full/rank:7` открывает ступень 8 и Dire; после 50 новых поражений и обрезки истории функции возвращают 0/false. Воспроизведено на чистых экспортированных функциях, не по впечатлению от общего dev-сервера.
- **Файлы:** `web/src/state/arcadeStore.ts`, существующий persistence, `web/test/arcadeStore.test.ts`; потребители трофеев/открытий в Штабе и настройке Arcade.
- **Решение:** хранить достигнутые акты/ранг и накопительные показатели отдельно от ограниченной ленты; мигрировать доступную историю монотонно. Не выдумывать уже потерянные победы.
- **DoD:** открытия и lifetime-трофеи сохраняются после 50+ новых результатов и перезагрузки; повтор финализации не начисляет второй результат; миграция старого профиля идемпотентна; разминка и просмотр реплея не открывают ранги.
- **Deps:** существующий T13.5. Обязательная основа перед общей прокачкой/мастерством.
- **Evidence:** [аудит и repro](audits/2026-09-08-arcade-content-and-progression.md). В этой сессии изменена документация, код не исправлен.
- **Сделано (2026-09-10):** `state/arcadeStore.ts` — постоянный профиль `ArcadeProgress` (`aegis-draft.arcade.progress`): взятые акты, лучшая ступень, счётчики и per-hero трофеи копятся `recordProgress` на каждом `finish()` до обрезки ленты; `hasActVictory/maxUnlockedRank/arcadeTrophies` читают профиль, а не `history` (Setup, Штаб). Миграция: без ключа профиль один раз сворачивается из оставшейся ленты (`progressFromHistory`); битая запись — тоже из ленты, не пустой; запись будущей версии читается по известным полям. Уже вытесненные из 50 записей победы не восстанавливаются — их нет. Разминка и реплей профиль не открывают (реплей вообще не пишет). Повтор `finish()` отсекается статусом `over`. Тесты `arcadeStore.test.ts`: вытеснение 50 поражениями, round trip через перезагрузку модуля, миграция/идемпотентность, битая и будущая запись.

### T13.38 — Подсказки Arcade соответствуют текущему управлению ✅ (2026-09-10) P2 (аудит 2026-09-08)

- **Проблема:** вступление говорит «auto-cast is on», тогда как `AUTOCAST_OFF` включает только автоатаку, а Q/W/E/R выключает. На чистом профиле UI показывает пустые checkbox умений; ручной Q работает. Дефолты приняты владельцем в T13.26 и не подлежат смене ради соответствия устаревшему тексту.
- **Файлы:** `web/src/i18n/core.ts`, читатель подсказки в `features/arcade/ArcadeScreen.tsx`; источник истины — `state/arcadeStore.ts`.
- **DoD:** RU/EN отдельно объясняют включённую автоатаку, ручные умения и индивидуальный переключатель автокаста; проверка чистого профиля и профиля с сохранёнными настройками.
- **Deps:** T13.26; [evidence](audits/2026-09-08-arcade-content-and-progression.md).
- **Сделано (2026-09-10):** `arcade.controls` RU/EN переписан по фактической раскладке `input.ts`: умения — 1·2·3·4 (Q, E, R дублируют 1, 2, 4; W занят движением), F — атака; автоатака сама, умения по нажатию, переключатель «А» у кнопки. Дефолты `AUTOCAST_OFF` не менялись.

### T13.39 — Яд как самостоятельный статус со стаками ✅ (2026-09-10) P1 (этап 1 аудита 2026-09-08)

- **Было:** Poison Sting (Venomancer) и Poison Attack (Viper) шли через `searing → applyBurn`; Poison Nova / Nethertoxin / Poison Touch / Shadow Poison — общие `nova`/`line_burst` без яда. Зелёная перекраска не давала новой механики.
- **Сделано:** `ARCADE.poison = { maxStacks 5, seconds 4, tickEvery 12, tickShare 0.2 }`, `Enemy.poisonUntil/poisonStacks/poisonDps`, `ArcadeSim.applyPoison`: стак до потолка, ОБЩИЙ таймер обновляется каждым попаданием, dps стака — сильнейший активный источник, истёкший яд теряет стаки; `unstoppable` иммунны, `resistStatus` ранга укорачивает. Тик рядом с горением в `moveEnemies`, урон = dps × стаки × доля. Источники: kind `venom` (пассивка на удар: +урон и стак, dps = value × 0.5 — один стак равен прежнему горению, дальше сильнее) у Venomancer W и Viper Q; поле `AbilityDef.poison` (dps стака = value × poison) читают `line_burst/meteor`, `nova`, `goo` — Venomous Gale 0.1, Poison Nova 0.06, Nethertoxin 0.1, Poison Touch 0.1, Shadow Poison 0.12. Яд ≠ горение: без пепла, без Blast, без Пара. Рендер: `drawPoisoned` — пузыри вверх и капли вниз (форма, не только цвет) + пипсы стаков над макушкой; токены `--arcade-venom/-dark`, tint `pal.venom`. Тексты RU/EN у семи умений. Версия `a0.40.0`.
- **Проверки:** `arcadePoison.test.ts` (стаки/таймер/dps, тик по формуле, истечение, не-горение, иммунитет, источники, ранг), частицы; полный набор тестов; headless `sim:arcade --act short`: Venomancer 58→52% (n=24/48, шум), Viper 37.5→37.5% при 0.5 (при 0.35 падал до 21% — потому один стак приравнен к прежнему горению), Shadow Demon 16.7→18.8% (n=24/48; яд только добавляет урон и RNG не тратит, разница — шум). Живой прогон Playwright на 5273: кобольды с 5 и 2 стаками, пипсы читаются.
- **Дальше по аудиту:** вражеский яд вместе с очищением (пруд), школа Venom и гибриды (этап 3), источник последнего урона на итоге.

### T13.40 — Заражённый лагерь: первая цель карты ✅ (2026-09-10) P1 (этап 1 аудита 2026-09-08)

- **Что:** один лагерь на акт, по seed (`Rng("camp:<seed>:<act>")`, поток забега не сдвигается) на 720–1000 px от старта; центр и три места тотемов проверены на препятствия (`ObstacleGrid.blocked`), в River — не в реке и не в поводке ямы. Три **тотема порчи** — `EnemyKind corruption_totem` (`totem: true`, `unstoppable`, speed 0, dmg 0): стоят, не бьют, не толкаются, иммунны к статусам, сносятся ударами/умениями как обычный враг (арт — `ward_plague`). Лагерь спит, пока герой не войдёт в `wakeRadius` 200 или не ударит тотем/Сатира; разбуженный отпускает за `engageRadius` 460 (гистерезис, как агро нейтралов Dota). Пока разбужен и тотемы стоят, каждые 4 с прибывает **охрана** (сатиры; в Dire/River через одного hellbear) в кольце 190–260 от центра: 2 + 1 за снесённый тотем, HP ×(1 + 0.35 × снесённых), интервал делится на (1 + 0.25 × снесённых). Ушёл — охрана не прибывает; штрафа нет. Снёс три — `clearCamp`: `events.camps++`, тряска, нова; **награда** — три карты апгрейдов гарантированной редкости `exotic` через тот же `pending` (`pendingSource = "camp"`: свой заголовок, реролла нет, изгнание есть); если в момент очищения висит выбор уровня — награда ждёт своей очереди. Итог забега: `campsCleared`.
- **UI:** пунктирное кольцо порчи и лужи под тотемами (`drawCamp`), стрелка у края экрана с числом живых тотемов, когда лагерь вне кадра (`drawCampMarker`, экранные координаты после мирового прохода), чип HUD «Лагерь порчи · тотемы n/3» в зоне лагеря, строка «Лагерь порчи: очищен» на итоге, звук «elite» + hit-stop на очищение. Версия `a0.41.0`.
- **Проверки:** `arcadeCamp.test.ts` — размещение по seed/актам и детерминизм, тотем стоит/не бьёт/не берёт статусы/умирает, охрана только в зоне и усиливается, награда exotic без реролла и мир стоит, очередь после уровня, `campsCleared`, digest; полный набор; headless Juggernaut short без регрессии (бот к лагерю не ходит). Playwright на 5273: маркер «3» у края, кольцо, три варда, чип, два сатира; после сноса — оверлей «Camp cleansed» с тремя exotic картами.
- **Дальше по аудиту:** обзорная башня, пруд/порчи, Сатир-Осквернитель как чемпион лагеря, 5–7 мест и «не больше двух приглашений» когда мест станет больше одного; бот калибровки лагерь игнорирует — при добавлении маршрутов научить.

### T13.41 — Сатир-Осквернитель: чемпион лагеря с поведением ✅ (2026-09-10) P1 (этап 1 аудита 2026-09-08)

- **Что:** `EnemyKind satyr_defiler` (elite, HP 1500 × множители минуты, r 20, арт — лист сатира) стоит в центре лагеря с самого старта. **Спит**, пока герой вне `engageRadius`, и лечится 2%/с; в лагере гонит героя (138 px/с, если тот дальше 240). **Щит тотемов:** входящий урон × (1 − 0.25 × живых тотемов) — «без тотемов уязвим». **Порыв:** в 110 px — телеграф 1 с на позиции героя (тот же `slamT/slamX/slamY`, что у Рошана), взрыв r 96 на 64 урона + стан 0.45 с, затем 1.2 с стоит — окно для мили. **Полосы порчи:** пока герой в лагере, Сатир жив и тотемов ≥ 2 — раз в 5 с между двумя случайными живыми тотемами пунктир-телеграф 1.2 с, потом 1 с активная полоса шириной 34, бьёт стоящего на ней (dmg × 1.8 раз в 0.6 с). **Поводок:** ушёл дальше `camp.radius + 260` или герой вышел из лагеря — идёт домой и лечится: отступить можно всегда. **Контроль:** стан/заморозка не дольше 0.5 с, после — 3 с иммунитета к повторному (`Enemy.ccResistUntil`); замедление — вполовину. Лагерь очищается только когда сняты все тотемы **и** убит Сатир (в любом порядке, `tryClearCamp`); как элита роняет предмет и звучит «elite». Версия `a0.42.0`.
- **UI:** полоса босса в HUD «Сатир-Осквернитель · щит тотемов N%» пока герой в лагере; дуги щита вокруг сатира по числу тотемов (тусклее во сне); телеграф порыва — красный круг как у Рошана; полоса порчи — пунктир на телеграфе, широкая зелёная полоса с тёмным ядром в активной фазе.
- **Проверки:** `arcadeDefiler.test.ts` — сон/регенерация/погоня, щит по тотемам, порыв (телеграф на позиции героя, промах при уходе, попадание в круге, окно восстановления), полоса (телеграф без урона → активная бьёт, вне полосы нет, при одном тотеме полос нет), поводок и cc-cap/иммунитет, очищение только с обоими условиями и лут элиты, digest; `arcadeCamp` обновлён (очищение требует Сатира; тесты снимают ссылку `defiler` как killEnemy — пул переиспользует объект). Playwright на 5273: телеграф порыва, полоса, полоса босса с 75%.
- **Шишка:** `sim.defiler` — ссылка в пул врагов; тест, который «убивает» его через `alive = false`, обязан обнулить и ссылку, иначе переиспользованный кобольд становится «Осквернителем» и тянет полосы порчи. В коде ссылка снимается в `killEnemy` (как у Рошана).
- **Баланс (headless, Juggernaut short, seed `camp`, n=48):** без лагеря 27.1% побед; лагерь с пробуждением на 460 px — 14.6% (кайтящий бот влетал в Сатира на 1-й минуте: смерти на минуте 1 — 2→5). После гистерезиса агро (`wakeRadius` 200 / удар по лагерю, отпуск на 460) — 33.3% (смерти на минуте 1 снова 2), то есть спящий лагерь не хуже пустой карты. Бот в лагерь не ходит намеренно, так что это оценка «случайного контакта», не ценности награды.

- **Стоимость лагеря на Herald (владелец 2026-09-11, `a0.60.0`).** Диагностика ботом `--places camp --verbose` (T13.63): в 14 из 24 забегов тотемов 0/3, Сатир ни разу не задет, смерть от обычного леса — автоатака бьёт ближайшего охранника, а тотем (210 HP) умирает только от AoE и вплотную; кайт у кромки лагеря возвращал Сатира домой в полный HP; после третьего тотема охрана 5×каждые 4 с не давала подойти к нему. Три правила: **(1) цель места приоритетнее толпы** — пока лагерь/курган разбужен, автоатака бьёт ближайший тотем/идол в дальности удара, а без тотемов — самого Сатира (`focusTotem`); **(2) Сатир лечится только когда лагерь отпущен** (герой за engageRadius), не при каждом выходе за 130 px; **(3) охрану зовут тотемы** — снесены все, остаётся дуэль. Замер (n=24, seed placecal): Juggernaut без лагеря 25.0% побед, с лагерем 4.2% → 16.7%, все тотемы снесены в 10 из 24 (было 3), Сатир убит в 2; **Sniper без лагеря 58.3%, с лагерем 50.0% и лагерь очищен в 66.7%** — для дальника лагерь стал проходимым, мили-бот всё ещё не добивает Сатира (кайтит по кругу). Заодно найден и закрыт баг переноса опыта (см. T13.6).
- **Числа Сатира — HP и щит (владелец 2026-09-11, сетка ботом через `--enemy satyr_defiler.hp=…` и `--set defiler.shieldPerTotem=…`, n=24, seed placecal, `--places camp`).** Juggernaut: HP 1500/1200/1000 → победы 16.7/12.5/16.7%, Сатир убит 2/2/3; щит 0.25/0.15/0 → 16.7% и 2 убийства во всех трёх. Sniper: HP 1500/1200/1000 → победы 50.0/41.7/41.7%, лагерь очищен 66.7/62.5/62.5%, Сатир убит 16/15/17; щит 0.25/0.15/0 → 50.0/33.3/37.5%, очищен 66.7/66.7/70.8%. **Вывод:** ни HP, ни щит не меняют исход за пределами шума ±10 п.п.: дальник добивает Сатира и на 1500 (16–18 из 24), мили-бот не добивает и на 1000 (2–3) — его ограничивает не запас Сатира, а «порыв» 64 урона и погоня 138, от которых бот бежит при 35% HP. Числа оставлены 1500 HP / 25% за тотем; следующий рычаг для мили — урон/скорость погони Сатира, но это уже ощущение боя, не бот: плейтест. Попутно починен парсер флагов бота: `--verbose` без значения съедал следующий `--ключ` (первая сетка вышла одинаковой во всех ячейках).
- **Порыв и погоня Сатира (2026-09-12, та же методика, n=24).** Juggernaut: порыв 64→48 — Сатир убит 1 (было 2), погоня 138→118 — 2, перезарядка порыва 3.4→4.5 с — 2, порыв 48 + погоня 118 — 2, победы 4–17% (шум). **Другие мили без правок: Axe — лагерь очищен 33.3%, Сатир убит 8 из 24, победы 29.2%; Phantom Assassin — очищен 25.0%, убит 6, победы 20.8%;** со смягчённым Сатиром у них 8 и 5 — не лучше. Значит дуэль с Сатиром для мили проходима, а «не добивает» — особенность именно Juggernaut-бота (его урон в одну цель на этом уровне), не числа Сатира. Все числа Сатира оставлены; лагерь на Herald считается откалиброванным до плейтеста.

### T13.42 — Аванпост (обзорная башня): удержание области за обзор ✅ (2026-09-10) P1 (этап 1 аудита 2026-09-08)

- **Что:** `Outpost` по seed (`Rng("outpost:<seed>:<act>")`) на 650–950 px от старта и не ближе 700 к лагерю — два места в разных сторонах = выбор маршрута; не в реке/яме, не в дереве. Пока герой в `radius` 120, копится захват до 25 с (`progress` в тиках); вышел — пауза без сброса, вернулся — продолжается. Спавн вокруг не меняется: держать область значит держать давление. Захвачен — до конца акта: маркеры у края экрана для всех активных точек с таймером (лавка, bounty, руна, сундук, токен нейтралки, руна щедрости), ночью обзор ×1.35 (`sim.visionMult()` читает рендер), сразу золото bounty × 2, звук levelup. До захвата у края экрана не больше двух приглашений: лагерь (число тотемов) и аванпост (процент, если начат). Итог забега: `outpostCaptured`. Версия `a0.43.0`.
- **UI:** процедурный постамент с шестом и знаменем (красное → золото Aegis после захвата), пунктирное кольцо зоны и дуга прогресса, чип HUD «Аванпост · захват N%» в зоне, строка на итоге. Общий `drawEdgeMarker` заменил маркер лагеря.
- **Проверки:** `arcadeOutpost.test.ts` — размещение по актам и относительно лагеря, детерминизм, накопление только в зоне, пауза без сброса, однократное начисление, обзор и итог, digest; полный набор; headless на тех же сидах; Playwright на 5273 (Dire): два маркера у края, дуга прогресса и чип, захват с расширенным обзором.
- **Дальше по аудиту:** пруд очищения и порчи забега (следующий срез), затем «Наследие Aegis»; когда мест станет 5–7 — правило «не больше двух приглашений» перевести на приоритеты, а не на список.

### T13.43 — Лотосовый пруд и порча «Увядание» с проклятого сундука ✅ (2026-09-10) P1 (этап 1 аудита 2026-09-08)

- **Пруд:** `Pond` по seed (`Rng("pond:<seed>:<act>")`) на 480–820 px от старта, не ближе 450 к лагерю и аванпосту, не в реке/яме. Рядом (48 px) кнопка подбора (G) открывает выбор с паузой мира (`pondOpen`, как лавка): восстановить 50% HP **или** снять порчу; уйти — пруд остаётся. Одно использование на забег, использованный больше не приглашает. Лечение пруда Увяданием не режется: пруд и есть очищение. Маркер у края: пока есть порча (со знаком ✚) или после захвата аванпоста.
- **Проклятый сундук:** каждый сундук после первого с шансом 0.5 помечен `chest.value = 1`, **только пока пруд не использован** (нельзя выдать порчу, которую нечем снять). Рисуется с ядовитым свечением и каплями. Вскрытие: добыча на ступень редкости выше (`rarityUp`), на экране добычи — предупреждение с наградой, штрафом и способом снятия до кнопки. Надеть или в сумку = принять **Увядание**; оставить у ног = без порчи, и предмет на земле уже чистый (порча — цена вскрытия, не предмет).
- **Увядание:** `player.curse = "withering"`: `heal()` и регенерация × 0.35 до снятия у пруда; чип в HUD; на итоге «Порчи: принято N, снята/не снята». Версия `a0.44.0`. Бот у пруда снимает порчу, лечится ниже половины HP, иначе уходит.
- **Проверки:** `arcadePond.test.ts` — размещение, кнопка/пауза/одноразовость, проклятый сундук (редкость выше, оставить = чисто, с земли чистый, принять = порча), ослабление лечения и регенерации, снятие у пруда и итог, проклятые сундуки только при неиспользованном пруде и никогда первый; полный набор 860; headless Juggernaut short seed `camp`: 22.9% (n=48) против 33.3% до среза, но контроль с `chestChance = 0` на тех же сидах (n=96) даёт те же 22.9% — просадка целиком от сдвига потока Rng (спавн сундука теперь тянет float), сама порча на бота не влияет; Playwright на 5273: свечение сундука, предупреждение на экране добычи (exotic вместо refined), чип «Withering», кнопка пруда «снять порчу», модалка, снятие.
- **Дальше по аудиту:** «Долг лавочнику» и «Кровавая охота» как вторая/третья порча (не больше одной активной), реликвии-награды за лагерь; затем этап 2 «Наследие Aegis».

### T13.44 — «Наследие Aegis»: общая ограниченная прокачка за победы ✅ (2026-09-10) P1 (этап 2 аудита 2026-09-08)

- **Что:** `content/legacy.ts` — три ветки по четыре пункта: Стойкость +2% HP/пункт (до +8%), Сила +1.5% всего исходящего урона (до +6%), Исследование +3% радиуса сбора опыта (до +12%). Один пункт = одна печать, сброс бесплатный. **Начисление** в `recordProgress` (профиль T13.37): победа в `full/dire/river` = 1 печать, +1 за первую полную победу этим героем; ключ `seed|hero|act|rank` (`legacy.claimed`, кап 200) — одна и та же комбинация награждается один раз (повтор сида, повторный callback, реимпорт); дейлик содержит дату в сиде — не чаще раза в день; разминка, смерть и просмотр реплея не дают. Свёртка старой истории при миграции выдаёт печати за уже записанные победы идемпотентно.
- **Применение:** снимок `ArcadeOptions.legacy` → `sim.legacy`; HP и радиус сбора — в `recomputeStats` один раз, урон — в `damageEnemy` один раз (удары, умения, DoT, питомцы; в `stats.damage` не кладём, иначе умения множили бы дважды). Кривой снимок → `LEGACY_NONE`. **Дейлик** — `LEGACY_NONE`; **реплей** несёт пункты девятой частью кода (`v.m.r`, только если не нули; старые коды без поля читаются) и не читает прокачку зрителя. Версия `a0.45.0`.
- **UI:** панель «Наследие Aegis» на экране настройки под лестницей рангов: печати «свободных из всего», три строки с пипсами, «+1» (заблокировано без печатей или на потолке), «Сбросить печати»; на итоге «+N печатей наследия». Стили `.arcade-legacy*`.
- **Проверки:** `arcadeLegacy.test.ts` — бонусы и потолок, начисление/идемпотентность/первая победа героем/разминка и смерть, свёртка истории, стор (вложение, потолок, сброс, профиль без поля), сим (HP/pickup в статах, урон один раз, кривой снимок), реплей (9 частей, старые коды, зритель видит силу автора); `arcadeStore`/`arcadeReplay` зелёные; полный набор. Playwright на 5273 с подложенным профилем (3 печати): панель, вложение в две ветки, сохранение в localStorage, старт забега со снимком (maxHp выше базы).
- **Дальше по аудиту:** трофеи/мастерство героев после потолка (этап 2 хвост), Кентавр и Тролль-Некромант, школа Venom и гибриды яда, пресеты косметики (этап 3).

### T13.45 — Кентавр-Страж рощи: чемпион с рывком ✅ (2026-09-10) P1 (этап 3 аудита 2026-09-08)

- **Что:** `EnemyKind centaur_warden` (elite, HP 1300, r 22, лист `centaur_creep`) стоит в **роще** по seed (`Rng("grove:<seed>:<act>")`, 600–950 px от старта, ≥500 от лагеря/аванпоста/пруда; из 40 проб берётся место с наибольшим числом камней в 240 px — без камня рывок нечем прервать). **Спит** и лечится (2%/с), пока рощу не разбудили; спящий — **не цель** (`isDormant`: не в сетке, не в `nearestEnemy/enemiesWithin`, урона не берёт, рисуется тусклее с «z»), будится только входом в `wakeRadius` 140, отпуск за 420 (гистерезис как у лагеря). Пробуждение ударом убрано: автоатака по «ближайшему» будила его случайно. **Рывок:** в 380 px — телеграф 0.8 с (пунктирная стрелка на позицию героя, пятно удара бежит по ней), затем бросок 540 px/с на 440 px: попал (34 px) — 70 урона; **врезался в камень** (`ObstacleGrid.near`, kind rock) — отскок, оглушён 2.5 с, берёт ×1.5 урона, звёзды над головой и HUD «Кентавр оглушён — бей!»; добежал — **широкий удар** r 130 после телеграфа 0.6 с (48 урона + 0.3 с стана). Кулдаун 4.5 с с окном восстановления 1 с. Поводок 320 от рощи; контроль игрока не дольше 0.5 с + 3 с иммунитета (`capControl`, общий с Сатиром). **Награда:** броня и сапоги exotic у ног (защита/мобильность на выбор), обычный элитный дроп не дублируется; итог `centaurSlain`. Маркер рощи у края — только после захвата аванпоста (не больше двух приглашений до него). Версия `a0.46.0`.
- **Баланс (headless, Juggernaut short, seed `camp`, n=48; контроль с невозможным пробуждением — 22.9%):** пробуждение ударом/220 px — 14.6% (сам Кентавр никого не добивал — `deaths by killer` в `sim_arcade` теперь печатается, — но разбуженный ломал кайт бота), 140/420 — 12.5%, спящий-неуязвимый — 18.8%: остаток — бот сам заходит в рощу; для игрока это читаемая необязательная охота.
- **Проверки:** `arcadeCentaur.test.ts` — размещение/сон (не цель, неуязвим)/пробуждение входом, телеграф на позицию героя и попадание на пути, уклонение без урона с читаемой концовкой, камень на пути → оглушение и ×1.5, cc-cap после, поводок/регенерация, награда и итог, digest; полный набор; Playwright на 5273 (телеграф-стрелка, оглушение у камня).
- **Дальше по аудиту:** Тролль-Некромант (идолы и подъём павших), охотничьи контракты как способ выбирать чемпиона, Громозавр/Страж переправы в резерве.

### T13.46 — Тролль-Некромант: чемпион-призыватель у кургана ✅ (2026-09-10) P1 (этап 3 аудита 2026-09-08)

- **Что:** курган по seed (`Rng("barrow:<seed>:<act>")`, 620–960 px от старта, ≥500 от лагеря/аванпоста/пруда/рощи); два **костяных идола** (`bone_idol`, totem-вид, арт `ward_death`) и **Тролль-Некромант** (`troll_necromancer`, elite, HP 1200, лист `dark_troll`). Спящие (`isDormant`) неуязвимы и не цели; будит вход в `wakeRadius` 160, отпуск за 460. Пока герой у кургана и стоит хоть один идол — раз в 6 с у случайного идола поднимаются **скелеты** (`skeleton_warrior`, 36 HP, 98 px/с): 2 + живых идолов за пульс, не больше 12 живых. Некромант держит дистанцию 190–300 и стреляет (дальность 340, раз в 2 с, ×1.4 урона); поводок 300, дома лечится 2%/с; контроль не дольше 0.5 с (общий `capControl`). **Идолы снесены** — призыва нет, он открыт (×1.3 урона), HUD «открыт — добивай». **Смерть:** скелеты рассыпаются, награда — три exotic-карты школы «Зверинец» («улучшение Beast»; `rollUpgradeOffer(exclude, only)`), если школа недоступна — обычные exotic; через `pending` с заголовком лагеря. Итог `necromancerSlain`. Маркер кургана (с числом идолов) — после захвата аванпоста. Версия `a0.47.0`.
- **Проверки:** `arcadeNecro.test.ts` — размещение и сон/пробуждение, подъём у идолов с потолком и без идолов, открытость без идолов, кайт/выстрелы/поводок/регенерация/cc-cap, смерть → рассыпание, награда Beast, итог, digest; полный набор; headless на тех же сидах; Playwright на 5273.
- **Дальше по аудиту:** школа Venom и гибриды яда, мастерство героев, пресеты косметики; охотничьи контракты, чтобы выбирать чемпиона осознанно (сейчас чемпионы просто стоят по seed).

### T13.47 — Школа Venom: пятая школа поверх яда, две легендарные развилки, три гибрида ✅ (2026-09-10) P1 (этап 3 аудита 2026-09-08)

- **Узлы:** `ven_sting` (attack: стак яда с удара, 4·p dps/стак), `ven_cloud` (strike: раз в 2.4/(1+0.1p) с облако r 90 у ближайшего — стак всем внутри, 5·p), `ven_fangs` (cast: раз в 1.5 с стак ближайшему в 260 — запасной источник для медленных героев), модификаторы с `requires` на источник: `ven_spread` (passive: отравленный при смерти передаёт min(стаки, 1+rank) стаков 2+rank ближайшим в 90; только на смерти → без рекурсии), `ven_virulence` (power: `venomMult` = 1+0.25p ко всему урону яда, включая умения героев, и +0.5p с длительности). **Легендарки:** `leg_ven_pandemic` — при смерти все стаки всем в 160, яд +2 с; `leg_ven_distill` — шестой стак сжигает пять: взрыв `dps·5·venomMult·6` по цели и половина соседям в 70, стаки в ноль. **Гибриды** (`requiresSchools`): `hyb_venom_frost` (охлаждение отравленного +0.6p с яда), `hyb_venom_beast` (удары питомцев кладут стак 3·p), `hyb_venom_fire` (горящий с полным стеком на новом стаке взрывается 30·p + 15·p соседям, без цепи). Иконка школы — `orb_of_venom` (зеркало `gen:art`). Цифры карточек в `upgradeFigures`, подписи `arcade.fig.poison*`. Версия `a0.48.0`.
- **Проверки:** `arcadeVenom.test.ts` — реестр (5/2/3, тексты и числа), жало/вирулентность/клыки, облако/заражение/пандемия, дистилляция, гибриды; figures/upgradeTree/legendary/poison; полный набор; headless Venomancer short n=32: `--school venom` 50.0% побед против `any` 43.8% — школа сравнима с остальными, не доминирует; Playwright на 5273 (карточки Venom с иконкой Orb of Venom и цифрами, отравленная толпа).
- **Дальше по аудиту:** мастерство героев после потолка Наследия, пресеты косметики; дальше — Громозавр/Страж переправы, контракты охоты.

### T13.48 — Мастерство героев и трофеи за отметки ✅ (2026-09-10) P2 (этап 2–3 аудита 2026-09-08)

- **Отметки** (`MARK_IDS`, 8 штук, в постоянном профиле `perHero[hero].marks`): `win_full/win_dire/win_river` (победа в акте), `flawless` (победа в полном акте без потраченного воскрешения — `ArcadeOutcome.revived` = `player.aegisUsed`), `camp` (очищен лагерь), `outpost` (захвачен), `centaur`, `necro` (повержены). Событийные отметки копятся независимо от исхода, разминка отметок не даёт. Звание по числу отметок: Новичок / Ветеран (1+) / Мастер (4+) / Легенда (7+). Панель «Мастерство» под китом героя на экране настройки, счётчик в Штабе. Записи истории несут флаги `camp/outpost/centaur/necro/revived`; свёртка старых записей даёт только победы по актам; профили без `marks` читаются пустым списком.
- **Трофеи** (`CosmeticDef.unlock: { mark }`): `trail_spores` (след спор за лагерь), `trail_hoofprints` (след копыт за Кентавра), `death_bones` (кости за Некроманта) — новые варианты эффектов в `effects.ts`; не выпадают (`rollCosmeticDrops` исключает `unlock`), не покупаются (`buyCosmetic` отказывает, в гардеробе подпись «трофей» с подсказкой), выдаются один раз в `finish()`, когда отметка появилась у любого героя, и показываются на итоге как дроп. Действующее правило «косметика бесплатна» не нарушено: старое остаётся доступным, новое — за игру, не за осколки.
- **Проверки:** `arcadeMastery.test.ts` — накопление и звания, свёртка/санитизация, трофеи не в дропе и не в покупке; store/legacy/cosmetics/wardrobe зелёные; полный набор; Playwright на 5273 (панель мастерства, трофей в гардеробе).
- **Дальше по аудиту:** пресеты косметики на героя (T13.35 продолжение), бестиарий и альтернативные стартовые особенности для 5–8 героев — после плейтеста.

### T13.49 — Пресеты образа на героя ✅ (2026-09-10) P2 (этап 3 аудита 2026-09-08, продолжение T13.35)

- **Что:** `CosmeticsState` получил `shared` (общий образ: слоты кроме `skin`), `perHero` (эффекты на героя) и `perHeroLook` (переключатель «свой образ у каждого героя»). `equipped` остался производным для выбранного героя: `shared` → поверх пресет героя (если включён) → его скин из `skins`; читатели (рендер, гардероб, HUD) не менялись. `equip()` пишет в `shared` или в `perHero[hero]` по режиму; `setPerHeroLook(true)` стартует героя с текущего общего образа, чтобы картинка не прыгала; выключение оставляет пресеты в сейве. Миграция: сейвы до T13.49 — старый `equipped` без скина становится `shared`. Переключатель — в гардеробе над эффектами с подсказкой режима.
- **Проверки:** `arcadeLookPresets.test.ts` — общий/пресет/выключение/скин всегда свой, миграция и round trip через перезагрузку; store/wardrobe/mastery/gear; полный набор; Playwright на 5273 (переключатель, след пишется только герою, общий не тронут).
- **Итог этапа 3 аудита:** Кентавр (T13.45), Некромант (T13.46), школа Venom (T13.47), мастерство и трофеи (T13.48), пресеты (T13.49) реализованы. Открытые кандидаты: охотничьи контракты, Громозавр/Страж переправы/Охотник Dire, разломы, кузня, караван, «Долг лавочнику»/«Кровавая охота», бестиарий и альтернативные стартовые особенности героев, удобное сравнение обликов в превью. Числа всех срезов — стартовые, ждут плейтеста владельца.

### T13.50 — Контракт охоты: выбор чемпиона с объявленной наградой ✅ (2026-09-10) P2 (кандидат аудита 2026-09-08)

- **Что:** один раз за забег по расписанию (`ARCADE.contract.at`: разминка 3:30 (лавка занимает 3:00), полные акты 8:30 — слот «крупной необязательной охоты» из ритма аудита), если живы хотя бы два чемпиона (Сатир лагеря, Кентавр, Некромант), открывается выбор с паузой мира (`contractOpen`, как лавка): две разные цели с разными наградами из `weapon / armor / school`; пропуск без следа, второго предложения нет. Принятый контракт: чип в HUD «Контракт: цель · награда», постоянный маркер «!» у края экрана на дом цели (это выбранное приглашение, поэтому сверх лимита двух). Цель убита (Сатир — очищение лагеря) — `completeContract`: оружие exotic / броня и шлем exotic у ног / одна exotic-карта школы через `pending` (после награды самого чемпиона, если она уже висит); `events.contracts` → звук и hit-stop; итог `contractDone`; девятая отметка мастерства `contract` (Легенда — с 8 отметок). Чужой чемпион контракт не закрывает. Бот контракт пропускает. Версия `a0.49.0`.
- **Проверки:** `arcadeContract.test.ts` — расписание и единственность, две разные цели/награды, пауза мира, пропуск, нет предложения при одном чемпионе, маркер дома, чужой чемпион, награда оружием сверх обычной, награда картой в очереди после карт Некроманта, итог и отметка; полный набор; Playwright на 5273 (модалка, чип, дом цели).

### T13.51 — Порчи «Долг лавочнику» и «Кровавая охота» ✅ (2026-09-10) P2 (кандидат аудита 2026-09-08)

- **Что:** тип порчи решается при вскрытии проклятого сундука (`rollCurse`, показывается в предупреждении до кнопки): `withering` (как раньше), `debt` — сумма `60 + 12 × минута`, половина всего дохода (убийства, bounty, аванпост) уходит в погашение через общий `gainGold`, выплатил — порча снята сама; `bloodhunt` — ближайший живой чемпион (Кентавр или Некромант) становится `hunter`: его зона разбужена, сон/поводок отключены, скорость ×1.15, спящим не считается, маркер «☠» и полоса босса всюду; убил — порча снята; только при живом чемпионе и без активного контракта («свободный слот большой угрозы»), иначе выпадает Увядание. Не больше одной порчи: пока порча активна, проклятые сундуки не появляются. Пруд снимает любую (охотник возвращается домой, долг прощён). Чип показывает остаток долга. Итог `lastCurse`. Версия `a0.50.0`.
- **Проверки:** `arcadeCurses.test.ts` — долг (сумма, доля, самоснятие, прощение у пруда), охота (охотник идёт через всю карту, смерть снимает, без чемпиона — увядание), лимит одной порчи и запрет охоты при контракте; полный набор; headless Juggernaut short seed `camp` n=48: 14.6% против контроля 20.8% (контракты и проклятые сундуки выключены на том же коде) — в пределах шума ±7 п.п. этой выборки, по `deaths by killer` охотник не выделяется (centaur 4–5 как и раньше); бот теперь берёт проклятую добычу только при заметном выигрыше (`sim_arcade`, как осторожный игрок); Playwright на 5273 (предупреждение охоты, чип, охотник в полосе босса).
- **Дальше:** кузня, Громозавр/Страж переправы/Охотник Dire, разломы, караван, бестиарий.

### T13.52 — Древняя кузня: одна перековка за забег в поздней части акта ✅ (2026-09-10) P2 (кандидат аудита 2026-09-08)

- **Что:** `Forge` по seed (500–850 px от старта, ≥450 от остальных мест), остывает к `fromTick` акта (разминка 4:30, полные 12:00 — «деньги получают назначение в поздней части акта»; до этого кнопка неактивна и над наковальней время). Рядом (52 px) кнопка подбора открывает модалку с паузой мира: выбрать надетую вещь (карточки-кнопки по слотам), затем одно из трёх: **закалить** (самый слабый относительно потолка аффикс — до потолка `affixMax`), **перековать** (гарантированный аффикс остаётся, опциональные перебрасываются в другие статы пула, `reforgeGear`), **переплавить** (вещь другого слота той же редкости и тира — сначала пустого, старое из целевого слота в сумку). Цены по минуте (40+6·м / 30+5·м / 20+4·м), DEV-режим — 0. **Одно использование** на забег; результат помечен «кузня» на карточке и попадает в инвентарь: новые вещи через добычу, закалённые стартовые — по uid (`finish` берёт версию из сима). Маркер у края после захвата аванпоста, пока кузня готова. Итог `forged`. Версия `a0.51.0`.
- **Проверки:** `arcadeForge.test.ts` — закалка/перековка на чистых функциях (потолок, статы пула, без дублей), размещение и холод/готовность, пауза мира, цена, без вещи нечего выбрать, одноразовость и uid, переплавка в пустой слот и сумка, без денег ничего; полный набор; Playwright на 5273 (модалка с карточками и ценами).

### T13.53 — Гром-голем («Громозавр» аудита): заряженные зоны и цепь молний ✅ (2026-09-10) P2 (кандидат аудита 2026-09-08)

- **Что:** `EnemyKind thunder_golem` (elite, HP 1400, r 24; модели громозавра в нашем наборе нет — лист `golem`), логово по seed (640–980 px, ≥500 от остальных мест). Спит и неуязвим до входа в 170, отпуск за 480, поводок 300, регенерация дома. В бою раз в 6.5 с ставит **три зоны** r 80 на 115 px от героя в трёх из четырёх сторон — четвёртая сторона случайна и всегда безопасна («безопасный сектор остаётся даже у стены»: зоны клампятся в мир, сторона всё равно свободна); телеграф 1.2 с (кольца наливаются), затем удар 55 по стоящим в зонах и 0.6 с **цепь** между соседними зонами (42 урона на отрезке шириной 26, раз в 0.5 с). Погоня медленная (66; издалека 118). Контроль ограничен (`capControl`). **Награда:** «гибрид молнии» — `hyb_superconductor`/`hyb_plasma` exotic, если их школы уже в билде, иначе карта Maelstrom exotic; цель контракта `thunder`; кандидат Кровавой охоты; десятая отметка мастерства; итог `thunderSlain`. HUD: «четвёртая сторона безопасна». Версия `a0.52.0`.
- **Проверки:** `arcadeThunder.test.ts` — размещение/сон/пробуждение, три зоны на нужной дистанции в трёх сторонах, телеграф без урона и удар в зоне, безопасная сторона без урона на всём паттерне, цепь на отрезке, поводок/регенерация, награда-гибрид при наличии школ и Maelstrom без, цель контракта, итог; полный набор; Playwright на 5273 (телеграф колец и активная цепь).
- **Дальше:** Страж переправы (River), Охотник Dire, разломы, караван, бестиарий, сравнение обликов.

### T13.54 — Страж переправы (River): щит по фазам и волны с островком ✅ (2026-09-10) P2 (кандидат аудита 2026-09-08, специализация River)

- **Что:** `EnemyKind river_warden` (elite, HP 1300, лист `slardar`, `acts: ["river"]`) стоит в русле на 700–1100 px от ямы (сторона по seed). Спит/неуязвим до входа в 170, отпуск за 460, поводок 260, дома лечится. В бою **фазы**: 3 с под щитом (урон не проходит, кольцо и HUD «жди окно»), 4 с открыт («бей»), снова щит. Каждые 2.6 с **волна**: вертикальная полоса во всю высоту русла с **островком-разрывом** 110 px на случайной высоте, идёт 240 px/с на 780 px; попал в воде вне разрыва — 38 урона раз в 0.5 с; на берегу и в разрыве безопасно («перемещаться между островками, не пытаться пробить временный щит»). Держит дистанцию 140–220 и не выходит из воды (кроме Кровавой охоты). **Награда руническая:** DD, щит и магия на 45 с плюс амулет exotic; цель контракта `warden`, кандидат Кровавой охоты, одиннадцатая отметка `warden`, итог `wardenSlain`. Версия `a0.53.0`.
- **Проверки:** `arcadeWarden.test.ts` — только River и русло, сон/пробуждение, фазы щита (урон не проходит → проходит → щит), волна бьёт в воде вне разрыва и не бьёт в разрыве/на берегу, страж не покидает воду, награда/контракт/итог; полный набор; Playwright на 5273 (River: волны с разрывом, щит-кольцо, HUD).
- **Дальше:** Охотник Dire, разломы, караван, бестиарий, сравнение обликов.

### T13.55 — Охотник Dire (Dire): метка засады и окно уязвимости ✅ (2026-09-10) P2 (кандидат аудита 2026-09-08, специализация Dire)

- **Что:** `EnemyKind dire_stalker` (elite, HP 1100, лист `night_stalker`, `acts: ["dire"]`), логово по seed. Охотится в 560 px от логова (отпуск за 700). **Скрыт** (`stalkerHidden`): не цель, неуязвим, не рисуется. Раз в 4.5 с ставит **метку** там, куда герой придёт через 1 с по направлению взгляда (стоящий — под ноги); телеграф 1 с — кольцо с перекрестием поверх ночного тумана и звук (`events.ambushes` → «crit»): «попадание нельзя получать из полной невидимости без предупреждения». Прыжок: появляется у метки, удар 58 в r 70 + стан 0.3 с, затем **открыт** 2.5 с (медленно преследует, уязвим), потом снова скрыт и отсиживается в стороне. Ушёл из угодий — возвращается и лечится, метка снята. Кровавая охота делает его видимым насовсем. **Награда «выбор крит/защита»:** две exotic-карты — типа `attack` и типа `passive` (иначе `power`) через `pending`; `rollUpgradeOffer` получил фильтр по типу. Цель контракта `stalker`, двенадцатая отметка. Версия `a0.54.0`.
- **Проверки:** `arcadeStalker.test.ts` — только Dire, скрыт/неуязвим/не цель, метка под ноги и удар после телеграфа, открыт → скрыт, метка по движению (свернувший не получает), урон в окне, награда двух разных карт, итог, возврат/регенерация/снятие метки, контракт, охота; полный набор; Playwright на 5273 (Dire: метка ночью, окно уязвимости).
- **Итог чемпионов:** Сатир (лагерь), Кентавр, Некромант, Гром-голем — во всех актах; Страж переправы — River; Охотник — Dire. Открытые кандидаты: разломы, караван, бестиарий, сравнение обликов.

### T13.56 — Бестиарий в Штабе ✅ (2026-09-10) P3 (кандидат аудита 2026-09-08: «записи в бестиарии» после потолка)

- **Что:** сим считает `killsByKind` (в `killEnemy`), итог несёт его, запись истории — `killsByKind`, профиль T13.37 суммирует в `bestiary: Record<kind, kills>` (санитизация отбрасывает кривые записи; профили без поля — пусто). В Штабе, во вкладке Arcade, список всех `ENEMY_KINDS` (24 вида): встреченные — имя, «убито N» и подсказка по паттерну (у чемпионов — как их читать), не встреченные — «???». Имена и подсказки RU/EN — `arcade.enemy.<id>` / `.desc`. Стили `.hq__bestiary`.
- **Проверки:** `arcadeBestiary.test.ts` — сумма по видам равна kills и все ключи из реестра, итог несёт карту, профиль суммирует за забеги и терпит старые/кривые записи; полный набор; Playwright на 5273 (Штаб с подложенным профилем: 4 встречены, остальные «???»).

### T13.57 — Превью гардероба: анимация вручную, фон акта, реальный размер, сравнение с базовым ✅ (2026-09-10) P3 (кандидат аудита 2026-09-08 «Удобное превью»)

- **Что:** под сценой гардероба ряд переключателей: анимация **Авто / Стойка / Ходьба / Удар** (+ **Форма** у героев с метаморфозой — лист `<облик>@meta`, иначе `<герой>@meta`), фон **Без фона / Radiant / Dire** (пятно земли в токенах акта — читаемость облика днём и ночью), **Как в бою** (множитель как на поле: `pixelScale`, без подгонки под холст), **Сравнить с базовым** (второй холст с базовым обликом героя в тех же анимации/фоне/размере). Автоцикл оставлен по умолчанию. `pickPreviewAnim` вынесена чистой функцией.
- **Проверки:** `arcadeWardrobe.test.ts` — auto повторяет прежний цикл, принудительные фиксируют анимацию, ходьба крутит модель по кругу, still всегда стойка; полный набор; Playwright на 5273 (переключатели, сравнение двух холстов на фоне Dire).

### T13.58 — Разлом: испытание по правилу с остановленными часами акта ✅ (2026-09-10) P2 (кандидат аудита 2026-09-08 «Разлом», `a0.55.0`)

- **Что:** одно место на акт по seed (`placeRift`, ≥450 px от остальных мест), открывается по часам акта (short 2:15, остальные 5:00) — до этого у портала и в подсказке видно время. Кнопка подбора рядом открывает выбор **одного из двух правил** (пара — по seed из четырёх): **Прилив** (спавн ×1.5, скорость ×1.2), **Стекло** (входящий урон ×1.6, HP врагов ×0.6), **Мгла** (обзор ×0.55 как ночью, урон врагов ×1.25), **Безмолвие** (умения молчат, автоатака ×1.8). Вход втягивает рядовых врагов без награды (боссы/тотемы/чемпионы остаются), **часы акта стоят** (`actTick = tick − pausedTicks`; расписание Рошана/Tormentor/Древнего/финала, лавка, контракт, кузня, тиры добычи и минуты сложности читают `actTick`, таймеры статусов и баффов — реальный `tick`), обычный спавн не идёт — разлом сам зовёт лес из текущего пула (×2.4 + волна каждые 10 с, кап 36). 50 с внутри кольца арены (340 px) — один апгрейд редкости **arcana** без реролла; выход за кольцо — провал без награды. После любого исхода 8 с без обычного спавна. Второй раз не войти. Итог помнит правило; отметка мастерства «Разлом» (13-я). Рендер: портал с дугами, кольцо входа, арена с затемнением снаружи и отсчётом **поверх** тумана «Мглы»/ночи, маркер ◇ у края после захвата аванпоста; HUD — чип «Разлом · правило · время», часы с `data-paused`.
- **Не сделано/наблюдения:** бот `sim:arcade` в разлом не входит (окно закрывает) — цена/ценность правил ждёт плейтеста; арена не мешает Охотнику «Кровавой охоты» преследовать внутрь (сознательно). Числа стартовые.
- **Проверки:** `arcadeRift.test.ts` — место/детерминизм/пара правил, вход и стоящие часы (Рошан не приходит по реальному тику), четыре правила, провал при выходе и передышка, награда/итог/профиль/детерминизм; `MARK_IDS` = 13 в контракте/мастерстве; полный набор; `sim:arcade` 24×Juggernaut short (бот разлом пропускает, поток Rng не сдвинут — место на своём Rng); Playwright на 5273: подсказка → окно правил → чип и часы 5:00 стоят два секунды → выход за кольцо → провал, часы идут.

### T13.59 — Караван лавочника: сопровождение без полосы HP, в награду дополнительная лавка ✅ (2026-09-10) P3 (кандидат аудита 2026-09-08 «Караван», `a0.56.0`)

- **Что:** путь по seed (`placeCaravan`: старт ≥300 px от мест, цель по прямой ~520 px, направление из 12 проб — дальше от других мест). Появляется по часам акта (short 4:00, остальные 9:00) и ждёт до 150 с; едет (62 px/с) **только пока герой в 150 px**, вышел — стоит на месте, прогресс не теряется. В пути каждые 6 с налёт — три врага из текущего пула вокруг повозки. Полосы HP у каравана нет (по аудиту: не наказывать за AI сопровождения). Доехал — на месте цели встаёт обычный торговец на 45 с (касание открывает лавку, закрытие убирает), счётчик события → звук покупки; итог помнит «доведён». Не дождался — уходит. Рендер: повозка с колёсами, пунктир до цели с флажком, кольцо сопровождения и «%» в пути, маркер `$` у края после захвата аванпоста; HUD — чип «Караван · % · держись рядом», пока едет или герой рядом.
- **Награда усилена (владелец 2026-09-11, `a0.59.0`):** торговец каравана (`shopkeeper.value === 1`) продаёт со скидкой `caravan.discount` = 30% на товары и реролл (`shopPriceMult()`); в окне лавки строка «Лавка каравана · скидка 30%», чип каравана подсказывает «лавка со скидкой». Плановый торговец 3:00/6:00 — по полной цене. Основание: замер T13.63 — сопровождение стоило 12 п.п. побед при награде, которую бот не ценил.
- **Не сделано/наблюдения:** бот в караван без флага не ходит; отметки мастерства нет (13 остаются). Если часы перемотаны, рядом может стоять и плановый торговец 3:00/6:00 — это не конфликт. Числа стартовые.
- **Проверки:** `arcadeCaravan.test.ts` — путь/детерминизм/скрыт до часов; стоит без героя, едет со скоростью speed и зовёт налёты, останавливается без героя; прибытие → торговец в точке цели, лавка, итог; уход по окну; детерминизм. Полный набор; Playwright на 5273: появление и чип, 52% в пути с налётом, прибытие → лавка открыта.

### T13.60 — Приглашения у края экрана: приоритет и лимит «не больше двух» ✅ (2026-09-11) P3 (правило аудита 2026-09-08 «на экране не больше двух приглашений»)

- **Что:** состав маркеров у края теперь решает сим (`ArcadeSim.invitations()`, чистая функция состояния), рендер только красит по виду. **Всегда** (вне лимита): угроза — Охотник «Кровавой охоты» ☠; выбранные/начатые цели — контракт «!», лагерь в бою (агро), аванпост с начатым прогрессом (%), караван в пути. **Необязательные** — по приоритету, не больше `ARCADE.invitations.max` = 2, пока аванпост не захвачен: караван ждёт → торговец → пруд при порче → лагерь → аванпост → разлом → кузня. Захват аванпоста — обзор: все активные точки без лимита (награда T13.42 сохранена). Освобождение слота (лагерь очищен, цель начата) сразу открывает следующее.
- **Наблюдения:** до захвата башни разлом/кузня видны у края только когда базовые цели заняты или закрыты — их места по-прежнему видны на карте и в HUD рядом. Без версии баланса: сим-правила не менялись.
- **Проверки:** `arcadeInvitations.test.ts` — старт = лагерь + аванпост, разлом ждёт свободного слота; срочное вперёд (караван, лавка), пруд при порче; угрозы/выбранные сверх лимита с подписями; после захвата — всё. Соседние наборы (аванпост, контракт, караван), полный набор; Playwright на 5273: старт `camp, outpost`, после захвата — лагерь, логово, роща, курган, пруд (что активно на 0:01).

### T13.61 — Прилив в River: цикл уровня воды с сухим бродом и ямой ✅ (2026-09-11) P3 (идея аудита 2026-09-08 «позднее — смена уровня воды», `a0.57.0`)

- **Что:** только River. По часам акта с 1:00 цикл **подъём 6 с → прилив 30 с → отлив 60 с** (первый прилив тоже телеграфирован) (`tidePhase()` — чистая функция `actTick`, состояния нет, пауза разлома фазу не двигает). В прилив русло шире в 1.7 (150 → 255 px), в воде герой и рядовые враги медленнее на 35% (управление не отнимается — по аудиту «течение не отнимает управление»); боссы/неудержимые не замедляются. **Всегда сухие:** полоса ±70 px у брода (`ford.x`) и яма Рошана — безопасный путь есть в любую фазу. Рендер: подъём — пульсирующий контур будущей ширины с пунктиром, прилив — широкая вода и светлая полоса брода с пунктирной кромкой; HUD — чип «Вода поднимается · t» / «Прилив · t · сухо у брода и в яме». Плановые точки River (лавка/руны по `riverPoint`) остаются в базовом русле — в прилив к ним идти медленнее, это и есть «управление пространством».
- **Хвост закрыт (2026-09-11):** вода расширяется в самой терре — `Terrain.riverHalfWidth` выставляется рендером из `sim.riverHalfWidth()`, ключ чанка (`chunkKey`) несёт полуширину только у чанков, задевающих максимальную полосу прилива (остальной кэш не сбрасывается); заливка поверх стала тонировкой (0.55). `arcadeTerrainTide.test.ts` — полоса и ключи; Playwright: прилив 255 → отлив 150, кромка воды двигается. Отлив базовое русло не сушит (так и задумано). Бот на River тайды не учитывает. Числа стартовые.
- **Проверки:** `arcadeTide.test.ts` — фазы по часам акта и цикл, вне River всегда отлив; ширина и замедление героя в воде, брод/яма/отлив без замедления; рядовой враг медленнее; детерминизм. Наборы Стража переправы/разлома/приглашений/реплея, полный набор, `sim:arcade` 12×River; Playwright на 5273: подъём (чип и контур), прилив (чип, `inCurrent` у героя в воде).

### T13.62 — Стартовые особенности героя: плюс и минус за отметки мастерства ✅ (2026-09-11) P3 (аудит 2026-09-08 §мастерство: «альтернативная стартовая особенность с преимуществом и недостатком», `a0.58.0`)

- **Что:** общий набор для всего ростера (`content/traits.ts`, не 126 деревьев): **Берсерк** (+20% урона, удар на 8% быстрее, −15% HP), **Бастион** (+20% HP, +3 брони, −10% скорости), **Скороход** (+12% скорости, +15% радиуса сбора, −10% урона), **Собиратель** (+30% сбора, +60% регенерации, −10% HP). Применяются в `recomputeStats` поверх `hero.base` до пассивок/апгрейдов/экипировки. Открываются числом отметок мастерства **этого героя** (1 / 1 / 2 / 2). Выбор на экране подготовки (ряд кнопок под китом; закрытые тусклые с подсказкой «откроется за N отм.»), сбрасывается при смене героя, если у него закрыто. Дейлик — всегда без особенности. Реплей: десятая часть кода `A1~…~<наследие или 0.0.0>~<trait>`; старые коды и коды без особенности не меняются, «0.0.0» при разборе не считается наследием. Итог и история помнят особенность.
- **Не сделано/наблюдения:** особенности не меняют поток Rng и китов — только базу. Бот: `npm run sim:arcade -- --trait <id>`; на 24 сидах Juggernaut/short (n мал): база 16.7% побед, Берсерк 16.7%, Бастион 20.8%, Скороход 16.7%, **Собиратель 29.2%** (уровень p50 17 против 13). **Перезамер 2026-09-11 на n=96, две базы героев:** Juggernaut — база 29.2%, Собиратель 26.0%, Бастион 21.9%; Sniper — база 56.3%, Собиратель 50.0%, Бастион 56.3%. Перевес Собирателя на n=24 был шумом: на n=96 ни одна особенность не доминирует, все в пределах ±7 п.п. от базы с минусом на стороне особенности — числа оставлены. Числа стартовые до плейтеста.
- **Калибровка на n=72 (2026-09-12, seed `traitcal`, short, Herald, `a0.61.0`).** До среза — Juggernaut: база 13.9% побед (уровень p50 12, убийств 743), Берсерк 12.5%, Бастион 13.9%, Скороход 22.2%, **Собиратель 22.2% (уровень 17, убийств 1188)**; Sniper: база 58.3% (уровень 26), Берсерк **44.4%**, Бастион 54.2%, Скороход 51.4%, Собиратель 59.7% (**уровень 32**). Собиратель подтверждён: для мили это +8 п.п., для дальника — на шесть уровней выше при тех же победах, а минус (−10% HP) дешёвый. **Срезано:** радиус сбора ×1.3 → ×1.2, регенерация ×1.6 → ×1.4 (HP оставлен 0.9). После среза: Juggernaut 20.8% (уровень 15, убийств 971), Sniper 61.1% (уровень 31) — преимущество по уровню ужалось с +5 до +3, победы мили всё ещё выше базы на 7 п.п. — это плата за отметки мастерства, дальше не режем без плейтеста. Берсерк ниже базы у обоих: минус HP 0.85 → 0.9 вернул Juggernaut к базе (13.9%), Sniper остался 44.4% (убийств p50 2519 против 3283 — умирает раньше, +урон/скорость удара дальнику не помогают). Оставлен как рискованный выбор для мили; ⬜ если владелец сочтёт его мёртвым для дальников — заменить минус HP на минус скорости. Первая серия вышла одинаковой во всех ячейках — `$arg` в zsh не делится на слова, флаг ушёл одной строкой «--trait berserk» (урок из T13.6 про парсер повторился с другой стороны).
- **Проверки:** `arcadeTraits.test.ts` — у каждой есть плюс и минус, множители на базе, чужой id → без; реплей с/без особенности, кривая → null, заглушка наследия, воспроизведение бит-в-бит; стор — закрытая не выбирается, отметки открывают, смена героя сбрасывает. Наборы реплея/наследия, полный набор; Playwright на 5273: с одной отметкой открыты Берсерк/Бастион, Скороход/Собиратель закрыты; выбран Берсерк → в забеге `trait=berserk`, HP ×0.85, урон ×1.2.

### T13.63 — Бот `sim:arcade` ходит в места (`--places`): ценность наград, а не только цена контакта ✅ (2026-09-11) P2 (хвост T13.58/T13.59: «бот в разлом/караван не ходит»)

- **Что:** `npm run sim:arcade -- --places rift,caravan,pond,forge,camp|all`. Одна активная цель (`placeGoal`): разлом (открыт → к входу, кнопка, первое правило; идёт → держаться у центра арены даже при бегстве), караван (ждёт/едет → к повозке, в пути тяга к ней сильнее бегства), пруд (порча или HP < 50%), кузня (готова, есть золото и вещь → первый надетый слот, закалить), лагерь (не раньше 8-го уровня). Окна мест обрабатываются до мировых действий (правило памяти про дедлок). В итоге печатается `places done: …` и доля побед среди прошедших разлом. Без флага бот прежний — базовые замеры не сдвинуты.
- **Замеры (Juggernaut, short, Herald, seed placecal n=24 / riftcal n=48):** база 20.8% побед. Пруд: 100% использован, победы те же, уровень выше — бесплатная страховка работает. Кузня: 42% использована, 25% побед. Разлом (n=48): пройден в 46% забегов, победы 22.9% против 20.8%, но смерть p25 160 с против 207 с — бот гибнет внутри на 2:15 короткого акта; среди прошедших разлом побеждает половина. Караван: 37.5% доведён, победы 8.3% — сопровождение дорого, награда (лавка) для бота почти ничего не стоит. Лагерь: даже с 8-го уровня 8% очищений и 4% побед — бот не умеет бить тотемы и уходить, это предел «случайного игрока», а не вердикт балансу.
- **Выводы для калибровки:** числа разлома/пруда/кузни не трогаем (нейтрально-положительно). Кандидаты на ручной плейтест: награда каравана (скидка или золото при прибытии), стоимость лагеря на Herald. `--trait`-замеры — в T13.62.
- **Проверки:** typecheck скрипта через временный tsconfig (в `npm run typecheck` `scripts/` не входит — там же поправлен устаревший `omniLeft` в трассе); прогоны выше; без флага результаты прежних сидов не изменились.

### T13.64 — Избранные и недавние герои на экране подготовки ✅ (2026-09-12) P2 (владелец: «чтобы не искать каждый раз во всём списке»)

- **Что:** над сеткой из 126 карточек — поиск по имени/id и подсказка; на каждой карточке в углу ☆/★ (клик не выбирает героя — `stopPropagation`, доступно с клавиатуры). Сетка делится на группы с заголовками: **Избранные** (порядок добавления, потолок 24), **Недавние** (последние 5 героев по ленте забегов, без повторов и без избранных — ничего нового не хранится), **Все герои** (порядок ростера); заголовок «Все герои» появляется только когда есть другие группы. Поиск сужает все группы; пусто — строка «Никого не нашлось». Логика — чистые функции `features/arcade/heroPicker.ts` (`recentHeroes`, `groupHeroes`, `toggleFavorite`), состояние — `favorites` в `arcadeStore` (`aegis-draft.arcade.favorites`, per-device как автокаст, чужие id отбрасываются при чтении).
- **Почему так:** избранное — явное действие игрока (звезда), недавние — бесплатный второй ярус из уже существующей ленты; поиск закрывает случай «знаю имя». Отдельной вкладки/фильтра нет: группы видны сразу, ничего переключать не надо.
- **Проверки:** `arcadeHeroPicker.test.ts` — недавние по ленте и лимиту, группировка и поиск (по имени и по id, чужой id игнорируется), toggle с потолком, стор с персистом; Playwright на 5273: звёзды у Zeus и Io → они первые, заголовки «Favorites / All heroes», поиск «jugg» → одна карточка, после перезагрузки порядок сохранён.


### T13.65 — Наследие: первая победа и дедупликация не зависят от последних 200 выплат ✅ (2026-09-12) P2 (аудит 2026-09-12)

- **Сделано:** первая полная победа героем определяется по постоянной отметке `win_*` в `perHero[hero].marks` (плюс прежняя проверка `claimed` как запас); `claimed` остаётся защитой от повтора той же комбинации seed/hero/act/rank в пределах последних 200 ключей — политика записана в комментарии `recordProgress`, пожизненная однократность повторов не обещается. Миграции не нужно: отметки уже есть у всех профилей (свёртка истории T13.37/T13.48). Регрессия — `arcadeLegacy.test.ts` (репро аудита с ожиданием 1 печати вместо 2).

- **Подтверждено:** `arcadeStore.recordProgress` определяет первую полную победу героя по `legacy.claimed`, а список обрезается до 200. Победа Axe, 200 уникальных побед Juggernaut, повтор исходного Axe → печати 203→205 вместо 203, хотя постоянный профиль помнит победу Axe. Потолок боевой силы не снимается; проблема в учёте наград.
- **Файлы:** `web/src/state/arcadeStore.ts`, `web/test/arcadeLegacy.test.ts`. [Отчёт и воспроизведение](audits/2026-09-12-arcade-second-pass.md).
- **DoD:** первая победа определяется постоянным признаком, миграция существующего профиля сохраняет достижения; политика дедупликации завершений явно определена и не обещает пожизненную однократность при ограниченном окне; тесты после >200 выплат проверяют старый и новый seed ранее победившего героя, обычные повторные callback/replay и отсутствие повторного бонуса первой победы.

### T13.68 — Отдельные награды событий не сливаются в один выбор ✅ (2026-09-12) P1 (аудит 2026-09-12)

- **Сделано:** `campRewardQueued: Offer[]` заменён очередью выдач `rewardQueue: Offer[][]`; `queueReward` при висящем `pending` кладёт выдачу целиком, `applyOffer` после выбора достаёт следующую — каждая награда (чемпион, контракт, лагерь, курган, разлом) показывается отдельным экраном в порядке событий. Регрессия — `arcadeContract.test.ts`: висящий выбор уровня + смерть Некроманта с контрактом → 3 карточки чемпиона → 1 карта контракта → пусто.

- **Подтверждено на фикстуре состояния:** при уже существующем `pending` убийство Некроманта с контрактом «карта школы» кладёт три карточки чемпиона и одну карту контракта в один `campRewardQueued: Offer[]`. После начального выбора появляется четыре альтернативы, после выбора одной очередь пуста — дополнительная награда потеряна. Без исходного pending существующий тест проходит. Частота естественного совпадения событий в одном шаге не измерена.
- **После параллельного рефактора T13.66:** `queueReward` устранил перезапись ранее поставленных карточек, но всё ещё объединяет разные выдачи через concat в один выбор. Этот остаточный дефект ведётся отдельно от завершённого рефактора.
- **Файлы:** `web/src/game/arcade/sim.ts` (`queueReward`, `openBarrowReward`, `clearCamp`, `completeContract`, `endRift`, `applyOffer`), `web/test/arcadeContract.test.ts` и тесты событий. [Отчёт и воспроизведение](audits/2026-09-12-arcade-second-pass.md).
- **DoD:** очередь хранит отдельные выдачи с источником/правилом выбора; контракт и награда чемпиона получаются независимо; несколько событий не перезаписывают/не объединяют друг друга; тесты с исходным pending и без него, сочетаниями лагерь/чемпион/контракт/разлом, исчерпанным пулом и replay. Редкость, лимит школ, изгнание и версия воспроизведения учтены.

### T13.67 — Подсказка Наследия правильно объясняет replay ✅ (2026-09-12) P2 (аудит 2026-09-12)

- **Сделано:** `arcade.legacy.hint` RU/EN: «Дейлик — без наследия; реплей берёт снимок исходного забега, а не текущую прокачку профиля». Логика реплея не трогалась.

- **Подтверждено:** `arcade.legacy.hint` в RU/EN утверждает, что реплеи не используют наследие; `arcadeStore.startReplay` применяет сохранённый снимок. Это текстовая неточность, не баг воспроизведения.
- **Файлы:** `web/src/i18n/core.ts`; существующая проверка снимка — `web/test/arcadeLegacy.test.ts`.
- **DoD:** обе локали объясняют «дейлик без наследия; реплей со снимком исходного забега, независимо от текущей прокачки профиля»; текст подтверждён в подготовке; логика replay не меняется. [Аудит](audits/2026-09-12-arcade-second-pass.md).

### Кандидаты следующего этапа Arcade — проект 2026-09-08, не утверждённая реализация

Запрос владельца — анализ и предложения по картам, косметике, мини-боссам, статусам и общей прокачке. Полный дизайн, ограничения референсов и приоритеты: [аудит контента и прогрессии](audits/2026-09-08-arcade-content-and-progression.md). **Статус 2026-09-11:** все направления таблицы реализованы срезами T13.37–T13.62 (цели карты, яд/порчи, чемпионы, Наследие, пресеты, кузня/разлом/караван, прилив River, приглашения, стартовые особенности); числа всех срезов стартовые — следующий шаг по плану аудита «4. По результатам игры»: плейтест владельца и калибровка. Таблица оставлена как история выбора. Существующие T13.27/T13.33/T13.36 по моделям и гардеробу не дублируются.

| Кандидат | Объём и точки изменения | DoD будущей реализации | Зависимости |
|---|---|---|---|
| Цели карты | Лагерь заражения, обзорная башня; `game/arcade/mapgen.ts`, `sim.ts`, `types.ts`, `features/arcade/renderer.ts`, HUD/i18n | Seed задаёт достижимые точки и два разумных маршрута; можно пропустить событие; нет обязательного боя в закрытом кармане; не больше двух активных приглашений | T13.19, текущие коллизии; первый рекомендуемый срез |
| Яд и очищаемая порча | Самостоятельный poison, одна порча с явным условием, пруд очищения; существующие обработчики статусов в `sim.ts`, `types.ts`, киты/школы и VFX | Источник/стаки/таймер читаемы; яд отличен от burn; нет рекурсивных взрывов; порча принимается по кнопке и всегда разрешима; replay детерминирован | Цели карты; начать с одного источника яда, затем расширять |
| Поведенческие чемпионы | Сатир-Осквернитель, затем Кентавр и Тролль-Некромант; `content/enemies.ts`, `sim.ts`, renderer/sound | У каждого телеграф, окно ответа, способ отступить и выбор награды; мили/дальник/призывы могут победить; крупная встреча занимает слот расписания, не перегружая босса | Сатир — после лагеря; остальные после плейтеста первого |
| Наследие Aegis | Общая ограниченная прокачка за победы `full/dire/river`; постоянный профиль, конфиг старта, `replay.ts`, `arcadeStore.ts`, итог/Штаб | Победа героя A усиливает B; потолок силы; daily с фиксированной нулевой метой; replay содержит снимок; награда идемпотентна; старые анлоки не теряются | **T13.37**, баланс новой базовой петли; версия Arcade отдельно от драфта |
| Пресеты и наградная косметика | Образ героя целиком, бесплатный текущий каталог, новые трофеи за события; `HeroWardrobe.tsx`, `cosmetics.ts`, `arcadeStore.ts`, effects | Пресеты не переносят эффекты на чужого героя без выбора; есть миграция общих эффектов; боевые зоны читаемы; текущие бесплатные облики не закрываются | T13.35; аудит моделей остаётся в T13.36 |
| Специализация актов и поздние цели | Dire-очищение/свет, River-переправы, поздняя подготовка (кузня — T13.52, разлом — T13.58, караван — T13.59) | Акты отличаются решениями; события не требуют недоступного билду действия; финал и путь к нему всегда проходимы; у разлома определено поведение часов (сделано: часы акта стоят) | Плейтест целей карты и чемпионов; не начинать с четвёртого биома |

## M14 — Ревизия кода 2026-09-13 (шесть слоёв: сим Аркады · клиент Аркады · драфт/сторы · UI/i18n · Go/CI · тесты/инструменты)

Владелец: «пройдись по проекту — что улучшить/оптимизировать/убрать, потом закоммить и запушь». Метод: шесть параллельных read-only аудитов с `file:line` → внедрение только перепроверенного, потоками в отдельных git worktree (драфт/сторы, клиент Аркады, сим Аркады, UI, Go/CI, тесты/инструменты) → интеграция cherry-pick в одну ветку, rebase на свежий `main`, полный прогон → fast-forward. Гейты: tsc; полный vitest на реальном датасете и на моке (golden); симуляторы бит-в-бит до/после (`sim_run` ×3 режима, `sweep_seeds_both`, `find_camp_seed`, `sim_manager`, бот Аркады + хэш дайджестов); e2e на своих портах (anteRun, smoke, arcade chromium+mobile, offline на прод-сборке и др.); Go `vet/build/test`; побайтный output пайплайна на локальном снапшоте; computed style 21 снимка до/после для CSS-переносов и code-splitting. Ложные «мёртвые» находки аудита снова были (геттер `RunEngine.manualAssignment` читают тесты, `encodeRunLink` нельзя импортировать в e2e) — каждое удаление проверено поиском и полным vitest.

### T14.1 — Драфт, рогалик, сторы ✅ (2026-09-13)
- **Сила этапа — одна сборка** (`game/runStrength.ts`: `stageBuild` / `stageBoss` / `evaluateStage` / `stagePowerOf`), её зовут игра, resume, `sim_run`, `sweep_seeds_both` и экран турнира. Баги: resume посреди этапа терял слои предметов и защиту от босса (поле после перезагрузки другой силы); игровое поле не учитывало ослабление редкости Wide Pool (экран и сим — учитывали). `rarityFactor` обязателен. Симуляторы бит-в-бит прежние → калибровка не сдвинута, `BALANCE_CONFIG_VERSION` не бампался. Тест `runStageResume`.
- **Вход в комнату Арены/Дуэли:** «подключение» до барьера squadSynergy, номер попытки (выход и повторный вход отменяют устаревшую), события заменённого сокета игнорируются. Тест `relayRoomConnect` (на старом коде 6/6 красные) — часть тестовой дыры T12.5.
- Мёртвый код (`isPlayerOpen/isHeroOpen`, `resetAssignment`, `RarityRow`, `careerRunIdFromRun`, групповые тики воспроизведения); `ELO_DIVISOR`/`eloWinProbability` вместо копии в `manager/world.ts` (сим менеджера бит-в-бит); `FORMATS`/`isFormat` в `game/packs.ts`; цикл `tournament ↔ tournamentPower` разорван (`game/fieldModel.ts`); подпись своей команды и «из/from» — ключи i18n; одна запись сейва на клик в Буткемпе (`logAction`); `campCelebration` сбрасывается в reset/resume; `scripts/lib/sim_shared.ts` (драфт «первым доступным» и конфиг e2e-сидов).

### T14.2 — Code-splitting ✅ (2026-09-13)
`React.lazy` для экранов вне основного пути забега (настройки, справочники, Штаб, Карьера, Правила, Менеджер, Дуэль, Арена, Аркада) + `vite:preloadError` → одна перезагрузка на свежую сборку (не чаще раза в 10 с). Старт → драфт → турнир → Буткемп остаются в стартовом чанке, поэтому `camp.css` по-прежнему общий. Сим Аркады, её контент и `arcadeStore` ушли в общий ленивый чанк (их импортируют только Аркада, Штаб и Карьера). Стартовый JS: **1384 → 856 КБ (gzip 426 → 276 КБ)**, основной CSS 198 → 132 КБ; precache 954 записи, offline-спека 3/3. Крупнейший модуль стартового чанка теперь `i18n/core.ts` (390 КБ, обе локали) — кандидат ниже.

### T14.3 — Клиент Аркады ✅ (2026-09-13)
Баги: «Смотреть реплей» терял особенность; «Ещё раз / Новый сид / Смотреть реплей» не перемонтировали сцену (стартовая экипировка и звуковые счётчики прошлого забега); ночной пепел не рисовался в пиксельном режиме; сундук включал сглаживание до конца кадра; клавиатура глушила Tab/Enter/Space во всех окнах, Esc в подтверждении снимал паузу; смена набора пиксельных листов сбрасывала не все кэши; кнопка изгнания оставалась на наградах мест (реролл там уже был скрыт). Перф: загрузка листа не сбрасывает кэш земли, LPC-смерть грузится только без клипа Dota. `formatClock` → `game/arcade/clock.ts` (Штаб и Карьера не тянут рендерер); модалки Аркады с именем и подписью крестика; мёртвые i18n-ключи; `docs/arcade-dota-sprites.md` — WebP, кадры 160/80, конвейер земли. e2e `arcade.spec` (+перемонтирование, +клавиатура) 12/12 chromium+mobile.

### T14.4 — Сим Аркады ✅ (2026-09-13, a0.72.0; T13.66 срез 2)
- **Часть A — без смены чисел** (гейт: бот, хэш дайджестов и хэш по всем 126 героям бит-в-бит): «Сердце Древнего» попадает в итог победы (`finish` копировал лут до выпадения); реролл уровня не теряется в коде реплея (байт 255 читался как 254; константы `REROLL_CHOOSE`/`BANISH_ACT`); горячие запросы к врагам — сначала дистанция, потом `isDormant`, счётчики тотемов/идолов за O(1), фаза прилива с кэшем; без аллокаций в циклах на тик (бот juggernaut full ≈ −28% времени, замер шумный); разбор забега пишет урон нужному источнику.
- **Часть B — исправления, меняющие числа** (a0.71.0 → a0.72.0, у каждого свой тест): Flesh Heap (Pudge, Undying) не обнуляется пересчётом статов; у каждого вида умения своё состояние игрока — Bedlam у Dark Willow не гасится Cursed Crown, Sunder у Terrorblade не затирает Метаморфозу (тест контента «нет двух видов с общим полем»; кольца зон в рендере — тоже по своим полям); волны, тролли, щедрость, торговец, bounty, руны и сундук идут по часам акта и стоят в разломе; изгнание запрещено на экране награды места (кнопка скрыта); победа не перезаписывается смертью в том же тике; станы уважают unstoppable, лечения с удара — после выходов по неуязвимости; потолок продления яда холодом; логова Гром-голема и Охотника видят разлом и караван (минимум до разлома 15/9 → 371/319 px); проклятый сундук не затирает уже взятую порчу.
- Бот до/после на 8–16 сидах в пределах шума, кроме Pudge (победы 25 → 50%, p50 смерти 566 → 1206 с). Объединённая ветка даёт тот же вывод бота, что ветка сима (juggernaut, lina — бит-в-бит); vitest 1007 зелёных; e2e arcade 12/12 chromium+mobile.

### T14.5 — UI / design / PWA ✅ (2026-09-13)
Кэш превью рынка переживает локальные рендеры, `before` не пересчитывается; Modal — доступное имя по умолчанию (`useId`), подпись крестика обязательна (было английское умолчание «Close»); App на булевом селекторе арены (не перерисовывается на каждый пик); таймер драфта арены только с дедлайном, пул раунда мемоизирован; `theme-color` из токена `--bg` (ThemeProvider, `index.html`, manifest); ключи кэша SW из `sw/policy`; пиксельные PNG Аркады в precache (+594 файла, +0.4 МБ); стили общих компонентов рядом с ними (`RarityBadge.module.css`, `buildCardInspector.css`, `campCards.css`, `playerInspector.css`, `.score-strip` в `App.css`) — предусловие T14.2; мёртвые `.run-meta`, `.duel__names`, `--bg-elevated`, `--bp-*`, `Breakpoint`, 9 экспортов барреля; `careerStatTiles`, `FORMAT` из `startOptions`.

### T14.6 — Go, данные, инфраструктура ✅ (2026-09-13)
- **data-refresh снова находит новые турниры.** С 2026-07-16 в 45 бот-коммитах не появилось ни одного события: discovery-ответы OpenDota жили в raw-кэше вечно. Explorer ограничен полночью UTC дня as-of, у `/leagues`, `/teams`, `/heroes` срок годности 20 ч (время загрузки в `<ключ>.meta.json`, при сбое — прежний ответ). Ближайший прогон догружает детали матчей с 07-16; при нехватке бюджета emit честно откладывается на следующий. ⬜ Проверить по первым прогонам data-refresh после ревизии: новые события в `events.json`, `squadSynergy.json` ≈ −68%.
- `squadSynergy` эмитит только пары (мок зеркально); CLI без источника — ошибка вместо пустого датасета в `--out`; мёртвый Go-код (Peak, остатки team-success, career/roster-клиенты); окна и даты из `formats`, атомарный emit.
- Сервер: выброшенный за переполненный outbox участник становится disconnected и комната чистится; `log.Fatalf` в горутине → `run() error` (T12.5); таймаут хендлеров < WriteTimeout; удалены `Identity`, `JoinedAt`, лишние ПДн initData; `go mod tidy`.
- Инфра: alpine 3.22, nginx 1.28, `setup-flyctl@1.6`, `--ha=false` (комнаты в памяти одного инстанса); nginx-стенд не срезает `/api`, ws-апгрейд, `.dockerignore`. README pipeline/server/schema/infra синхронизированы.

### T14.7 — Тесты, CI, инструменты, документы ✅ (2026-09-13)
- CI: 14 тестов реального датасета теперь выполняются (vitest в `web-e2e-real`); offline-спека в `playwright.offline.config.ts` на готовом `dist`, порядок `typecheck → build → e2e`; sparse checkout в мелких джобах, `timeout-minutes` во всех; antipatterns блокирует деплой по 4 критичным категориям.
- `gen:mock` пишет в `web/.mock-data`, тесты читают `AEGIS_DATA_DIR` (`npm run test:mock`) — боевой датасет больше не перезаписывается ради golden; `loadGameData` кэширует датасет, `loadManifest` для skipIf; `tsx` в devDependencies; e2e — хелперы входа в Classic и run-link, `toPass` вместо `waitForTimeout`.
- `antipatterns_grep.sh` на `git grep` (без шума черновиков и ложного «секрета»); `pixel_terrain.mts` на WebP, разовый `sheets_to_webp.sh` удалён; `.gitignore` без дублей (`**/.tmp_*`), `make dev` не ставит зависимости каждый раз; устаревшие брифы → `docs/archive/`; чек-лист самопроверки — под мок в `web/.mock-data`.

### T14.8 — Кандидаты, не взятые в этот проход ⬜
- **Бандл/i18n:** `i18n/core.ts` — 390 из 715 КБ стартового чанка (обе локали, 64% строк — Аркада): словари по локалям с ленивой неактивной и `arcade.*` в чанке Аркады. Движки Менеджера и Арены остаются в старте через сторы в шелле.
- **Драфт/сторы:** `restoreRun` и `buildPrepView` из `runStore` в `game/`; одна константа сброса полей забега (`start`/`reset`/`startArenaTournament` расходятся); хелпер однотипных Camp Actions; неиспользуемые `data`/`format` у `TournamentEngine`/`AnteRunEngine`; ~34 лишних `?? {}` в `RunEconomy`; подписи «Arena» и «Bot N» через i18n; `dismissError` Арены vs Дуэли (нужна семантика протокола); общая `TeamRadarPanel` на 4 экрана + форматтеры; ветки старых сейвов — только с фикстурой.
- **Клиент Аркады:** строки смерти по направлениям 1..N−1 не рисуются нигде (~17% строк листов, 90–130 МБ на двух наборах — снимать вместе со следующим пакетом рендера); непиксельная ветка `?pixel=0`, LPC и риги (решение владельца); отдельное ведро SW для арта Аркады с версией набора; `index.json` меты листов; culling по камере; разрез `ArcadeScreen.tsx`/`renderer.ts` + `ArcadeOverlay` (хвост T13.66); LRU полноразмерных canvas-копий гардероба; вечный rAF у статичных миниатюр; перекраска ауры раз на цвет; предзагрузка всех врагов на экране выбора; литералы canvas/HUD и цвета через i18n/токены; `--surface-solid` в `arcade.css`.
- **Сим Аркады:** `activeModal()` — единственный порядок окон для сима, бота, `sim_kit` и экрана (сейчас при одновременных луте и контракте действие бота уходит не в то окно) + именованные коды кузни вместо `10+i` и литерала `5`; перенос `ArcadeHistoryEntry` из `state/` в `game/arcade/types.ts`; мёртвые поля (`Rift.kills`, `Grove.rocks`, `Caravan.raids`, `ArcadeOutcome.lastCurse`, `thunder.zones`, числа `PETS.summon`); общий `test/helpers/arcade.ts`; честные тексты Ice Vortex/Cursed Crown (игнорируют `duration`), мёртвый `count` у Luna R; сроки жизни событий (сундук, торговец, караван, мегакрипы) истекают по реальным тикам, пока герой в разломе; Плазма поджигает неуязвимого чемпиона; `archerLines.find` и `life_drain` через `enemies.find` на тик; бот на lone_druid сид #8 идёт часами; таблица CHAMPIONS вместо ~14 списков; разрез `sim.ts` по карте аудита; диспетчер `castAbility`; каденс контакта лучников/шаманов (подтвердить).
- **UI:** палитры режимов `start.css` через `data-accent`; `camp-boss*`/`camp-player-card*`/`offer-overlay*` рисуются в турнире и Штабе, а стили в `camp.css`; утилиты `.list-reset`/`.truncate`, брейкпоинты 900/1020 вне канона; `ui/sound.ts`/`ui/motion.ts` вне `ui/`; precache непиксельных `art/abilities` (1.9 МБ).
- **Данные/Go/CI:** `teammates.json` (789 КБ, клиент не читает) и `playerHeroStats.json` (фолбэк, который не срабатывает) — из контракта; минифицированный emit (−44% сырого объёма); `[games, wins]` вместо `{games, winrate}` (правка инварианта CLAUDE.md); срез career по players.json; кап relay-лога/комнат и rate-limit `POST /api/rooms`; закрытие ws при shutdown; лимит повторов «ядовитого» матча; перезапрос `/matches/{id}` без `lane_role`; чистка explorer-ответов прошлых дат; счётчик сессий в `RoomManager`; paths-фильтры и `concurrency` в `ci.yml`; промежуточные артефакты normalized/aggregate; описание групп 3–5 в `schema/squadSynergy.schema.json` и `types/data.ts`.
- **Тесты/инструменты/агентская инфраструктура:** typecheck для `test/`, `e2e/`, `scripts/` (162 ошибки типов); архив закрытых блоков BACKLOG (~37% файла); px-манифесты TSV выводятся из px2; глобальный `testTimeout`; `with { type: "json" }` в `src/data`, чтобы e2e мог импортировать кодеки; хук `remind_frontend.sh` проверяет весь файл, а не правку; `.codex/hooks.json` с абсолютным путём и копией хука; `.cursorrules` дублирует маршрутизацию; скилл `external-data-etl` упоминает удалённый `/players/{id}/heroes`; `settings.local.json` — сотни разовых разрешений (решает владелец).

## M15 — Ревизия кода и живой прогон Аркады 2026-09-19

Владелец: «очередной прогон кода и того, что есть в игре: что улучшить/оптимизировать/убрать в коде и что улучшить/добавить в Arcade; можно гонять тесты или играть руками». Метод: три read-only аудита с `file:line` (сим Аркады · клиент Аркады, упор на не проходивший аудит код T13.80 · всё остальное + триаж T14.8) → перепроверка чтением кода → правки с тестом; параллельно живой headless-прогон на 5273 (Playwright, бот-кайтер на клавиатуре с рулением из `sim_arcade.ts`, desktop 1440×900 и iPhone 13, скриншоты ключевых экранов, замер кадров, консоль, HTTP ≥ 400) и сим-проба «новичок». Правки сима и Go-сервера — отдельными потоками в git worktree.

### T15.1 — Живой прогон Аркады: факты
- **Кадр и загрузка — в норме.** Juggernaut/Sniper/Crystal Maiden, до 136 живых врагов: p50 8.3 мс, p99 10.4 мс (120 Гц headless), куча 45–60 МБ, загрузка забега 0.3 с, 0 ошибок консоли, 0 ответов ≥ 400. Все «рывки» > 50 мс совпали с моими же `page.screenshot` — собственных фризов игры не найдено.
- **Герой уходил под HUD у края мира (исправлено, T15.2).** Камера упиралась в границу, и в углу мира герой оказывался под панелью умений и миникартой: на desktop — под миникартой, на телефоне — под кнопкой R и портретом. Поймано на смерти бота «в углу, которого не видно», подтверждено телепортом через `__arcadeSim()`.
- **Кнопки итога уезжали под сгиб (исправлено, T15.2).** 1440×900, смерть с лутом и школами: «Ещё раз / Новый сид / В меню» ниже видимой части сцены, нужна прокрутка оверлея.
- **Проба «новичок» (`web/.tmp_probe_idle.mts`, Herald, полный акт, 12 сидов, карточки — первая, все умения жмутся).** Стоящий на месте герой живёт: Juggernaut p50 **43 с** (мин. 42), Axe **36 с**, Lina **42 с**, Crystal Maiden 68 с, Sniper 131 с; идущий по прямой — почти так же (Juggernaut 48 с, Axe 31 с, Lina 39 с); идущий по кругу — 295–600 с. Причина — стартовый темп `spawn.base = 1.7` врага/с при ~0.8 убийства/с у стоящего мили: к 0:43 рядом 45 кобольдов (81% полученного урона). То есть на низшем ранге первая минута не прощает остановки вообще — см. предложение в T15.5.

### T15.2 — Клиент Аркады ✅ (2026-09-19)
- **Память гардероба (P1).** `glowScans` держал полный `ImageData` листа (39–48 МиБ у плотной арканы), `gemSheets` — полноразмерные canvas-копии, оба — `Map` без предела с ключом по имени листа, а имя композита меняется с каждой примеркой части: каждая смена оставляла +77–96 МиБ навсегда (ByteLru ограничивал только сами композиты). Теперь скан хранит только тон и долю, пиксели читаются заново на новую пару «лист + самоцвет», `gemSheets` — под байтовым LRU 192 МиБ. Заодно `rgbToHsv` без кортежа на пиксель и без списков тонов на миллионы элементов (второй проход вместо `push`), а при заданном `meta.glow` проход уточнения тона не выполняется.
- **Облик в дейлике и реплее.** `startDaily`/`startReplay` меняли героя, не пересчитывая `cosmetics.equipped` (`withHeroSkin`): эффекты шли из пресета прежнего героя, гардероб после выхода показывал одно, а правил другое. Тест в `arcadeStore.test.ts`.
- **Камера с запасом за край мира** (`CAM_OVERSCROLL = 0.3`, одна пара `camX/camY` вместо четырёх копий формулы): у границы герой не ближе 30% от кромки экрана; миникарта бледнеет, если герой под ней; рамка окна камеры на миникарте режется по миру.
- **Итог забега:** главные кнопки прилипают к низу прокрутки карточки.
- **Звук в скрытой вкладке:** `visibilitychange` гасит музыку, петли героя и Radiance сразу (rAF в скрытой вкладке стоит, и кадр, выключающий звук на паузе, не приходил — боевая тема играла и переключала треки).
- **Ввод:** кнопка пада, подтвердившая карточку/закрывшая окно, больше не уходит умением на следующем кадре (зажатые в меню молчат до отпускания); сочетания с Cmd/Ctrl/Alt не перехватываются (Cmd+R кастовал ульт, на macOS залипало направление); автоповтор удержанной клавиши не дёргает паузу/сборку.
- **Звук:** `Soundscape.play` возвращает результат `sfxSample` — пока сэмпл грузится, играет синтетика, а не тишина.
- **Кадр:** запасной LPC-облик врага собирается только когда нет листа Dota (было 300 объектов в кадр впустую); строки шрифта canvas кэшируются по (вес, размер).
- **CSS:** компактная витрина гардероба на узком экране не применялась вовсе (медиазапрос стоял выше базового правила той же специфичности).

### T15.6 — Сим Аркады ✅ (2026-09-19, a0.72.0 → a0.73.0)
Каждое исправление поведения — с тестом, красным на старом коде. Часть без смены чисел сверена ботом бит-в-бит (juggernaut/sniper/axe, `--seed gate --places all`, плюс bloodseeker и lone_druid после перф-правок).
- **Контроль чемпионов длился 1 тик (P1).** `capControl` резал стан до `ccCap` и ставил `ccResistUntil`, а на следующем тике ветка иммунитета обнуляла сам стан: Сатир, Кентавр, Некромант, Гром-голем, Страж и Охотник не оглушались вовсе, окно «Кентавр врезался в камень» (2.5 с, ×1.5 урона) жило один тик. Прежние тесты проверяли только `<= ccCap`, что верно и при нуле. `arcadeControlCap.test.ts`.
- **Некромант стрелял вдвое чаще конфига** (`shotCd` декрементировался дважды за тик: раз в 1.0 с вместо `shot.every` = 2.0 с). ⬜ **Решение владельца:** его калибровали на удвоенной стрельбе — теперь дальний урон вдвое ниже; при необходимости поднять `dmgMult` или `shot.every`.
- **Выбор целей без `isDormant`.** Один предикат `targetable(e)` для `eliteWithin`, `strongestWithin`, `cullTarget`, `chainLightning`, `spin`, клива, Spirits, `nearestEnemy`, `enemiesWithin`: автокаст сжигал кулдаун в спящего чемпиона и скрытого Охотника, статусы писались на спящего, луч выдавал позицию Охотника.
- **Culling Blade** добивает всегда (раньше урон `hp + 1` резался Клятвой охотника ×0.85 и щитом шамана ×0.3 — цель выживала, а короткий кулдаун и ускорение выдавались); слот — по ключу, а не буквой `.r`. **Потолок ранга в `applyOffer`** + `damageEnemy` отсекает NaN (путь «карта Q висит при Q=3 → Клятва даёт +1 → выбор даёт Q=5 → `value[5]` undefined → hp врага NaN, он бессмертен»). **Вампиризм и Death Pact** идут через `heal()` — порча «Увядание» на них теперь действует. **Отрицательная броня** — формула Dota вместо полюса при −16.7 (пять Mask of Madness давали урон ×25 или отрицательный). **Второй Рошан** не спавнится поверх живого первого (`secondRoshan.respawnGap` = 30 с после смерти). **Караван** не исчезает по `leaveAt` посреди сопровождения. **Контракт**: запасная награда золотом, если школьная карта не выпала. **Io**: индекс Tether ведётся за юнитом при уплотнении `pets`. **Цифры Chain Lightning** учитывают Сверхпроводник.
- **Препятствия на границе ячейки сетки.** `near()` отдаёт одну ячейку 256 px, а камень был зарегистрирован без радиуса сущности: у границы ячейки герой входил в камень до ~16 px и рывком выталкивался, рывок Кентавра проскакивал камень (главная контригра). Регистрация раздута на 40 px. ⬜ **Побочный эффект:** у 74 из 600 пар «сид × акт» сдвинулась раскладка мест (реплеи закрыты версией); откатывается одной правкой `mapgen.ts`, если раскладку нужно сохранить.
- **Бот `sim:arcade`.** Вечный цикл у заражённого пруда (воспроизведён: lone_druid `gate-13` full wilds стоял на тике 18318 — тот же механизм, что у известного «сид #8 идёт часами») и при одновременных лавке и токене нейтралки — устранены: сим отдаёт порядок окон через `activeModal()`, им пользуются и `step`, и бот; цикл прогона ограничен по `sim.steps` и падает с сидом/тиком/окном после 600 шагов без роста тика. Бот не считает спящих и скрытых врагов угрозой. Перф: O(1)-счётчики вместо `filter` каждый тик, волны Стража уплотняются на месте, `tickRupture` только у кита с Rupture. ⬜ `scripts/sim_kit.ts:34` по-прежнему шлёт act 1 всем окнам.
- **Бот до/после, 60 забегов, short, Herald (победы):** Juggernaut 23.3 → 31.7%, Sniper 65.0 → 61.7%, Axe 25.0 → 35.0% (смерть p50 186 → 415 с — Culling Blade наконец добивает). Наблюдения агента без правки: бот у ненужной добычи крутит «подобрать → закрыть» до истечения лута; `cullTarget` выбирает любого босса в радиусе без проверки порога.

### T15.7 — Клиент Аркады, второй проход ✅ (2026-09-19, вечер)
- **Индекс листов вместо пробы 404.** На проде каждый старт забега давал пять ошибок консоли (`tormentor`, `skeleton_warrior` в обоих наборах, `dota_px/terrain/water.webp`), гардероб с 708 слоями — десятки: отсутствие листа узнавалось по 404. `npm run gen:sheets-index` пишет `public/art/sprites/<набор>/index.json` (811 листов) и `features/arcade/sheetIndexRev.ts` (хеш — им запрос пробивает cache-first воркера); лист вне индекса — «нет» без запроса, без индекса — прежняя проба. `test/arcadeSheetIndex.test.ts` краснеет при забытой перегенерации; строка в `docs/arcade-dota-sprites.md` §5.
- **Сбой сети ≠ «листа нет»** (`netRetry.ts`): «нет» — только 404/410 или индекс; сетевая ошибка, 5xx и битое тело — три попытки с бэк-оффом, затем отложенный повтор через 8 с вместо `null` до перезагрузки страницы. То же для индексов звуков (`LazyJson`). **Гонка `setPixelSheets`** закрыта счётчиком поколения (тест загрузчика: на старом коде 8 из 10 красные).
- **Формы:** лист формы выбирается из кандидатов по факту существования (`<hero>@<skin>@meta` → `<hero>@meta`; стиль облика больше не попадает в имя листа формы) — превью формы не пустое, миниатюра «как у облика» показывает форму надетого; лист формы героя предзагружается со стартом забега (первая Метаморфоза TB/DK/LD/Alchemist — сразу в своей модели).
- **Гардероб:** часть, скин призыва и формы покупаются кнопкой, а не тычком по миниатюре (`pickThumb`, e2e); миниатюры без вечного rAF (один цикл на окно вместо до 13); вкладки — `tabpanel`, `aria-controls`, стрелки/Home/End, `aria-pressed` у 18 переключателей.
- **Кадр и звук:** сцена на паузе и на итоге не рисуется каждый кадр (~2100 → ~40 drawImage/с), `auraGeo()` только при свечении, градиент столба добычи кэшируется, `getBoundingClientRect` раз на жест стика, индекс обликов по герою вместо `COSMETICS.filter` на каждую из 126 карточек; озвучка героя грузится при старте забега, а не по клику на карточку. Геймпад — только `mapping === "standard"`, отключение сбрасывает удержанные кнопки.
- **Порядок:** `lpcHeroLook`, `heroPicker.ts` → `game/arcade/` (стор больше не импортирует `features/`), мёртвые ключи i18n и экспорты удалены.
- ⬜ Хвосты: LRU звуковых сэмплов (нужно вынести `ByteLru` из `features/` в общий слой — `ui/sound.ts` общий для всех режимов); `span role="button"` внутри `button` на карточке героя; `scanGlow`/`gemSheet` на главном потоке; повторная миграция косметики старым бандлом из SW.

### T15.8 — Сим Аркады, второй проход ✅ (2026-09-19, вечер, a0.73.0 → a0.74.0)
- **Сим быстрее на ~40%** (juggernaut full × 8: 13.4–13.8 → 8.1 с), вывод бота бит-в-бит на 16 связках герой × акт × ранг. Главное — не запросы, а **единая форма объектов `ENEMY_KINDS`**: разные наборы полей у литералов видов врагов выбивали inline-кэш движка на каждом `e.kind.*` в циклах (25–30% времени) — **новые виды заводить через `uniform()`**. Плюс плоские массивы ячеек сеток, грубый отсев по квадрату дистанции, `chainLightning` без `Set`, ленивый подсчёт соседей в `wantsCast`, один проход в залпах лучников. Не сделано намеренно: запросы через сетку (она строится до движения врагов — точного совпадения нет) и free-list пулов (`alive = false` пишут мимо сима 37 мест в тестах).
- **Мировые системы при живом Рошане и на передышке после разлома** (`tickWorld`): ранний выход глушил не только лес — не будились места, не поднимались скелеты кургана, лучники переставали стрелять с висящим телеграфом, не истекали события, не предлагался контракт; линии лучников переживали `riftPurge`. **Сроки событий в разломе** сдвигаются вместе с остановленными часами акта (сундук, торговец, руны, токен, добыча на земле, караван, мегакрипы).
- **Происхождение урона вместо вида эффекта** (`DmgOrigin`): Vampiric Spirit и Backstab срабатывают только на автоатаку героя — включая крит, который раньше обе пассивки терял, — и больше не включаются питомцами, иллюзиями и призывами; «Кровавик» лечит со всего, кроме автоатак и призывов, на 10% от нанесённого (тексты RU/EN обновлены). ⬜ **Решение владельца:** строгий вариант «только слоты Q/W/E/R» резал бы билды на школах и DoT — оставлен широкий, по комментарию автора.
- Окно уязвимости Стража переправы не пропускается станом на тике спада щита; штраф Клятвы охотника снимается со смертью Сатира (закрытие контракта очисткой лагеря оставлено — T13.50); плановая лавка и лавка каравана не затирают друг друга (подарок каравана терялся в `short`); станы Thunderclap/Headshot/Time Lock/подъёма/медведя уважают «статусы короче» ранга Divine (`statusSec` внутри `stun()`); Aegis с Рошана лежит на земле, пока воскрешение уже есть, «Феникс» в это время не предлагается; распространение яда со смерти — очередью (Пандемия + Дистилляция на 120 врагах: глубина вложенности 61 → 2); Плазма не поджигает неуязвимого чемпиона.
- **Бот:** не подбирает повторно добычу, от которой отказался (крутил «подобрать → закрыть» на месте до истечения лута) — забеги `full`, упиравшиеся в потолок 1560 с, теперь заканчиваются: **метрики бота на `full` до этой правки занижены** (zeus 33 → 50%, venomancer 0 → 50%, juggernaut 0 → 12.5%, n=6–8). `sim_kit.ts` отвечает окнам по `activeModal()`. `short`, 60 забегов: Juggernaut 31.7 → 31.7%, Sniper 61.7 → 58.3%, Axe 35.0 → 38.3% — в пределах шума.
- ⬜ Хвосты: похожая на яд рекурсия у `rad_blast`; второй Рошан перезаписывает точку неподобранного Aegis.

### T15.9 — Конвейер слоёв ✅ (2026-09-19, вечер)
`dota_part_layers.mts`: основа арканы ищется по Valve-имени героя из самих частей (`index.bases` ключуется `nevermore`/`windrunner`, а читался по `shadow_fiend`/`windranger` — подмены моделей арканы SF (3) и Windranger (59) молча не применились бы); слои проверяются до публикации `+body`; любой отказ — ненулевой код выхода; `dota_part_wave.sh` не удаляет сырьё при упавшем build. ⬜ Осталось: базовая часть того же слота не входит в слой сета (`axe_ponytail`), список `TOP`/`OVER_BACK` в двух местах, `manRows` в тесте без файла даёт зелёный тест без ассертов.

### T15.3 — Вне Аркады ✅ (2026-09-19)
- **Арена не стирает чужой соло-сейв (P1).** `reset()` и `finishTournament()` звали `clearSavedRun()` без проверки режима, хотя `persist()` Арену нарочно пропускает: финиш или выход из турнира Арены молча удалял незаконченный Roguelite/Classic-забег. Тест в `runPersist.test.ts`.
- **SW: смешанное ведро данных.** `downloadBucket` брал файлы обычным HTTP-кэшем при `no-store`-манифесте: деплой данных между загрузкой страницы и докачкой клал в ведро нового хеша файлы старой версии (Pages отдаёт `max-age=600`) с маркером полноты. Теперь `cache: "no-cache"` — ревалидация по ETag, неизменённое возвращается 304.
- **data-refresh.** (1) Факт по GitHub API: бот-коммиты данных 14, 15, 17 и 18 сентября не дали **ни одного** прогона CI — push дефолтным `GITHUB_TOKEN` не триггерит `on: push`, датасет доезжал до Pages только со следующим человеческим пушем; теперь после push — `gh workflow run ci.yml --ref main` (`actions: write`). (2) raw-кэш сохраняется и при красном джобе (`cache/restore` + `cache/save` с `if: always()`; прогон 09-16 упал — скачанное за него пропало). (3) `timeout-minutes: 120`. (4) входы `workflow_dispatch` — через `env`, не подстановкой в shell. ⬜ Проверить по первому крон-прогону: CI стартует от `workflow_dispatch`, деплой Pages проходит.
- **Сервер комнат (Арена/Дуэль).** (1) **Порядок relay (P1):** `seq` ставился под `rooms.mu`, а рассылка шла отдельно под `hub.mu` — при одновременных пиках (в Арене они одновременные по дизайну) seq 6 мог уйти раньше seq 5, клиенты отбрасывают `seq <= lastSeq`, ход терялся у всех онлайн, а в серверном логе оставался. Теперь штамп и постановка в outbox — в одной критической секции (`AppendRelay(..., deliver)`); тесты с конкурентными отправителями на старом коде падали 3 из 3. (2) Имя участника: `TrimSpace` + 32 руны, `SetReadLimit(16 КиБ)`. (3) Отклонённый relay сообщается отправителю: `error relay_rejected` / `relay_log_full` (нефатальные). (4) Потолки: 1024 записи и 256 КиБ на лог комнаты, 500 комнат → `503 rooms_limit`. (5) Дедлайны записи welcome/`relay_log`, конец сессии в `defer`. (6) Сейв > 1 МиБ — `413 too_large` вместо `400 bad_json`. ⬜ Клиент: `arenaStore`/`duelStore` на любой `error` ставят `status: "error"` — для нефатальных кодов нужен свой текст («лог комнаты переполнен — создайте новую»), комментарий в `data/api/arena.ts` «сокет закроется следом» для них неверен.
- **Дуэль:** незнакомый код ошибки показывал сырой ключ «duel.error.internal» (`t()` возвращает ключ, а не пустую строку) — теперь читается как сетевой сбой, как в лобби Арены.

### T15.4 — Кандидаты, не взятые в этот проход ⬜ (перепроверять по коду перед работой)
- **Клиент Аркады** (часть закрыта вторым проходом — см. T15.7; актуальны только его ⬜ и то, чего там нет). Превью формы пустое, когда у облика нет своего листа формы (`HeroWardrobe.tsx:264`), и листы форм `<hero>@meta` не предзагружаются — первая Метаморфоза идёт в чужой модели; `scanGlow`/`gemSheet` всё ещё на главном потоке (проставить `meta.glow` всем арканам или считать в `preloadArcadeArt`); кэш звуковых сэмплов без вытеснения, озвучка грузится по клику на карточку героя (~5.8 МБ PCM на героя); единичный сбой `fetch` индекса звуков/листа запоминается как «нет» до перезагрузки; гонка `setPixelSheets` при смене DPR (нужен счётчик поколения); повторная миграция косметики v1→v2 старым бандлом из SW (маркер версии отдельным ключом) и непокрытый тестом путь чтения; покупка одним кликом по миниатюре части/призыва/формы (незаметно при `SHARD_PRICE = 0`); `auraGeo()` без надетого свечения, градиент на каждый редкий предмет на земле, `getComputedStyle` в каждом кадре превью, сцена рисуется каждый кадр на паузе, `COSMETICS.filter` на каждую из 126 карточек при каждой букве поиска; любой геймпад без `mapping === "standard"`; a11y вкладок гардероба (`tabpanel`, `aria-controls`, стрелки) и `span role="button"` внутри `button`; два разных `heroLook` (LPC и косметика) под одним именем; `state/arcadeStore.ts` импортирует `features/arcade/heroPicker.ts`; мёртвые ключи `arcade.wardrobe.lead`, `arcade.cosmetics.slot.{skin,summon,form}` и экспорты `compositeCacheStats`, `COMPOSITE_BUDGET`, `COSMETICS_VERSION`. Статус T14.8: строки смерти по направлениям — **закрыто** (все 811 мет с `death.dirs = 1`); вечный rAF миниатюр стал хуже (до 13 миниатюр на вкладке у DK); `index.json` мет листов стал нужнее (708 слоёв, двухступенчатая проба px2 → px с 404).
- **Конвейер слоёв** (первые два пункта закрыты — T15.9). `dota_part_layers.mts`: индекс `bases` ключуется npc-именем (`nevermore`, `windrunner`), а читается по нашему id — подмены `*_arcana_refit` молча не применятся; упавший слой не роняет прогон (`+body` публикуется до проверки, код выхода 0, волна удаляет сырьё); базовая часть того же слота не входит в слой сета (`axe_ponytail`); `TOP` в скрипте и `OVER_BACK` в рантайме — один список в двух местах; `manRows` в тесте возвращает `[]` без файла — зелёный тест без ассертов.
- **Драфт/сторы/режимы.** После турнира Арены «Сменить» строит тот же турнир заново (нужна «В лобби» с `leaveRoom()`), «Ещё раз» в Арене запускает соло-драфт под режимом `arena`; нативный Back в Telegram при активной комнате/забеге Аркады выходит без confirm и без `leaveRoom` (`state/navigation.ts:36`); **TMA: таймаут CloudStorage → следующая запись затирает карьеру/зал/мету** (`careerStore.ts:345`, `persist.ts:102`: отличать «облако не ответило» от «значения нет»); записи одного ключа в `persist` не сериализованы (хвост короткой записи удаляет чанки длинной); `await ensureSquadSynergy()` без catch и без защиты от двойного клика (две карьеры Менеджера); в `careerRunId` нет stakes и `cheatMode`; ссылка с `m:"manager"` декодируется; клиент не знает поддерживаемую версию схемы, плашка обновления не видна при упавшей загрузке; блокирующий `@import` Google Fonts в `base.css`; у `ensure-offline` нет single-flight, у `navigationFirst` — таймаута, `cacheFirst` кэширует `/api` на nginx-стенде; таймер раунда Арены/хода Дуэли живёт только у хоста/актора.
- **Данные/Go/CI.** Срез `squadSynergy` по игрокам из `players.json` (23 193 → 6 891 пара, 2.37 → 0.45 МБ) и career (5249 → 729 ключей); клиент не читает `teammates.json` (792 КБ обязательного файла); `arcade.*` — 2228 из 3349 ключей i18n, ~−250 КБ сырого из стартового чанка; отложенный emit зелёным джобом (падать, если `builtAt` старше N дней); один «ядовитый» матч валит конвейер каждый прогон; sanity перед авто-коммитом — только `packs.length > 0`; `*url.Error` несёт `api_key` в лог; emit до проверки схемой; гейт деплоя сервера без Postgres; `concurrency` в `ci.yml` (осторожно с отменой посреди deploy-pages).

### T15.5 — Предложения по Аркаде (решения владельца, не реализовано)
- ✅ **Закрыто T18.2 (2026-09-26).** **Первая минута на Herald.** Факт из T15.1: остановка = смерть к 0:36–0:43. Вариант: разгон `spawn.base` 0.8 → 1.7 за первые 45–60 с только на Herald/Guardian (ранговое правило, а не общий конфиг) — старшие ранги и калибровка бота на них не сдвигаются; Herald перемерить ботом на двух базах по 30.
- ✅ **Закрыто T18.2 (2026-09-26).** **Порча проклятого сундука обходится**: «Оставить» кладёт предмет к ногам чистым, следующий подбор даёт +1 редкость без порчи (закреплено тестом `arcadePond.test.ts:60` и текстом кнопки); бот делает это автоматически — метрики порч недействительны. Варианты: порча едет с предметом на землю либо берётся при вскрытии.
- ✅ **Закрыто T16.4 (2026-09-25).** **Рошан:** удар по телеграфу — голая константа (ранговый `dmgMult` до ×2.95 и второй Рошан на него не действуют); таймер ярости в River идёт, пока Рошан ждёт в яме (пришедший после 9:00 сразу получает ×3); у Рошана нет капа контроля — Time Lock под Frenzy и Call дают почти перманентный стан главного босса.
- ✅ **Закрыто T16.3 (2026-09-25).** **Мёртвые ранги у 35 умений** (20× `armor_buff` всегда +25 брони, 6× `dash` и 9× `mass_freeze` с нулевым `value`, включая Chronosphere и Global Silence): карточки прокачки предлагаются, ранги 2–4 ничего не меняют. Io стартует с Tether, которому не к кому привязаться.
- ✅ **Закрыто T18.2 (2026-09-26).** **Экономика лута:** шаман и знаменосец (`elite: true`) гарантированно роняют экипировку — в композиции `siege` это ~2 предмета в минуту против сундука раз в 150 с; редкость карт Зверинца не даёт силы (читается `rank`, не `power`) — награда Некроманта «три exotic-карты» равна обычным; `hyb_venom_fire` взрывается на каждое наложение яда без кулдауна; `stats.cleave` и Battle Fury мертвы у дальнобойных; второй BKB бесполезен; чемпионы спавнятся с множителями нулевой минуты при наградах exotic/arcana.
- **Ранний `return` в `spawnTick` при живом Рошане** глушит не только лес: разбуженность мест, залпы лучников (телеграф висит), сроки событий, предложение контракта.
- **UX:** тёмные силуэты некупленных обликов в гардеробе при цене 0 — не видно, что берёшь; карточки уровня разной высоты «пляшут» по вертикали.

## M16 — Новые механики Аркады 2026-09-25: Blink, благословения «Рывок», живые ранги, Рошан

Запрос владельца: «проверь Аркаду — что ещё улучшить/добавить; только игровые механики, без скинов». Ревизия показала главный пробел ядра боя относительно референса: из семи типов благословений DMD (Attack, Dash, Strike, Cast, Power, Summon, Passive — бриф §1) у нас не было **Dash** — у героя нет уклонения, и все телеграфы (удар Рошана, залп лучников, рывок Кентавра, зоны Гром-голема) обегались только ногами. Плюс висевшие в T15.5 игровые дефекты. `ARCADE_CONFIG_VERSION` a0.74.0 → **a0.75.0** (старые реплеи закрыты версией).

### T16.1 — Blink: рывок героя ✅ (2026-09-25)
- **Механика** (`ARCADE.blink`, `sim.ts` `blink()`): у каждого героя, как Blink Dagger. Мгновенный рывок на 150 px по направлению движения (стоишь — куда смотришь), 0.3 с неуязвимости, 1 заряд, заряд копится **6 с** × перезарядка героя (Octarine, Arcane Aura — как предметы в Dota), между рывками ≥ 0.25 с. Оглушённый не блинкует; «Безмолвие» разлома — блинкует (предмет, не умение); в разломе точка прибытия не выходит за арену (рывок не проваливает испытание); камни и деревья выталкивают. Ввод — бит `BLINK_MASK = 32` в `cast`: input-лог и реплеи прежнего формата.
- **Управление:** Shift (по нажатию, автоповтор не тратит заряды), L2 на геймпаде (по фронту), кнопка в HUD с иконкой Blink Dagger, зарядами и откатом. Подсказки `arcade.controls` RU/EN обновлены.
- **Отклик:** вспышка `fx "blink"` (свечение ухода/прибытия + пунктир, токен `--arcade-blink`), мерцание неуязвимости, звук Blink из клиента Dota (`fx.blink`, у легендарок — свои: `blinkOver/Swift/Arcane`; `scripts/dota_sfx_pack.py`), счётчик `events.blinks`.
- **Калибровка ботом** (`sim:arcade` уклоняется Blink от удара Рошана по телеграфу и отрывается от толпы при HP < 40%; `--noblink` — прежний бот; `--seed gate`, Herald): short, 60 забегов — Juggernaut 16.7 → **41.7%**, Sniper 41.7 → **53.3%**, Axe 16.7 → **38.3%**; full, 36 забегов — Juggernaut 13.9 → **16.7%** (дошёл до Рошана 36 → 61%). A/B перезарядки на Juggernaut short: 3.5 с → 55.0%, 6 с → 41.7%, 8 с → 31.7%; без неуязвимости (3.5 с) — 46.7%: основную силу даёт сам отрыв. Выбрано 6 с — кнопка спасения, а не спам-уклонение; полосы 30–50% (short) и 15–25% (full) соблюдены. Archon (ранг 15) short Juggernaut: 0 → 2.5%.
- **Тесты:** `arcadeBlink.test.ts` (рывок, заряды и перезарядка, неуязвимость, стан, разлом с «Безмолвием», благословения, легендарки), e2e `arcade.spec.ts` «Blink» (Shift и кнопка тратят заряд, откат виден, кнопки HUD внутри сцены на desktop и mobile).

### T16.2 — Благословения типа «Рывок» (Dash DMD) ✅ (2026-09-25)
- По одному на школу, срабатывают на каждом Blink (`blinkEffects`), числа на карточке — `upgradeFigures`: **Огненный след** (Radiance: вспышка пламени в точке ухода — урон и горение), **Морозная вспышка** (Skadi: ледяная нова в точке прибытия — урон и замедление до 60%, копит стаки «Заморозки»), **Грозовой шаг** (Maelstrom: молния в ближайшего с цепью), **Стая следом** (Beast: питомцы прыгают к точке прибытия, бьют сразу и 3 с сильнее на 30% за ранг; нужен волк или медведь), **Ядовитый шлейф** (Venom: облако — два стака яда в точке ухода).
- Огонь/холод/молния/яд — **источники статуса**: открывают модификаторы своей школы (`requires` у rad_blast/rad_inferno, ska_snap/ska_shatter, mae_overcharge/mae_mjollnir, ven_spread/ven_virulence).
- Три нейтральные легендарки — апгрейды Blink Dagger из Dota: **Overwhelming Blink** (удар 60 + 6 за уровень героя и замедление 50% на 3 с в точке прибытия), **Swift Blink** (3 с удары на 30% чаще и бег на 25% быстрее), **Arcane Blink** (второй заряд, заряды копятся вдвое быстрее). Иконки — `gen:art` (слаг `blink` для HUD добавлен в зеркало) и `dota_item_icons.sh` из vpk.
- ⬜ Бот `--noblink` берёт карты «Рывка» и не пользуется ими (−5…−10 п.п. к его прежним цифрам) — для A/B «без механики» учитывать это.

### T16.3 — Живые ранги умений и старт Io ✅ (2026-09-25, закрывает пункт T15.5)
- 20× `armor_buff` — броня по рангу **+20/26/32/38** вместо +25 на любом ранге (Berserker's Call оставлен +25: `armorBuffAmt`); тексты RU/EN — «+20–38 брони… растёт с рангом».
- Умения без числа (6× `dash`: Blink AM/QoP, Time Walk, Leap, Teleportation, Relocate; 8× `mass_freeze`: Chronosphere, Global Silence, Kinetic Field, Power Cogs, Song of the Siren…) — `abilityRankScale`: каждый ранг после первого −10% перезарядки и +15% длительности контроля.
- Карточка уровня умения показывает «сейчас → после» для брони, перезарядки и длительности (`abilityRankFigures`, ключи `arcade.fig.ab*`); кулдаун-заливка кнопки в HUD учитывает ранг.
- Io стартует со Spirits (`HeroDef.startKey`): Tether без своего юнита не кастуется, старт был голым.
- Тест `arcadeAbilityRanks.test.ts`: по всем героям на каждом ранге что-то растёт (значение, число целей, перезарядка или длительность), броня режет урон сильнее, Blink AM и Chronosphere по формуле.

### T16.4 — Рошан: предел контроля, удар по рангу, ярость в яме ✅ (2026-09-25, закрывает пункт T15.5)
- Контроль Рошана — `capControl` как у чемпионов: стан/заморозка ≤ 1.2 с, затем 3 с иммунитета (`ARCADE.boss.ccCap/ccResist`).
- Удар по телеграфу × `e.dmg / kind.dmg` — тот же множитель, что у контакта: ранг (до ×2.95), руна щедрости, второй Рошан. На Herald без руны — прежние 92.
- River: пока герой вне ямы, часы ярости стоят (раньше пришедший после 9:00 сразу получал ×3).
- Тест `arcadeRoshanControl.test.ts`. Бот: кап контроля почти не влияет на победы Juggernaut в полном акте (13.9% без капа против 16.7%).

### T16.5 — HUD на телефоне и миникарта ✅ (2026-09-25)
- Семь кнопок в строку на 375 px не влезали (R уезжал за край сцены; и до Blink обрезался на ~22 px). На ≤ 680 px — двухрядный кластер справа: сверху «Удар» и Blink под большой палец, снизу Q W E R; полосы HP/XP — строкой выше (`data-slot` у кнопок, `arcade.css`).
- Миникарта встаёт над нижней панелью HUD по её реальной высоте (`hudInset`, ResizeObserver панели): раньше на ширине 720–1000 px кнопка R ложилась на угол миникарты, на телефоне — полоса HP.
- Ввод, нажатый при открытом окне уровня/лавки/паузе, больше не копится (`pollPad(menu)` чистит маску): Shift на карточке давал внезапный Blink после выбора, цифра — каст умения.

## M17 — Серия убийств и элита с аффиксами 2026-09-26

Запрос владельца: «проверь ещё раз Аркаду — что можно добавить». Инвентаризация после M16: пробелы — награда за чистую игру, разнообразие обычных боёв и смысл золота в конце акта. Выбраны владельцем: серия с комментатором и аффиксы элит (из предложенных также выкуп и доделки T15.5 — не взяты). Отброшено: барьерные волны DMD («убей элиту, чтобы выйти» — правила генерации запрещают закрывать все выходы, аудит 2026-09-08); второй активный предмет (HUD телефона). Отложено: Aghanim's с Рошана/Tormentor, «сигнатура про-игрока» из брифа (данные меняются ежедневно, реплеи и дейлик — нет). `ARCADE_CONFIG_VERSION` a0.75.0 → **a0.76.0**.

### T17.1 — Серия убийств с комментатором Dota ✅ (2026-09-26)
- **Механика** (`ARCADE.streak`, `Player.streak/bestStreak`): убийства подряд, пока ни один удар не дошёл до HP (щит руны и неуязвимость Blink серию не рвут; `damagePlayer` теперь возвращает снятый HP). Ступени Killing Spree / Dominating / Mega Kill / Unstoppable / Wicked Sick / Monster Kill / Godlike / Beyond Godlike на 10/20/35/50/75/100/150/200; каждая — +10% золота за убийство (баунти). Пороги — по пробе ботом: медианный максимум серии за забег у мили ≈ 90–100, у стрелков и магов 240–350.
- **Отклик:** значок у часов (виден и на телефоне, «выпрыгивает» на новой ступени, с Monster Kill — красный), голос комментатора по умолчанию из клиента Dota на каждой ступени и First Blood на первом убийстве (`ui.streak1..8`, `ui.firstBlood`, `scripts/dota_sfx_pack.py`), бамп HUD; «Лучшая серия» в итоге забега (`ArcadeOutcome.bestStreak`).

### T17.2 — Элита с аффиксами ✅ (2026-09-26)
- **Механика** (`ARCADE.affix`, `Enemy.affix` — битовая маска `AFFIX` в `content/enemies.ts`): с 3:00 раз в 45 с по часам акта обычный враг пула минуты приходит усиленным (HP ×4, опыт ×5, золото ×5, 35% — предмет) с одним аффиксом, с 10-й минуты или с ранга Legend (20) — с двумя. Аффиксы: **Быстрый** (скорость ×1.4), **Вампир** (лечится вчетверо от урона своего удара), **Взрывной** (после смерти круг-телеграф 1.1 с, затем урон ×2.5 внутри), **Морозный** (удар замедляет героя на 30% на 1.5 с), **Раскол** (после смерти двое того же вида по 30% HP без аффиксов).
- Для пассивок героя, автокаста по элите (`eliteWithin`), счётчика элит и миникарты — как элита. Видна кольцом цвета аффикса и точками над полосой HP; рядом — строка HUD «Элита: Быстрый · Взрывной» с полосой HP (как у босса).
- **Найдено тестом:** спавн «Раскола» прямо в `killEnemy` получал из пула объект умирающего врага, и остаток `killEnemy` (награда, пассивки) читал ребёнка — дети встают в очередь и спавнятся в конце тика (`tickBlasts`), как распространение яда.
- **Бот** считает элиту с аффиксами опасной как элиту и выходит из круга «Взрывного» (не успевает — Blink).
- **Калибровка** (Herald, бот): short, 60 забегов, `--seed gate` — Juggernaut 41.7 → 41.7%, Axe 38.3 → 43.3%, Sniper 53.3 → 65.0%; `--seed sim` — Juggernaut 45.0 → 46.7%, Sniper 63.3 → 56.7%, Crystal Maiden 48.3 → 61.7%; full Juggernaut, 100 забегов — 15.0 → 19.0%. В среднем около +4 п.п., знак у одного героя меняется между базами сидов — в пределах шума. A/B на Sniper (gate): без аффиксов — 53.3% (тот же поток Rng, что у базы), без баунти серии — 63.3%, без наград элит — 60.0%. Legend (ранг 20) short Juggernaut: 0 → 0% (дошёл до Рошана 5 → 10%).
- **Тесты:** `arcadeStreakAffix.test.ts` (ступени и баунти серии, обрыв ударом/щит/неуязвимость, расписание и число аффиксов по рангу, Быстрый, Вампир и Морозный, Взрывной внутри/вне круга, Раскол и награды).

## M18 — Выкуп и доделки баланса T15.5 2026-09-26

Запрос владельца: «сделай выкуп и доделки баланса T15.5» (кандидаты, не взятые в M17). `ARCADE_CONFIG_VERSION` a0.76.0 → **a0.77.0**. UX-пункты T15.5 (тёмные силуэты в гардеробе, «пляшущие» карточки уровня) — не баланс, не брались.

### T18.1 — Выкуп (Buyback) ✅ (2026-09-26)
- **Механика** (`ARCADE.buyback`, окно сима `buyback` — первым в `activeModal`): смерть без Aegis и Reincarnation, пока лимит не исчерпан и золота хватает, — окно «Выкупиться — N золота / Сдаться», мир стоит. Цена — большее из `100 + 25 × минута` и 50% золота на руках (как в Dota, где цена растёт с ценностью героя); один выкуп за забег (аналог перезарядки); встаёт с половиной HP и 1.5 с неуязвимости, **без** толчка и стана толпы — выбираться самому (Blink). Для отметки «без единой смерти» выкуп — воскрешение (`revived`), в итоге забега — «Выкупы ×N». Звук — стингер Dota `buy_back` (`ui.buyback`).
- **Калибровка** (бот выкупается всегда; `--nobuyback` — без выкупа). Бот копит золото (к смерти медиана 1722 у Juggernaut, 4071 у Sniper при двух лавках разминки), поэтому цена почти не ограничивает: A/B «50% золота / 50% HP» против «75% / 30%» и с толчком против без — одинаково, решает сам подъём. Итог a0.76 → a0.77 (Herald): short `gate` Juggernaut 41.7 → 63.3%, Sniper 65.0 → 76.7%; short `sim` Sniper 56.7 → 68.3%, Axe 53.3 → 73.3%; full Juggernaut (48) 16.7 → 22.9%; Legend (20) short Juggernaut 0 → 2.5%. ⬜ **Решение владельца:** разминка на Herald теперь выше полосы 30–50% (полный акт — в полосе 15–25%). Рычаги: `maxPerAct` (0 — выкл.), `goldShare`/`base`/`perMin`, `hpFrac`, `push`.
- UI: окно выкупа, строка итога; ввод окна идёт через `shopAct` стора (в его список окон выкуп добавлен — без этого кнопка молчала, поймано вживую). Тесты: `arcadeBuybackBalance.test.ts`, e2e «выкуп поднимает героя…».

### T18.2 — Доделки баланса T15.5 ✅ (2026-09-26, закрывает T15.5)
- **Мягкий старт Herald/Guardian** (ранговое правило `gentleStart`, `ARCADE.spawn.gentleStart`): база спавна 0.8 → 1.7 за первую минуту; опыт и золото убийств в этом окне ×(1.7 / текущая база) — без компенсации бот приходил к Рошану слабее (Sniper 63.3 → 45.0%). Стоящий новичок (проба, 12 сидов, медиана): Juggernaut 44 → 62 с, Axe 40 → 60 с, Lina 42 → 66 с, Crystal Maiden 62 → 69 с. Старшие ранги не меняются; строка правила в выборе ранга.
- **Порча едет с предметом:** «Оставить» кладёт проклятый предмет на землю вместе с порчей (`groundLoot[].curse`) — подобрать снова = тот же выбор; пока на герое другая порча, взять нельзя (кнопки выключены, объяснение). Раньше предмет становился чистым и давал +1 редкость даром — бот делал это всегда.
- **Лут поддержки:** шаман и знаменосец роняют предмет с шансом 20% (`loot.supportChance`), а не всегда.
- **Зверинец:** урон волка и медведя — от силы апгрейда (ранги × множитель редкости), как у остальных школ; «три exotic-карты» Некроманта теперь сильнее обычных; цифры карточек и тексты — «× редкость карты».
- **Яд + огонь** (`hyb_venom_fire`): взрыв не чаще раза в секунду на цель (`Enemy.fireBlastAt`).
- **Клив у дальнобойных:** Battle Fury и клив-аффиксы у стрелков — всплеск: `stats.cleave` соседей цели в `cleaveRadius`, по 60% удара (`rangedSplash`).
- **BKB:** лавка не предлагает второй экземпляр (`ArcadeItemDef.unique`).
- **Чемпионы мест** при первом пробуждении подтягиваются к минуте: HP и урон — как у леса, не дальше `ARCADE.champion.scaleCapMin = 6`.
- Без выкупа правки баланса — мягкий минус около 5 п.п. (закрыты лазейки бота): short `gate` Juggernaut 41.7 → 43.3%, Sniper 65.0 → 58.3%; `sim` Sniper 56.7 → 48.3%, Axe 53.3 → 45.0%.
- Тесты: `arcadeBuybackBalance.test.ts` (по пункту на каждую правку), обновлены `arcadePond` (порча на предмете), `arcadeVenom` (перезарядка взрыва), `arcadeAutoCast` (Crusader: мягкий старт разрежает лес), `arcadeStreakAffix` (золото без компенсации старта).
