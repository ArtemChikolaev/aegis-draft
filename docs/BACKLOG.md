...
### T13.28 — Аркана Drow не собирается конвейером: облик выключен (владелец 2026-09-06: «у drow ranger опять тоже проблемы с арканой») 🟨
...
- **Пока что.** Строка `skin_drow_arcana` в `content/cosmetics.ts` закомментирована (и убрана из `SHEET_STYLE_SKINS`), листы `drow_ranger@arcana*` остаются в репозитории. Строки i18n оставлены — вернутся вместе с обликом.
