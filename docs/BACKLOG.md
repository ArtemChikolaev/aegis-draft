...
- **T7.9 — Края шкалы OVR читаются с карточки: фольга и «погасшая» 🟨.** 2026-07-18. Тонировки по тиру не хватало, чтобы выцепить взглядом лучшего и худшего в паке — все пять карточек читались как «примерно одно и то же».
  - **Пороги — от данных, не на глаз.** Замер по `packs.json` (7075 значений OVR, 54–99, медиана 74): `elite ≥88` = **4.8%**, `liability <60` = **1.9%**. Оба края редкие ⇒ эффект становится событием («редкий пул»), а не фоном. Специально НЕ вешали эффект на существующий `weak`: он один покрывает **29%** игроков, там это был бы шум. Ради этого в `playerOvrTier` добавлена шестая ступень `liability` — тот же домен «ранг игрока», просто нижняя ступень.
  ...
  - **Тесты:** границы всех шести тиров + проверка, что края шкалы остаются редкими на реальном датасете (если пайплайн сдвинет шкалу OVR, эффекты расползутся на полпака — упадёт здесь).
...
- **T7.10 — Справочник и настройки: страницы вне забега + хардкор 🟨.** Начато 2026-07-18. Заход 1 из 3 (дальше — страница героев, затем паутина тиммейтов).
  ...
  - **Навигация без роутера.** `state/shellStore.ts` (`AppView` + `location.hash` + `pushState`): отдельно от `runStore`, потому что уход на страницу — не событие забега. Кнопка «назад» браузера работает (на телефоне это единственный выход со страницы), неизвестный хеш = игра, а не 404.
  ...
  - **Осталось:** заход 3 — паутина тиммейтов (player-centric: игрок в центре, клик по соседу перецентровывает, фильтр окна). Обдумать: показывать ли на странице героя топ-игроков — это обратный лук-ап к тому, что закрывает хардкор, и его стоит решать продуктово, а не по умолчанию.
