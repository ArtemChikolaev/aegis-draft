...
- **T9.6 — Сейвы в TMA ⬜.** В webview `localStorage` живёт по своим правилам и может очищаться, а у нас сейв local-first. Два пути: облачные сейвы T8.4 либо `WebApp.CloudStorage` (лимиты порядка 1024 ключей × 4KB — наш `config+seed+лог действий` укладывается) без всякого сервера. **Решить, что берём.** **Deps:** T9.4; альтернативно T8.4.
- **T9.7 — Оформление бота в BotFather ⬜.** Аватар — `web/public/bot-avatar.png` (512×512, уже есть, рендерится из `favicon.svg`). Нужно доделать: **обложка Direct Link 640×360** (или GIF 320×180 / 640×360 / 960×540), короткое описание для пустого чата, полное описание, список команд, кнопка меню на TMA. **DoD:** обложка рендерится из того же знака, а не рисуется отдельно — один источник правды, как у аватара. **Deps:** T9.1.
