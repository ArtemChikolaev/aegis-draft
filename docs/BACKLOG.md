...
*(R13.1 и R13.2 закрыты 2026-07-30; R13.3–R13.5 открыты)*
...
- 🟨 **R13.3 — два уровня информации: карточка = решение, инспектор = математика.** *(рынок сделан
  2026-07-30; награды и Build — следующим шагом)* Это прямой ответ на
  ...
  - ✅ **Сделано для карточек рынка (2026-07-30).** Новый чистый слой `features/run/campPresentation.ts`
    ...
    объяснить). Полный `до → после` по слагаемым уехал в `OfferInspector` поверх существующего
    `ui/Modal` — второй механизм «показать подробности, не уводя с экрана» рядом с `HeroTagInspector`
    заводить не стали.
  ...
  - 🐛 **Попутно поймана своя же ошибка класса `R13.2`:** дельту карточки я сперва подписал
    `camp.power` («Run power»), хотя она измеряет сдвиг **Team OVR** — множители предметов в неё не
    входят. Подпись обязана называть ту величину, которую показывает; исправлено до коммита.
  ...
  - 🔁 **Взаимодействие с карточкой пересмотрено (решение 2026-07-31).** Сделанный «?» на цифре
    дельты, открывающий модалку с разбором по слагаемым, — у́же требования. Нужно: **кликается вся
    карточка целиком**, и она разворачивается в себя же большего размера со всей информацией.
    ...
    (подъём + масштаб), а не перекомпоновка ряда. `OfferInspector` в нынешнем виде будет **заменён**,
    а не дополнен; `campPresentation` и фиксированные размеры нужны в любом варианте и остаются.
...
- **R13.4 — разделение `CampScreen` — ЭТО И ЕСТЬ ответ на «страница как инвентарь в ММО», а не
  побочная поддерживаемость (переоценено 2026-07-31).** Верхняя навигация
  `Reward ✓ · Market · Build 2/3 · Preparation 1/2`, ниже — только АКТИВНЫЙ раздел. Завершённые
  сворачиваются в строку (`Reward: +6 gold ✓`). `Next stage` закрепляется внизу экрана, а не живёт в
  конце ленты. Постоянные пояснения-абзацы («Raises an active hero's rarity…») уходят в тултип по
...
2. 🟨 **R13.3** — компактные карточки. Сделано наполовину: высота снята, но количество карточек на
   экране не изменилось, а взаимодействие переделывается на «вся карточка → оверлей».
3. **R13.4 ПЕРВЫМ из оставшегося** — навигация по разделам. Даёт ~80% эффекта вместе с оверлеем
   карточки; без него любая работа над высотой карточек упирается в ту же ленту.
3. **Анимации** по таймингам из §14 плана (они разумные и совпадают с нашим правилом
...
4. **R13.5 `Charged`** — одна Edition, замеренная симулятором, прежде чем добавлять остальные.
5. **Решение по Stakes vs Contracts/мутаторы**, затем `R12.6` целиком.
