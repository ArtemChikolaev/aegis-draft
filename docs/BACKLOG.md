...
- **TREF1 — Драфт героев из паков (P1).** У референса пак = 5 игроков **+ 5 героев**; герои драфтятся и привязываются к игрокам, формируя Hero Synergy (половина цикла). У нас героев не драфтишь — `heroPool` авто-копит signature-героев. **Цель:** решить и (если да) реализовать явный hero-draft. **Файлы:** `web/src/game/{engine,packs}.ts`, `web/src/ui/DraftScreen.tsx`, контракт паков. **Скиллы:** `reference-parity-audit`, `scoring-model`, `plan-first-communication`. **DoD:** пики героев меняют synergy интерактивно; покрыто тестом; либо зафиксировано как intentional-divergence в PRD. **Deps:** PRD §10-G. ⬜
...
- **TREF6 — Chemistry не работает + нельзя взять тиммейта (P1).** Химия всегда +0.0: датасет = 1 событие (команда встречается 1 раз, `squadSynergy` только внутрикомандный), а `engine.usedTeamIds` исключает выбранную команду из будущих паков. **Цель:** (а) **богатые multi-event данные** (расширить сбор пайплайна: несколько турниров/годы, чтобы команды/игроки повторялись и `squadSynergy`/`teammates` были плотными); (б) убрать вечное исключение команды, ввести `usedPlayers` (не дубль игрока), чтобы можно было собрать бывших тиммейтов; (в) проверить, что химия оживает. **Файлы:** `web/src/game/{engine,packs,score}.ts`, `pipeline` (сбор), `web/scripts/verify_engine.ts`. **Скиллы:** `reference-parity-audit`, `scoring-model`, `external-data-etl`. **DoD:** на богатом датасете два тиммейта дают положительную химию, тест подтверждает; без вечного team-exclusion, без дублей игрока. **Deps:** больше данных (M2.5/S4 collect-window). ⬜
