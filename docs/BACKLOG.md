...
- ✅ **Phantom Assassin, аркана «голая» (2026-09-07).** Сборка была верной (тело + оружие), дело в текстуре: платье «Manifold Paradox» в color-текстуре **белое**, а линии узора — прозрачные (альфа 0, 16% текстуры). В Dota белое гасит шейдер ткани, у нас Workbench рисовал его как есть — светлое облегающее «тело». Новый флаг `--white-to R,G,B[,порог]` красит почти белые непрозрачные пиксели в тёмную ткань, `--glow-from-alpha` по-прежнему красит линии узора в голубое свечение. Итог: тёмное платье с голубым лабиринтом, как в игре.
- ✅ **Terrorblade: свечение груди и клинков (2026-09-07).** Ядро груди — selfillum в альфе `terrorblade_detailmask` (0.7% текстуры): в 160-px кадре точка терялась → `--glow-grow N` расширяет маску max-фильтром (14 текселей). Клинки в Dota красит градиент `weapon_grad`, а `weapon_detailmask` = 1 всюду и как маска не годится → `--glow-mat weapon=R,G,B,сила`: светлые места текстуры материала тянутся к цвету свечения. Плюс `--light 1.6 --autoexpose 0.3 --expose-target 0.5` — база и аркана были почти чёрными. Метаморфоза (`terrorblade@meta`, `demon.vmdl_c`) не тронута: у `demon_detailmask` альфа пуста, свечение спины там — отдельный материал `demon_back_grad` с selfillum-маской, которого в glb не видно (⬜).
