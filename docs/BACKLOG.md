...
- ⬜ **Дальше волнами по 8:** Lycan, Lone Druid, Alchemist, Bane, Batrider, Bounty Hunter, Broodmother, Clockwerk → Earth Spirit, Elder Titan, Ember Spirit, Grimstroke, Gyrocopter, Io, Keeper of the Light, Kunkka… → … Порядок — по популярности в датасете (`heroes.json` / picks). После каждой волны — `npm run sim:arcade` по новым героям, полоса 30–50% в разминке.
