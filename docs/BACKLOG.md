...
- **Первая минута на Herald.** Факт из T15.1: остановка = смерть к 0:36–0:43. Вариант: разгон `spawn.base` 0.8 → 1.7 за первые 45–60 с только на Herald/Guardian (ранговое правило, а не общий конфиг) — старшие ранги и калибровка бота на них не сдвигаются; Herald перемерить ботом на двух базах по 30.
- **Порча проклятого сундука обходится**: «Оставить» кладёт предмет к ногам чистым, следующий подбор даёт +1 редкость без порчи (закреплено тестом `arcadePond.test.ts:60` и текстом кнопки); бот делает это автоматически — метрики порч недействительны. Варианты: порча едет с предметом на землю либо берётся при вскрытии.
