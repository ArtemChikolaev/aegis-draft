...
### T13.15 — Фирменные пассивки героев (владелец 2026-09-06: «больше особенностей под конкретного героя, пример — души Shadow Fiend») ✅ срез 1 (2026-09-06, a0.11.0) · ⬜ остальные герои, HUD-иконка
...
- ⬜ **Стили и самоцветы аркан** (владелец: «у Terrorblade куча гемов, у Juggernaut и Drow разные цвета арканы»). В vpk они лежат отдельными материалами и текстурами рядом с базовыми: `materials/models/items/<hero>/<part>/arcana_style1/..._style1_color_*.vtex_c`, у Pudge — `*_style1` по каждой части. План: извлекать текстуру стиля из vpk отдельно и подменять `base_color_image` в Blender (`--tex-style style1`), лист называть `<variant>~<style>`; UI и стор уже готовы.
- ⬜ **Предметы из сетов Dota** (вопрос владельца «можно ли конкретные предметы из сетов»). Да: в `pak01_dir.vpk` 14 033 модели `models/items/<hero>/…`, из них ~1300 полных сетов (части с общим префиксом и суффиксами `_head/_arms/_legs/_back/_weapon`) — ровно та же схема, что уже работает у аркан (части пришиваются к скелету базового героя через Copy Transforms). Дёшево: сет целиком = один лист `<hero>@<set>`. Дорого: смешивание по слотам — нужен послойный рендер каждой части с телом-holdout, иначе комбинаций больше, чем листов.
...
- ⬜ Дальше: дым/искры при смерти горящего, пыль под ногами при рывке, вспышка Aegis, погодные частицы ночью (Dire), снаряды врагов (осада) — трассер.
...
### T13.18 — Легендарные апгрейды: тиры «как в Death Must Die» (владелец 2026-09-06: «разбить улучшения на тиры, чтобы выпадали мега-сильные пассивки») ✅ срез 1 (2026-09-06, a0.13.0) · ⬜ больше легендарок, редкость влияет на потолок ранга
...
### T13.14 — Инвентарь предметов как в Death Must Die (фидбэк владельца 2026-09-06) ✅ срез 1 (2026-09-06, a0.10.0) · ⬜ баланс аффиксов, больше баз/уникальных
