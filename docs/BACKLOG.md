...
- **Первый замер ботом (n=12, seed `compcal`, полный акт, Herald, `--places all`, шум ±15 п.п.):** Juggernaut — все места 8.3% побед, Дикие угодья 0%, Торговый путь 0%; Sniper — все места 58.3%, Дикие угодья 58.3%, **Торговый путь 25.0%**. Гипотеза: без лагеря/кургана дальнику меньше опыта и наград чемпионов, а бот в разлом/караван ходит «в лоб»; проверить на n≥48 и отдельно без `--places`, прежде чем трогать числа. Бот: `--composition all|wilds|trade`.
