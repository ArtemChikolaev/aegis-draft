...
- ⬜ **Дальше волнами по 8:** Silencer, Skywrath, Dazzle, Jakiro, Shadow Shaman, Warlock, Enigma, Tinker → Omniknight, Abaddon, Beastmaster, Brewmaster, Centaur, Dark Seer, Death Prophet, Disruptor → … Порядок — по популярности в датасете (`heroes.json` / picks). После каждой волны — `npm run sim:arcade` по новым героям, полоса 30–50% в разминке.
