...
*(R14.1–R14.7 закрыты; размер пака 3+3 принят, пересмотр по плейтесту)*
...
- ✅ **R14.7 — рынок сужен до трёх карт в паке (2026-08-04).** `MARKET_PACK.size = 3` вместо «карта
  на каждую роль»; сужение идёт тем же `balancedPackSlots`, что обслуживает Last Dance, поэтому
  ...
  - ✅ **Решение 2026-08-04: оставляем 3+3, компенсацию не вводим.** Пересмотр — по плейтесту, а не
    по симулятору. Если забег будет ощущаться голодным, кандидаты в том же порядке: вернуть 4+4
    (компромисс) либо удешевить реролл (сделать задуманную контригру реальной). Пока `rrl` у
    агентов не вырастет, повторный прогон sim на этот вопрос не ответит — нужен живой проход.
- ⬜ **Арт, который остался за R14.5:** аватары игроков и логотипы команд. У OpenDota они есть
  (`profile.avatarfull`, `teams/{id}.logo_url`), в [players.schema.json](../schema/players.schema.json)
  поля нет вовсе — это правка контракта данных + пайплайна, а не UI. Фолбэк на монограмму
  `--sigil-*` уже написан.
