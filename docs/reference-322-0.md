...
### Подача симуляции — фазы и тайминги (дословно)

Компонент прогона держит фазу в одном стейте: `draw → groups → quali → playoffs → done`.

```js
draw:     жеребьёвка, +1 команда каждые         50 мс
groups:   новый матч в ленту                    35 мс, каждая карта серии 60 мс
                                                (лента обрезается .slice(0, 24))
quali:    подпись + раскраска команды          140 мс, пауза между маршрутами 1100 мс
playoffs: новая серия                          300 мс, карта 170 мс, после серии 140 мс
```

**`quali` — это «кто куда попал».** Три маршрута объявляются подряд, каждый своей крупной центрированной подписью, и под неё построчно красятся ровно его команды (`bg-radiant/25` · `bg-gold/25` · `bg-dire/25`):

| Подпись | Кого красит |
|---|---|
| `▲ Advancing to the Upper Bracket` | `groups.A.slice(0,4)` + `groups.B.slice(0,4)` |
| `▼ Dropping to the Lower Bracket` | `slice(4,8)` обеих групп |
| `✖ Eliminated` | `A[8]`, `B[8]` — им же проставляются 17th/18th |

Подпись живёт **только** в фазе `quali` (`d === "quali" && b && …`) и исчезает при переходе к сетке. Итог фазы ~5.2 с.

**Итоговая таблица** — `[standings.slice(0,9), standings.slice(9)]` в `sm:grid-cols-2`, то есть 9+9 колонками; ещё не доигранные места показывают `—`. Строка — одна строка (`flex items-center`), имя под `truncate`.

Наша реализация — BACKLOG **T7.10**.
