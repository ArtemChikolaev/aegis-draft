...
| Что | У нас | У них | Влияет? |
|---|---|---|---|
| Поле ботов | кусочное, медиана 84 | `N(86, 5)` кламп [76,99] | поле мягче на ~2 |
| Вероятность победы | `1/(1+e^((b−a)/12))` | `1/(1+10^(−(a−b)/22))`, экв. делитель 9.55 | их фаворит выигрывает чаще |
| Развод по группам | случайный шафл | змейка по силе | сетка честнее у них |
| Компоненты внутри команды | impact 4.9, rel 4.3 | 3.8, 5.5 | на OVR не влияет |
| Placement | нет (Liquipedia недоступна) | есть в данных | прокси — winrate на событии |

Первые три — в [tournament.ts](../web/src/game/tournament.ts), замерены, но **не применены**: на момент замера правились рейтинги, и мешать это в один заход не стали.
