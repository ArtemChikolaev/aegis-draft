...
func Default() Config {
	return Config{
		...
		// CalibrationSpread и TeamComponentWeight СВЯЗАНЫ — порознь их тюнить нельзя.
		// Spread масштабирует всё; W задаёт долю команды; но обе цели (общий sd 7.8 и
		// разброс внутри команды 2.0 — оба замерены у 322-0) зависят от обеих ручек.
		// Прогон с W=0.6/Spread=0.639 попал во внутрикомандный разброс 2.0 ровно, но
		// разнёс общий sd до 13.9: командный перцентиль много шире индивидуального
		// (σT≈28.9 против σI≈12.3, их корреляция 0.86), и разброс рангов вырос 12.3→21.7.
		// Решение системы
		//   Spread·f(W) = 7.8   и   Spread·(1−W)·7.82 = 2.0
		// даёт W=0.40, Spread=0.426 (доля команды 93% при их 92%). Mid — поправка на то,
		// что ранг центрирован не ровно в 50 (перцентиль с ties + веса ролей + округление).
		CalibrationMid: 73.8, CalibrationSpread: 0.426,
