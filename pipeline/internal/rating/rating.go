...
// плейсменты/призовые/топ-финиши в данных нулевые до Liquipedia (T1.3 ⛔). Полная модель
// v1.2.0 (placement/prize/top-finish) жила неподключённой в internal/teamsuccess и удалена
// 2026-09-02 как мёртвый код (см. git history); когда приедут данные — поменяются ВХОДНЫЕ
// числа, а отображение на игровую шкалу останется тем же.
...
// v1.13.0 — Chemistry считает только десять уникальных пар пятёрки. v1.8.0 складывала поверх
// пар вложенные тройки/четвёрки/пятёрку: одни и те же отношения учитывались повторно, общий
// cap 13 насыщался заранее, и замена на реально сыгранного тиммейта визуально ничего не меняла.
// Входные агрегаты Go не изменились, но клиентская формула и итоговый Team OVR изменились.
// 2026-09: squadSynergy эмитит только пары — группы 3–5 клиент v1.13 не читал, счёт тот же,
// поэтому версия модели не бампалась.
const ModelVersion = "v1.13.0"
