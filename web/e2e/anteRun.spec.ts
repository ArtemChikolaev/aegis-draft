...
// Пере-подобран 2026-08-09 (сдвиг рынка R14.8/R14.9 сделал camp-e2e-3 непроходным на этапе 3)
// и 2026-09-02 (T12.6: data-refresh 08-31/09-01 сдвинул пул — camp-e2e-26 стал красным на
// реальном датасете, CI на mock этого не видел).
// Сид обязан проходить на ОБОИХ датасетах: локально e2e гоняются на реальном OpenDota-слайсе,
// CI — на эфемерном mock (gen:mock). Перебор: `npm run sim:sweep` дважды (mock/real),
...
//
// Seed привязан к содержимому пула и к правилам рынка: любая правка данных или того, что рынок
// вообще предлагает, разводит забег и роняет тест НЕ на регрессии кода. Так он упал на снятии
// лимита резерва героев (снятые герои исключаются из рынка ⇒ другие офферы). Переподбирать через
// перебор, а не ослаблять проверку: `CHEAT_SEED=cheat-e2e-N npx playwright test e2e/anteRun.spec.ts
// --project=chromium -g "cheat mode"`.
// Пере-подобран 2026-08-09 тем же сдвигом рынка; критерии и двойной датасет — как у CAMP_SEED
// выше (boost-модель доживает до финала акта И static гибнет ≤6 этапов, на mock И на реальном).
...
// турнира, до которого несколько этапов. Seed подобран оффлайн: `camp-e2e-302` выдаёт карточку
// Scouting наградой первого Буткемпа (пул карточек детерминирован по seed+campId) и проходит оба
...
//
// Seed привязан к составу пула: правка данных двигает ростер, условие предмета перестаёт
