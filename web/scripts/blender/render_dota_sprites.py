            ...
            for i, fr in enumerate(sample_frames):
                ...
                if not a.no_root_lock and clip_root0 is not None:
                    ...
                    if cur is not None:
                        ...
                        # Гасим смещение корня ПОЛНОСТЬЮ, обрезая только заведомо абсурдные значения.
                        # Раньше здесь стояло «если смещение больше половины охвата — не
                        # компенсировать вовсе», и это давало ровно обратный эффект: в длинном клипе
                        # бега корень уходит далеко, компенсация выключалась, и герой просто выбегал
                        # за кадр (владелец 2026-09-06: «джагер во время бега опять пропадает» —
                        # 18 пустых кадров из 64, всего таких листов было 41 из 293). Порог в
                        # полкадра тоже не годится: он не спасал ни Meepo, ни axe@blackthorn, ни
                        # bristleback@wrathrunner — герой доезжал до края и обрезался. Замеры на
                        # моделях, ради которых порог когда-то вводили: Meepo при пределе 3 стоит в
                        # центре все 10 кадров, а персонам Dragon Knight и Mirana предел вообще ни
                        # разу не срабатывает. Смещение относительное (от первого кадра клипа), так
                        # что постоянный сдвиг корня сокращается сам; предел остаётся страховкой от
                        # мусорных значений.
                        lim = extent * 3.0
