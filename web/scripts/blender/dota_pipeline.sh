...
while IFS=$'\t' read -r id vmdl args parts; do
  ...
  "$BLENDER" -b -P "$HERE/render_dota_sprites.py" -- --glb "$GLB" --name "$id" --out "$SPRITES" $args ${PARTS:+--parts "$PARTS"} 2>&1 | grep -E 'actions in file|attached|orientation|sheet |cast:|style |autoexpose|loop |glow-|mat-map|rootlock:|WARN|Error|Traceback' || true
