...
Выход: web/public/art/sfx/dota/pack/<группа>/<имя>_N.(m4a|mp3) и pack/index.json:
  ...
  ui.{levelup,abilitylevel,coins,buy,lasthit,itemPickup,rune,heroPicked,deny,aegis,streak1..8,firstBlood,buyback}
...
ABILITIES = {
    ...
    "phantom_lancer": hero("phantom_lancer", ["spirit_lance_cast"], ["doppel_cast"], ["phantom_edge"], ["doppel_appear"]),
    "lifestealer": hero("lifestealer", ["rage"], ["open_wounds_cast"], [], ["infest"]),
    "enchantress": hero("enchantress", ["enchant_cast"], ["natures_attendants"], ["little_friends_cast"], ["impetus01"]),
    "chen": hero("chen", ["penitence_cast"], ["holy_persuasion_cast"], ["divine_favor_target"], ["test_of_faith_cast"]),
    "ancient_apparition": hero("ancient_apparition", ["cold_feet"], ["ice_vortex_cast"], ["chilling_touch_cast"], ["iceblast_release"]),
    "monkey_king": hero("monkey_king", ["strike_cast01"], ["spring_target"], ["tree_jump01"], ["fur_army"]),
    "dark_willow": hero("dark_willow", ["bramble_cast"], ["shadow_realm"], ["ley_cast"], ["wisp_strike"]),
    "keeper_of_the_light": hero("keeper", ["illuminate_discharge"], ["blinding_light"], ["chakra_target"], ["wisp_spawn"]),
    "underlord": hero("underlord", ["firestorm_cast"], ["pit_of_malice"], [], ["dark_rift_cast"]),
    "vengeful_spirit": hero("vengeful_spirit", ["magic_missile1"], ["wave_of_terror"], [], ["nether_swap"]),
    ...
    "meepo": hero("meepo", ["earthbind_cast"], ["channel_tap"], [], ["mega_cast"]),
    "io": hero("wisp", ["tether_target"], ["spirits_cast"], ["overcharge"], ["relocate"]),
...
UI = {
    ...
    **{f"streak{i + 1}": [f"{A}announcer_kill_{n}_01"] for i, n in enumerate(["spree", "dominate", "mega", "unstop", "wicked", "monster", "godlike", "holy"])},
    ...
    # Выкуп (ARCADE.buyback) — музыкальный стингер Dota по умолчанию.
    "buyback": ["sounds/music/valve_dota_001/stingers/buy_back"],
}
FX = {
