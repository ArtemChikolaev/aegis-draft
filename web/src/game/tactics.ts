...
export const TACTICS = {
  ...
  lastDance: {
     ...
     *
     *  Было 2 при базовом паке в 5 карт. R14.7 сузил базу до 3, и прежняя двойка оставляла бы
     *  ОДНУ карту: при паке меньше двух `balancedPackSlots` уже не может гарантировать и core, и
     *  support, то есть роль вымирала бы целиком — ровно тот дефект, который чинил R9.1, только
     *  теперь через заднюю дверь. Смысл trade-off'а — «меньше опций», а не «запрет роли»,
     *  поэтому цена уменьшена пропорционально базе: 2 из 5 ≈ 1 из 3. */
    marketPackPenalty: 1,
  },
} as const;
