...
export const TACTICS = {
  ...
  widePool: {
    ...
    minTags: 10,
    /** Плейтест 2026-08-31 (проба 300 сидов): намеренно широкий драфт достигает 10+ в 96%
     *  (11+ — 73%) ценой ~1.1 Team OVR синергии. При perTag 0.8 медианные +1.6 почти съедались
     *  этой ценой — карта была постной даже для того, кто под неё играет. 1.2 даёт медианные
     *  +2.4 (нетто ~+1.3 early), потолок 3.6 сопоставим с oldTeammates.max=3; поздняя игра
     *  всё равно вытесняет карту фактором редкости — арка «ранний рычаг» сохраняется.
     *  Сим-агенты условие не отыгрывают (A/B ≈0), поэтому числа калибруются пробой+руками. */
    perTag: 1.2,
    max: 3.6,
...
/** Прогресс условия Wide Pool для UI (плейтест 2026-08-31): до порога карта была полностью
 *  немой («не работает» без числа), а build-around карте обязателен счётчик «N из 10» — иначе
 *  игрок не видит, двигают ли его пики к цели. Отдельный чистый хелпер, а не source с delta 0:
 *  запись в sources сделала бы невыполненную карту «активной» для зарядов/рейла (activeCardIds). */
export function widePoolProgress(ctx: TacticContext): { distinct: number; need: number } {
  return { distinct: distinctGameplayTags(ctx.assignedHeroes), need: TACTICS.widePool.minTags };
}
