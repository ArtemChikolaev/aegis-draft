...
export const MANAGER_ECONOMY_VERSION = "m1.7.0"; // m1.7.0: оффсезонный кап зарплат + тренировочный сбор (замер sim_manager 2026-08-24: cheap копил $7.1M без давления, greedy жил в −$2M без последствий)
...
/** Оффсезонный тренировочный сбор (m1.7.0) — синк копилки, покупающий РАЗВИТИЕ: дрифт формы
 *  каждого игрока смещается на +driftBonus (кламп ±3 остаётся, механизм тот же, что
 *  happiness-bias в offseasonDrift). Замер до: cheap-стратегия копила $7.1M к 6-му сезону при
 *  нуле титулов финала — деньгам не на что было влиять. Зарплаты сбор не пересматривает:
 *  переговоры уже подписаны, форма растёт после них. */
export const OFFSEASON_BOOTCAMP = { costK: 150, driftBonus: 1 } as const;
