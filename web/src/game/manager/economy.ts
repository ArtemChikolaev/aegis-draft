...
/** Оффсезонный тренировочный сбор (m1.7.0, лестница с m1.8.0) — синк копилки, покупающий
 *  РАЗВИТИЕ: каждый купленный уровень смещает дрифт формы всех игроков на +driftBonus (кламп
 *  ±3 остаётся — верхние уровни часто упираются в него, отдача убывает по построению).
 *  Лестница цен вместо одной покупки — по замеру m1.7.0: один сбор $150k был дешевле
 *  сезонного профицита ~$1.1M, и копилка cheap-контроля продолжала расти до $6.6M; теперь
 *  полный сбор стоит $1.6M за оффсезон, а сцепка с оффсезонным капом зарплат делает богатство
 *  самоограниченным: форма растит OVR → пересмотры дорожают → кап требует отпускать.
 *  Зарплаты сам сбор не пересматривает: переговоры уже подписаны, форма растёт после них. */
export const OFFSEASON_BOOTCAMP = { costsK: [150, 450, 1000] as readonly number[], driftBonus: 1 } as const;

/** Оценка зарплаты филлера для оффсезонного бюджета: точная цифра появляется только при
 *  подтверждении (свой Rng-поток), а гейту хватает консервативной оценки дешёвейшего кандидата. */
export const FILLER_WAGE_ESTIMATE_K = 8;
