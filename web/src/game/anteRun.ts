...
export const ANTE_THREAT = {
  ...
  dynastyMultPerAct: 0.14,
} as const;

/** Множитель поля по пройденным актам (R7.2, мультипликативная часть). Внутри акта — 1.
 *  За пределами сезона акты идут своим, более пологим шагом (`dynastyMultPerAct`): множитель
 *  на границе сезона непрерывен — Династия продолжает ту же кривую, просто с другим наклоном. */
export function anteFieldMult(absoluteStageIndex: number, season: SeasonRules = SEASON): number {
  ...
  return ((1 + ANTE_THREAT.multPerAct) ** seasonActs) * ((1 + ANTE_THREAT.dynastyMultPerAct) ** dynastyActs);
}
