...
export interface RunConfig {
  ...
  cheatMode?: boolean;
  /** Stakes (T6.4): добровольные правила, под которыми играется весь сезон Roguelite Run —
   *  то же множество, что мутаторы кругов Династии (решение 2026-08-09 «мутаторы = Stakes»).
   *  С T6.4-2 их может быть несколько (комбинация открывается первой победой со ставкой).
   *  Опционально — старые сейвы/ссылки читаются без них. Только Roguelite Run; с Cheat Mode
   *  не совмещается (несоревновательный забег не носит соревновательную метку). */
  stakes?: readonly MutatorId[];
  /** УСТАРЕЛО (T6.4 срез 1, одна ставка): читается ТОЛЬКО ради сейвов/ссылок эпохи b1.41.0,
   *  новые конфиги пишут `stakes`. Единственный читатель — `stakesOf`. */
  stake?: MutatorId;
  /** Playbook (T6.4 срез 2): 6–10 карт (тактики/предметы), из которых берутся карточные награды и
   *  trade-in этого забега. Канонический порядок — каталога (`normalizePlaybook`). Опционально —
   *  без него пул полный, как раньше; только Roguelite Run; на RNG вне фильтра не влияет. */
  playbook?: readonly string[];
}

/** Нормализованные Stakes конфига: новые `stakes` либо legacy-одиночный `stake` (b1.41.0).
 *  ЕДИНСТВЕННАЯ точка чтения обоих полей — движок, стор, сим и карьера обязаны звать её,
 *  а не разбирать поля сами. */
export function stakesOf(config: Pick<RunConfig, "stake" | "stakes"> | null | undefined): readonly MutatorId[] {
  if (!config) return [];
  if (config.stakes && config.stakes.length > 0) return config.stakes;
  return config.stake ? [config.stake] : [];
}
