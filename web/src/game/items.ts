...
export const ITEM_RARITY = {
  ...
  /** Платный тир предмета (LG3-хвост, b1.46.0): за золото поднимается ТОЛЬКО standard → refined
   *  (×1.25); exotic и arcana — только дроп/награда-улучшение. Замер 300 сидов (select-build,
   *  полоса PRD 30–40%): любой платный путь до exotic давал 52–61% и 5–12 покупок за забег при
   *  цене 4…36 и гейте по кривой — золото уходило в тиры целиком; refined-only с эскалацией 0.75:
   *  42.3% (было 35.0), а «слепой» synergy-opt 15 → 33%: дешёвый ограниченный рычаг помогает
   *  слабым билдам сильнее, чем оптимальным. */
  refinedCost: 4,
} as const;

/** Базовая цена платного тира; null — платно поднимается только standard → refined. */
export function itemUpgradeCost(from: Rarity): number | null {
  return from === "common" ? ITEM_RARITY.refinedCost : null;
}
