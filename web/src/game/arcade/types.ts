...
export interface ArcadeInput {
   ...
   *  Токен нейтралки использует те же коды: 1..3 — взять вариант, 5 — пропустить.
...
export interface Projectile {
  ...
  attack: boolean;
  /** Чей снаряд (умение героя, призыв…): в полёте фаза сима уже другая, происхождение урона едет со снарядом. */
  origin: DmgOrigin;
}
...
/** Происхождение урона по врагу — для правил «с автоатаки героя» / «с умений» / «не с призывов» (Vampiric Spirit, Backstab,
 *  Кровавик). НЕ вид эффекта: `FxKind` — картинка попадания, у питомца и героя она одна («hit»), у добивающего умения — «crit».
 *  `attack` — основной удар автоатаки героя (в т.ч. крит и удары по соседям милишника); `ability` — слоты Q/W/E/R и их
