...
export const ARCADE_CONFIG_VERSION = "a0.77.0";
...
export const ARCADE = {
  ...
  /** Строй стрелков (T13.81, аудит §4): с `fromMin` раз в `every` линия из `count` лучников через `spacing` px держит `range` от героя;
  ...
  /** Выкуп (Buyback, как в Dota): смерть без Aegis и Reincarnation открывает окно «выкупиться» — мир стоит. Цена — большее
   *  из `base + perMin × минута` (×`growth` за каждый прошлый выкуп) и доли `goldShare` золота на руках: как у Dota, где цена
   *  растёт с ценностью героя, — иначе накопленное к концу акта золото делало выкуп бесплатной жизнью (бот 41.7 → 81.7% побед).
   *  Не больше `maxPerAct` за забег (аналог перезарядки выкупа); встаёт с `hpFrac` здоровья, неуязвимостью и толчком, как после
   *  Aegis. Не хватает золота — окна нет, это смерть. Для отметки «без единой смерти» выкуп — воскрешение. */
  buyback: { base: 100, perMin: 25, growth: 2, goldShare: 0.5, maxPerAct: 1, hpFrac: 0.5, push: false as boolean, invulnSec: 1.5 },
  /** Чемпионы мест стоят на карте с начала забега (с множителями 0-й минуты), а будят их позже — к наградам exotic/arcana
   *  они были слабее рядового огра (T15.5). При первом пробуждении HP и урон растут, как у леса к текущей минуте, но не
   *  дальше `scaleCapMin`: поздний визит — длиннее бой, а не стена. */
  champion: { scaleCapMin: 6 },
  ...
  spawn: {
    ...
    base: 1.7,
    /** Мягкий старт Herald/Guardian (ранговое правило `gentleStart`, T15.5): база спавна растёт от `from` к `base` за `sec` —
     *  стоящий на месте новичок умирал к 0:36–0:43 при 1.7 врага/с против ~0.8 убийства/с. Опыт и золото убийств в этом
     *  окне ×(base / текущая база): давление ниже, темп роста героя прежний. Старшие ранги не меняются. */
    gentleStart: { from: 0.8, sec: 60 },
    perMin: 0.85,
    cap: 460,
    ringMin: 560,
    ringMax: 680,
   ...
   *  в минуту против сундука раз в 150 с (T15.5). Остальные элиты роняют всегда. */
  loot: { chestFirstAt: sec(60), chestEvery: sec(150), chestLifetime: sec(90), distMin: 220, distMax: 320, commonChance: 0.004, supportChance: 0.2, bagCap: 12, lootLifetime: sec(90), tier2At: sec(7 * 60), tier3At: sec(14 * 60) },
