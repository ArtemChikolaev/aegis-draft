...
/** Индексы клипов: сетевой сбой не запоминается как «клипов нет» (netRetry.ts) — чтение повторится после паузы. */
const sfxIndex = new LazyJson<Record<string, HeroSfxEntry>>(`${ROOT}index.json`, {});
...
const IMPACT: Record<string, "blade" | "heavy" | "blunt" | "light" | "none"> = {
  ...
  silencer: "light", skywrath_mage: "light", dazzle: "light", jakiro: "light", shadow_shaman: "light", warlock: "light", enigma: "light", tinker: "light",
  omniknight: "heavy", abaddon: "blade", beastmaster: "blade", brewmaster: "blunt", centaur: "heavy", dark_seer: "blunt", death_prophet: "light", disruptor: "light",
  ...
  phantom_lancer: "blade", lifestealer: "heavy", enchantress: "light", chen: "blade", ancient_apparition: "light", monkey_king: "blunt", dark_willow: "light",
};
...
const voiceIndex = new LazyJson<VoiceIndex>(`${ROOT}voice/index.json`, {});
...
function voiceKey(hero: string): string {
  const has = (k: string) => { const e = voiceIndex.value?.[k]; return !!e && Object.values(e).some((files) => files && files.length > 0); };
...
export function preloadHeroVoice(hero: string): void {
  voiceIndex.whenReady(() => { const key = voiceKey(hero); const e = voiceIndex.value?.[key]; if (!e) return; for (const files of Object.values(e)) for (const f of files ?? []) preloadSample(vurl(key, f)); });
}
...
export function heroVoice(hero: string, cat: VoiceCat, now: number, chance = 1, cooldownMs = 0): boolean {
  ...
  const pool = voiceIndex.value?.[key]?.[cat];
...
export function heroSpinSfx(hero: string, spinning: boolean): void {
  ...
  if (spinning && !spinStop && e?.spinLoop) {
    ...
    spinStop();
    spinStop = null;
    const stop = sfxIndex.value?.[spinHero]?.spinStop;
