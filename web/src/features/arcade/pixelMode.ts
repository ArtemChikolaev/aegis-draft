...
// буфере в N CSS-пикселей на арт-пиксель и растягивается nearest, спрайты берутся из `dota_px2/` (кадр героя 160 px с запасом 1.4,
// крипа 96) или `dota_px/` (80 / 48), иконки — из `items_px/`/`abilities_px/`.
...
/** Плотные листы (`dota_px2/`, кадр героя 160) нужны только при факторе 1: при 2+ хватает кадров `dota_px/` (80). */
