...
## Статус
Играбельное ядро в проде: старт → драфт → турнир → итог, Team Packs и Mixed Draft, счёт с пентагоном, RU/EN + system/light/dark темы. Поверх ядра: сейв и резюм незавершённого забега, шеринг забега ссылкой и старт по сид-коду (челлендж по сиду — паки те же, драфтишь сам), история забегов в localStorage, hardcore-режим (подсказки выключены), дейлик (общий сид дня без сервера), Штаб (коллекция карт рогалика, трофеи и Playbook — набор карт, ограничивающий награды забега) и справочники — популярность героев и ростер-веб. Данные — **реальный OpenDota-слайс** (события/паки/игроки/рейтинги), обновляется по расписанию. Фронт — на собственной design-system (токены + UIkit + features). Бэкенд (Go API) — скелет, поднимается по [ADR 0002](docs/adr/0002-backend-now.md). План задач — [docs/BACKLOG.md](docs/BACKLOG.md).
