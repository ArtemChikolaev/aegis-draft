...
## Перед «готово»
Скилл `self-review-checklist` + `bash .claude/skills/self-review-checklist/tools/antipatterns_grep.sh`. Go: `gofmt -l`, `go vet`, `go build`. TS: `tsc --noEmit`. Менял данные → валидатор `node .claude/skills/data-contract/tools/validate_data.mjs`. Нет несвязанных правок в diff. **Push ≠ deploy:** после пуша дождись CI и проверь live; `tsc`/`build` не ловят ошибки в `verify_*`-скриптах (их видит только CI-шаг verify). `npm run verify` ждёт мок — при реальном `public/data` сначала `gen:mock`.
