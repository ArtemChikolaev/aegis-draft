...
- `schema/*.schema.json` — **единственный источник правды** о формате статических ETL-данных (не локальных типов Аркады и самостоятельных API DTO). Меняешь формат → сначала схему, потом оба конца (Go `pipeline/internal/model` + TS `web/src/types`).
...
- **Backend (`server/`, по [ADR 0002](docs/adr/0002-backend-now.md)):** гибрид — игровые данные остаются static-first (CDN), сервер держит **только** пользовательское/общее состояние (аккаунты/сейвы/лидерборд/дейлик/Manager). Слои `internal/{transport,service,store,model}` не пробивать; дейлик/лидерборд валидирует сервер ре-симуляцией на Go (целевая архитектура ADR; переиспользовать доступный общий пакет, не импортировать `pipeline/internal/*` из отдельного Go-модуля). Детали — скилл `backend-architecture`.

## Перед «готово»
Скилл `self-review-checklist` + `bash .claude/skills/self-review-checklist/tools/antipatterns_grep.sh` для кода/инструментов. Проверки соразмерны изменению: Go — из затронутого модуля `gofmt -l`, `go vet`, `go build`, тесты; TS — `npm run typecheck` и релевантные тесты из `web/`; данные — `node .claude/skills/data-contract/tools/validate_data.mjs`. Полный web-прогон при затрагивании игровой логики: `npm run validate:data && npm run test`. Mock/golden проверять в изолированной копии или CI: `gen:mock` не запускать поверх рабочего датасета и не возвращать данные слепым checkout/restore. Для docs/скиллов — проверки из `docs/ai/PRINCIPLES.md`, без сборки игры. Чужие изменения сохранять. **Push ≠ deploy:** если push поручен и выполнен, дождись CI и проверь live.

## Сравнение с референсом
Если пользователь просит «потестить и сравнить с референсом», «пройтись по оригиналу», «найти, чего не хватает» или сверить с 322-0/Balatro — **обязательно** используй скилл `reference-parity-audit`. Для аудита поведения пройди живьём референс и нашу реализацию, классифицируй различия с доказательствами; недоступное помечай unknown. Для запроса идей отделяй исследование и предложения от live parity; подтверждённые/порученные продуктовые решения синхронизируй с PRD, а баги и gaps — с BACKLOG. Статус ✅ в backlog сам по себе не является доказательством.

## Дисциплина скиллов
1. Повторяющуюся инструкцию — в скилл/CLAUDE.md, а не объяснять заново.
2. Скилл = `description` (когда брать) + инструкции; tools/references добавляются по необходимости.
