...
Порт **5173 оставляем пользователю** (он держит на нём свой `npm run dev`). Свой preview/dev-сервер для ручной проверки агент поднимает на **5273**: `.claude/launch.json` уже настроен, preview-инструмент стартует его сам. Не занимай 5173.
**Спрашивать разрешение на старт сервера не нужно** — `preview_start`/`preview_stop`/`preview_logs` разрешены в `.claude/settings.json`; поднимай 5273 молча, когда правку надо посмотреть живьём. Три конфигурации: `web-dev` (`npm run dev`, обычная проверка UI), `web-preview` (`npm run preview` поверх `npm run build` — нужна для всего, что живёт только в прод-сборке: service worker и офлайн, ADR 0003) и `web-preview-pages` (то же под сабпутём `/aegis-draft/`, как на GitHub Pages — для scope воркера и относительных путей манифеста). Origin у них общий, поэтому SW от `web-preview` остаётся у браузера и после переключения на dev. Защита двухслойная: dev-сервер отдаёт по `/sw.js` самоуничтожающийся воркер (`web/vite-plugin-sw-killswitch.ts` — старый precache иначе подменял бы index.html и новый код вообще не запускался), а новый бандл дополнительно сносит регистрации (`registerServiceWorker`). Симптом «make dev-all, а изменений нет» = старый воркер; лечится двумя перезагрузками страницы или DevTools → Application → Service Workers → Unregister. Если 5273 уже занят **другой агентской сессией** — подключайся браузером к работающему серверу (`preview_start` с `url`), третий порт не поднимай; но помни: это общее рабочее дерево, чужие правки прилетают HMR-ом в твою страницу — багрепорты с такого прогона только со статусом `unknown` (правило в скилле `reference-parity-audit`). Исключение — CI/Playwright (`playwright.config.ts`) и дефолтный `npm run dev`: они остаются на 5173 без изменений, это правило только про сервер, который агент запускает сам для инспекции. 5273 — отдельный origin, так что его localStorage/сейвы не пересекаются с пользовательским 5173 (для теста resume это скорее плюс).
